...
### D9 上下文值放宽、传播注记与类型化通道边界（对标行业复评，2026-08-23）

D8 后对照 Effect（Cause 堆叠/类型化错误通道）、Rust anyhow（`context()` 途中附加）、
Zod/Stripe（结构化 issues）复评，三处差距裁决如下：

1. **上下文放宽为只读 JSON 值（D9a）**：`ErrorContext` 值域从四标量扩为递归只读 JSON
   （标量 / 数组 / 嵌套对象）。结构化校验事实（字段路径列表等，v1 `ValidationError.details`
   一族）可进 context，不再逼迫消费者在根契约外自建通道；JSON 可序列化仍是硬边界
   （日志与出站安全）。
2. **传播注记 `annotate()`（D9b）**：解决「错误上浮途中无法累积语境」而**不引入包装**——
   符号键、非枚举、实例稳定的注记通道；外层补事实（requestId、channelId 等）后
   `recordOf` 按时间序合并（constructor 上下文为底，后写注记胜出）。instanceof 与
   三性分类全程不动——这是相对 anyhow 包装式 `context()`（丢失 downcast）的明确改进。
3. **类型化错误通道：裁决不做（D9c）**：TS 无 typed throws；全仓 Result 化与既有
   throw/Hono onError 生态及 v1 行为等价迁移纪律冲突。目录即「可抛码」的声明面
   （grep 单一来源；文档生成随第一个消费者单元）。此为语言级边界声明，非遗漏。
...
| 包装式富化（`withContext` 返回新错误） | instanceof 链断裂——固化类经富化退化为基类，精确捕获失效；D9b 采纳实例稳定注记 |
| 可变 context（直接改写 `e.context`） | 构造点事实与传播事实混同，身份漂移；注记与构造上下文分离可审计（时间序可重建） |
| 全仓 Result/Either 通道 | 推翻 throw/Hono 生态与行为等价迁移纪律，收益（编译期可抛集）在目录声明面已大部分兑现 |
