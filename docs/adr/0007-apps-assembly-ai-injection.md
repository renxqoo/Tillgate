# ADR-0007: apps 装配面直依赖 `@tillgate/ai` 注入 inference 的形态认可
...
1. `@tillgate/ai` 的 import **只允许出现在两个位置**：
...
3. 机器门禁：各 app 架构测试锁定「除上述两位置外，src 内不得出现
   `@tillgate/ai` import」（gateway 已有；worker/admin-api 同款补齐，随本 ADR 落地）。
...
| 经 `@tillgate/ai/composition` 子入口 | composition 子入口是能力包装配契约（§5.3）；`ai` 是零依赖库不是能力包，加 composition 是仪式化——否决 |
