...
| 层 | 选型 | 说明 |
|---|---|---|
| 语言/运行时 | TypeScript + **bun**（1.4） | 全链 bun：运行时、构建（bun build / bun x next build）、测试（bun x vitest）都跑在 bun 上 |
...
| 事件唤醒 | **PG LISTEN/NOTIFY** | 结算即时唤醒（纯门铃，可丢）；结算以 DB poll 为权威，定时扫描兜底 |
...
| 包管理/构建 | bun workspaces / bun build（服务端） | Next.js 自带构建；无 dts——类型走 exports.types 直指源码 |
...
│   ├── worker/         # 后台循环消费者：结算/回收/生成轮询/对账哨兵/告警投递/分区维护（领域逻辑在 packages/service；PG NOTIFY 仅做唤醒）
...
| 依赖 | bun.lock 锁文件 + 定期扫描（可选 trivy 进 CI） |
