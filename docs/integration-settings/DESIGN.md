...
| admin UI 形态 | 与 `/dashboard/settings` 现有卡片风格一致；**设置按钮放卡片右上方、与标题对齐**；**卡面不显示配置字段值**（配置收进弹窗，2026-08-25 裁决）；**SMTP 无独立卡**——配置/启停挂「邮箱验证码二次登录」卡（邮件通道是 2FA 的实现细节，不另立邮件服务面）；**OAuth 三键（base/github/google）合并为一张「OAuth 登录」组合卡**——基地址是共享部署配置与 OAuth 总闸，不独立占卡（2026-08-25 裁决） |
...
- [x] 7 个集成全部可在 `/dashboard/settings` 配置/启停，风格与现有卡片一致，
      ...
      平铺配置字段值（收进弹窗），SMTP 挂「邮箱验证码二次登录」卡（配置按钮 +
      弹窗内启停开关），独立集成卡余 6 张；同日再收敛：OAuth 三键合并为
      一张「OAuth 登录」组合卡（基础地址行 + GitHub 行 + Google 行，各自
      独立启停/配置/step-up），独立集成卡余 3 张 + 2 张组合卡；
