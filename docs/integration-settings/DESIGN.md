...
| admin UI 形态 | 与 `/dashboard/settings` 现有卡片风格一致；**设置按钮放卡片右上方、与标题对齐**；**卡面不显示配置字段值**（配置收进弹窗，2026-08-25 裁决）；**SMTP 独立卡**——2026-08-25 二次裁决推翻首裁「挂 2FA 卡、不另立邮件服务面」：邮件通道是系统级配置（登录验证码/找回密码/2FA/告警共用），与 OAuth/支付同级进集成卡区，门控粒度对齐 `settings:integrations`；「邮箱验证码二次登录」卡回归纯个人自助（SELF 域，不残留通道状态行——同日裁决 D1）；**无 `settings:integrations` 权限者隐藏配置/启停操作位，保留只读卡**（2026-08-25 裁决 D1；与 RBAC 页 canUpdate/canDelete 模式同源） |
...
- [x] 7 个集成全部可在 `/dashboard/settings` 配置/启停，风格与现有卡片一致，
      设置按钮位于卡片右上方与标题对齐（用户裁决）；2026-08-25 收敛：卡面不再
      平铺配置字段值（收进弹窗）；2026-08-25 二次裁决修订：SMTP 独立成卡进
      集成卡区（推翻首裁「挂 2FA 卡」——系统级配置与个人自助分离，见分叉表），
      独立集成卡共 6 张，编辑弹窗不再含启停开关（启停在卡面按钮）；
