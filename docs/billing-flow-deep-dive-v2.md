# 扣款全流程深度解析 · v2（gateway-v2 / packages/ai / worker-v2 / wallet）

> 本文是 **v2 生产链路**的扣款真相文档（2026-08-19 资金安全大审查后口径）。
> 旧版 [`billing-flow-deep-dive.md`](billing-flow-deep-dive.md) 描述的是 v1 管线
> （apps/gateway + packages/ledger + `reserved_balance` 列 + uncertain 状态），仅作历史参考，
> **v2 的实现以本文为准**。
>
> 覆盖范围：`apps/gateway-v2`（热路径）、`packages/ai`（上游传输）、`packages/service`
> （billing/settlement/generation 域）、`apps/worker-v2`（结算）、`packages/wallet`
...
```
客户端 ──HTTP/SSE──▶ apps/gateway-v2（热路径：鉴权→限流→预扣→调上游→落收据）
                          │  ▲
                    ...
                    PostgreSQL（billing_requests = 唯一事实源）
                          ...
                          ▼
apps/worker-v2（结算：BullMQ 唤醒 + 认领→实扣→对账→恢复）─▶ Redis（限流/门铃队列，必配）
        ▲
...
| 应用 | apps/gateway、apps/worker | apps/gateway-v2、apps/worker-v2（v1 待退役） |

---

## 2. 分图①：鉴权

> 代码：`apps/gateway-v2/src/middleware/api-key.ts`、`packages/repository/src/credential.repo.ts`
...
Authorization: Bearer <token>
   ...
   └─ 否则 ──▶ 【JWT】（jose HS256，iss=ai-gateway / aud=ai-gateway-api 强制）
         ...
         └─ typ=playground：client-api-v2 每请求现签（TTL 300s，rpm 10 / tpm 200k）
...
> 代码：`apps/gateway-v2/src/rate-limit/gate.ts`、`packages/core/src/rate-limiter.ts`
...
> 代码：`apps/gateway-v2/src/pipeline/output-cap.ts`
...
> 代码：`apps/gateway-v2/src/pipeline/run-chat.ts`、`packages/ai/src/create-ai.ts`
...
> 代码：`apps/gateway-v2/src/pipeline/run-chat.ts`（streamReceiptUsage）、`receipt.ts`、
...
> 代码：`apps/worker-v2/src/run-once.ts`、`wakeup.ts`、`packages/service/src/settlement/*`
...
| | 预扣（授权时，gateway-v2） | 实扣（结算时，worker-v2） |
|---|---|---|
...
（E2E 钉死：`apps/gateway-v2/src/__tests__/e2e-cost-drain.test.ts`）：
...
| 模块 | 文件 | 职责 |
|---|---|---|
| gateway-v2 | `src/pipeline/run-chat.ts` | 管线编排（流式/非流式双分支 + 估算收据 + TPM 归还） |
| gateway-v2 | `src/pipeline/output-cap.ts` | outputCap 口径 + 转发钳制（D2） |
| gateway-v2 | `src/pipeline/receipt.ts` | 收据装配（价格快照 + usage + units 计量） |
| gateway-v2 | `src/quote/build-quote.ts` | 候选链解析 + 系数 + 单位上界 + 定价策略 |
| gateway-v2 | `src/routes/oauth-token.ts` | App JWT 签发（iss/aud + ipGuard） |
...
| worker-v2 | `src/run-once.ts` / `wakeup.ts` / `index.ts` | 认领批次 + 唤醒排空 + 生命周期 |
