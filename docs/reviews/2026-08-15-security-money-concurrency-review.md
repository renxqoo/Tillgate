...
> **v2 移植注（2026-08-23）**：本文是 v1 时代的审查存档。所指问题已在 v1 全部修复并通过全量回归；
> v2 结构重构迁移时对各项修复逐项保持行为等价（v1 → v2 映射见
> [project-structure-refactoring.md](../project-structure-refactoring.md)）。
> 文中 v1 路径与脚本引用（如 `packages/ledger/...`、`scripts/security-audit/...`）保留历史原貌，不做改写。
