...
| 维度 | 现行设计 |
|---|---|
| 账本 | wallet 双分录内核：`wallet_authorizations`（hold）+ `wallet_legs`（复式流水），`in_flight` 为冻结口径 |
| 结算余额口径 | `balance + credit_limit − in_flight = available`（`assertCanDebit` 唯一守卫） |
| 超预扣 | `#over` 同事务补充授权；**余额不足时降级收满预留**（不死信，见 §7d） |
| 异常态 | 无 uncertain 挂起：完成缺 usage / 用户取消 → 估算结算；上游异常 / 崩溃 → 即时释放 |
| 预扣不足 | 未声明 `balanceFloor` 的模型**足额 fail-closed**（402 整单拒绝，上游零调用） |
