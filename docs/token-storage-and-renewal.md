...
> 本文回答四个问题：本项目会话为什么**不用 refresh token**；四端（BFF / 纯 SPA / App / 本项目）
> 各自的正确形态；服务端各机制的实现要点；Nginx 同域/跨域的落地配置。
> 会话代码事实源：`packages/identity/src/session.ts`、`packages/api-client/src/session.ts`。
...
| 项 | 实现 |
|---|---|
| Token | 单 JWT（HS256）。client 面 `type:'user'`（密钥 `JWT_SECRET`）；admin 面 `type:'admin'`（**独立密钥** `ADMIN_JWT_SECRET`，双身份物理隔离） |
| 有效期 | `SESSION_TTL_SECONDS`，client-api 与 admin-api 各自配置，默认 **86,400s = 24h 固定**；cookie `maxAge` 与 JWT `exp` 对齐 |
| 存放 | Next.js **BFF 持有**：`ag_session` / `ag_admin_session` cookie（httpOnly / sameSite=lax / 生产 secure / path=/）——浏览器 JS 从头到尾碰不到 token |
| 吊销 | ① 每请求查库校验账号 status（封禁/注销即时生效）② jti 黑名单单会话强制下线（Redis 加速 + DB 权威；登出即吊销 jti） |
| 续期 | **无**——活跃用户 24h 整点登出（滑动续期方案见 §5.2） |

## 2. 为什么不用 refresh token（威胁模型论证）
...
2. **吊销已集中**：每请求查库 + jti 黑名单，不需要刷新端点充当吊销集中点。
...
（`identity-core` 的 `identity_session_anchors` 表是现成地基）。
...
| 吊销 | rotation + 重用检测 | 同左 | rotation + 设备级吊销 | status 查库 + jti 黑名单 |
| CSRF | SameSite=Lax 覆盖 | None + 自定义请求头 | N/A | SameSite=lax 覆盖 |
...
### 5.1 本项目现状（代码事实，供阅读入口）

- 签发：`identity/session.ts` `signSession({ type, id, expiresInSeconds })` →
  jose HS256，payload 含 `sub/jti/iss/exp`；默认 TTL 常量 `SESSION_DEFAULT_TTL_S`
- cookie 写入：`api-client/session.ts` `setSessionToken` / `setAdminSessionToken`
  （httpOnly + sameSite lax + secure(生产) + maxAge=TTL）
- 吊销：登出路由吊销 jti（Redis 黑名单 `identity` 包；查询 fail-open，DB 权威）；
  中间件每请求查库校验 status
- 配置：`SESSION_TTL_SECONDS`（client-api / admin-api 各自 config schema，默认 86400）

### 5.2 滑动续期（本项目可加，无需 refresh 机制）

语义：活跃用户不掉线；闲置 24h 照样登出；安全语义不变。实现要点（BFF 层）：

```ts
// BFF 会话读取处（admin/client 的 server 侧）：
const token = await getSessionToken();
const { exp } = decodeJwt(token);
if (exp - now < TTL_S / 2) {              // 剩余寿命 < 一半 → 静默重签
  const oldJti = decodeJwt(token).jti;
  const fresh = await fetch(`${API}/v1/auth/renew`, {   // 新端点：验旧签新
...
服务端 `/v1/auth/renew`：验签旧 token（未过期 + jti 未吊销）→ 同 subject 签新 token
（新 jti）→ 旧 jti 入黑名单（TTL = 旧 token 剩余寿命）。admin 面建议**不加**（管理面
短会话是最佳实践），仅 client 面启用。
...
### 6.3 SSE 流式（本网关生产实配，见 docker/nginx/nginx.conf）
...
BFF 形态？     → 本项目：单 JWT + httpOnly + status 查库 + jti 吊销，不需要 refresh
