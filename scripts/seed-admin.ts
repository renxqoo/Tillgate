 ...
 * 用法：pnpm --filter @ai-gateway/db exec tsx ../../scripts/seed-admin.ts --password=xxx [--email=admin@ai-gateway.local]
 * 幂等：存在则更新密码，不存在则创建。
 */
...
if (!password || password.length < 8) {
  ...
  console.error(
    '  用法：pnpm --filter @ai-gateway/db exec tsx ../../scripts/seed-admin.ts --password=YourStrongPass1',
  );
  process.exit(1);
}
