...
const psql = (sql) => {
  const out = execSync(`psql "${DATABASE_URL}" -At -F '|' -c ${JSON.stringify(sql)}`, {
    encoding: 'utf8',
  });
  return out.trim();
};
...
async function main() {
  ...
  const [[uid]] = psqlRows(
    `INSERT INTO users (issuer, subject, identity_provider, display_name, balance, rate_card_id, status) VALUES ('test', '${subject}', 'local', 'E2E', 1, 1, 0) RETURNING id`,
  );
  psql(
    `INSERT INTO api_keys (key_hash, key_preview, user_id, name, status) VALUES ('${keyHash}', 'ag_****${token.slice(-4)}', ${uid}, 'e2e', 0)`,
  );
  ...
  try {
    ...
    const r1 = await callGateway(
      token,
      '请写一篇关于人工智能发展历史的详细介绍，要求至少600字，分多个段落展开论述，越详细越好。',
    );
    console.log(`   HTTP ${r1.status}`);
    if (r1.status !== 200) {
      console.log('  ', JSON.stringify(r1.body).slice(0, 300));
      throw new Error('第一次应 200');
    }
    ok('上游真实返回 200');
    // 等结算
    let bal1 = null,
    ...
    for (let i = 0; i < 40; i++) {
      logs1 = psqlRows(
        `SELECT amount, payg_amount, status, input_tokens, output_tokens FROM usage_logs WHERE user_id=${uid} ORDER BY id`,
      );
      if (logs1.length >= 1) {
        bal1 = psql(`SELECT balance FROM users WHERE id=${uid}`);
        break;
      }
    ...
    bal1 = Number(bal1);
    console.log(
      `   余额: 1 → ${bal1} | usage: amount=${logs1[0][0]} payg=${logs1[0][1]} status=${logs1[0][2]} tokens=${logs1[0][3]}+${logs1[0][4]}`,
    );
    ...
    const [[bb, ba, amt]] = psqlRows(
      `SELECT balance_before, balance_after, amount FROM transactions WHERE user_id=${uid} AND type='consume'`,
    );
    if (Math.abs(Number(bb) - Number(ba)) !== Math.abs(Number(amt)))
      throw new Error(`流水不变量破坏`);
    ...
    if (r2.status !== 402) {
      ...
      throw new Error(`应 402，实际 ${r2.status}`);
    }
    ok('欠款用户被拦截（402）');
    ...
    execSync(
      `redis-cli -h 127.0.0.1 -p 6379 -a root123 DEL billing:balance:${uid} >/dev/null 2>&1`,
    );
    ...
    if (r3.status !== 200) {
      ...
      throw new Error(`充值后应 200，实际 ${r3.status}`);
    }
    for (let i = 0; i < 40; i++) {
    ...
    const logs3 = psqlRows(
      `SELECT amount, status FROM usage_logs WHERE user_id=${uid} ORDER BY id`,
    );
    ...
    // === 最终对账 ===
    section('最终对账');
    const consume = psqlRows(
      `SELECT amount, balance_before, balance_after FROM transactions WHERE user_id=${uid} AND type='consume' ORDER BY id`,
    );
    console.log(`   consume 流水 ${consume.length} 条`);
    for (const [a, b, c] of consume) {
      if (Math.abs(Number(b) - Number(c)) !== Math.abs(Number(a)))
        throw new Error(`流水不变量破坏: ${b}-${c}≠${a}`);
    }
    ...
    psql(
      `DELETE FROM request_logs WHERE api_key_id IN (SELECT id FROM api_keys WHERE user_id=${uid})`,
    );
