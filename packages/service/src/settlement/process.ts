...
export interface ProcessClaimDeps {
  settleClaim: (ctx: RunContext, claim: SettlementClaim) => Promise<SettleClaimResult>;
  finishFailure: (ctx: RunContext, claim: SettlementClaim, error: unknown) => Promise<FailureOutcome>;
}

export function createProcessClaimUseCase(deps: ProcessClaimDeps) {
  return async function processClaim(
    ctx: RunContext,
    claim: SettlementClaim,
  ): Promise<ClaimOutcome> {
    try {
      const result = await deps.settleClaim(ctx, claim);
      ...
      return deps.finishFailure(ctx, claim, error);
    }
  };
}
