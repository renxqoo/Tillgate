 ...
 * - children 放 DataTable；分页条在 page/pageSize/total 齐备时自动渲染。
...
export function ListPage({
  ...
  error?: string | null;
  children: ReactNode;
  /** 传 page/pageSize 且 total !== undefined 时渲染分页条 */
  page?: number;
  pageSize?: number;
}) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          {description ? (
            ...
            <p className="text-sm text-muted-foreground">共 {total.toLocaleString()} {totalUnit}</p>
          ) : null}
        </div>
