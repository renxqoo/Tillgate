...
describe('decodeOtlpJson', () => {
  ...
  it('status 枚举名与数值都接受;events 带时间归一', () => {
    ...
    expect(row.statusMessage).toBe('upstream failed');
    expect(row.events).toHaveLength(1);
    expect(defined(row.events[0], 'event').timeMs).toBeGreaterThan(0);
  });
  ...
  it('嵌套值类型(array/kvlist)归一化', () => {
    ...
    expect(defined(rows[0], 'rows[0]').attributes['dims']).toEqual({
      region: 'cn',
      tags: ['a', 1],
    });
  });
  ...
  it('截断门:name>256 / statusMessage>512 / service>64 / event name>128 逐字段截断', () => {
    ...
    const row = defined(rows[0], 'rows[0]');
    expect(row.name).toHaveLength(256);
    expect(row.statusMessage).toHaveLength(512);
    expect(row.service).toHaveLength(64);
    expect(defined(row.events[0], 'event').name).toHaveLength(128);
  });
  ...
  it('events 时间坏值回落 span startTime;非对象 resourceSpans 条目跳过计数', () => {
    ...
    const row = defined(rows[0], 'rows[0]');
    expect(defined(row.events[0], 'event').timeMs).toBe(row.startTime.getTime());
  });
