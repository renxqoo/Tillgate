...
/** 管理列表条件:恒 status=0 已计费 + q(外部名/真实名/requestId uuid 转文本)/窗口/维度过滤 */
function adminListConditions(input: UsageAdminListInput): SQL[] {
  const conditions: SQL[] = [eq(usageLogs.status, 0)];
  if (input.q) {
...
async function adminList(db: Db, input: UsageAdminListInput) {
  const where = and(...adminListConditions(input));
  const sorts = {
    id: usageLogs.id,
    amount: usageLogs.amount,
    inputTokens: usageLogs.inputTokens,
...
async function overviewToday(db: Db, since: Date) {
  const [row] = await db
    .select({
...
/** 概览·累计:总消费与总请求数 */
async function overviewTotals(db: Db) {
  const [row] = await db
    .select({
      cost: sql<string>`coalesce(sum(${usageLogs.amount}), 0)::numeric::text`,
      requests: sql<number>`count(*)::int`,
...
/** 按日趋势(管理台折线图数据源):日界北京时间——UTC 日界会把早 8 点前的量切进昨日;只带 from 下界 */
async function dailyTrends(db: Db, from: Date) {
 ...
 * (延迟取整是展示统计,非资金运算)。
 */
async function channelTtft(db: Db, since: Date) {
...
export function createPgUsageStore(db: Db): UsageStatsStore {
  return {
    adminList: (input) => adminList(db, input),
    overviewToday: (since) => overviewToday(db, since),
    overviewTotals: () => overviewTotals(db),
    channelStatusCounts: () =>
      db
    ...
    usageGroups: (input) => usageGroups(db, input),
    dailyTrends: (from) => dailyTrends(db, from),
    channelTtft: (since) => channelTtft(db, since),
  };
}
