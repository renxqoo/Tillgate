...
export class UsageLogRepository {
  ...
  async insertUsageLog(
    c: RepoContext,
    values: typeof usageLogs.$inferInsert,
  ): Promise<boolean> {
  ...
  async listAdminUsage(
    ...
    const [rows, countRows] = await Promise.all([
      c.db
        ...
        .limit(input.limit)
        .offset(input.offset),
      c.db.select({ count: sql<number>`count(*)::int` }).from(usageLogs).leftJoin(users, eq(usageLogs.userId, users.id)).where(where),
    ]);
  ...
  async listRequestLogs(
    ...
    const conditions = [gte(requestLogs.createdAt, input.from ?? new Date(input.now.getTime() - 30 * 86_400_000))];
    ...
    if (input.statusCode !== undefined) {
      if (typeof input.statusCode === 'number') {
        ...
        const [lo, hi] = input.statusCode === '2xx' ? [200, 299] : input.statusCode === '4xx' ? [400, 499] : [500, 599];
    ...
    const [rows, countRows] = await Promise.all([
      c.db
        ...
        .limit(input.limit)
        .offset(input.offset),
      c.db.select({ count: sql<number>`count(*)::int` }).from(requestLogs).where(where),
    ]);
    return { rows, total: countRows[0]?.count ?? 0 };
  }

  /** 概览·今日（UTC 起）：请求数/token/消费/成功率（全量行——不筛 status） */
  async statsOverviewToday(c: RepoContext, since: Date): Promise<{
    requests: number;
    inputTokens: number;
    outputTokens: number;
    cost: string;
  ...
  > {
    ...
    const groupCol = input.group === 'user' ? usageLogs.userId : input.group === 'channel' ? usageLogs.channelId : usageLogs.externalModel;
    return c.db
      .select({
        key: groupCol,
        requests: sql<number>`count(*)::int`,
