 ...
 * 状态机不变量(claim fencing/终态)由 store 的 SQL 承载(DESIGN §2.5),本层只承可测纯计算。
 */
import { createHmac } from 'node:crypto';
import type { NotificationChannel } from './channel';

/** 指数退避:min(cap, base × 2^attempts)(v1:base 15s/cap 300s;attempts 为已失败次数) */
...
/** HMAC-SHA256(secret, `${timestamp}.${body}`) → 小写 hex(DESIGN §2.4 接收方验签口径) */
 ...
 * 活跃过滤(status=0)由 store.listActive 承载;此处对 status 再设防(v1 语义:快照内不再复查,
 * 防御性恒等)。
 */
