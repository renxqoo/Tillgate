...
| B5 | `secrets.ts` | `generateApiKey(prefix='ag_')` 默认值是部署可变值（v1 注释自认「须与网关识别端同一 env 值」） | 零写死 | prefix 必填 |
...
| `generateApiKey(prefix='ag_')` | `generateApiKey(prefix)` | B5 零写死 |
| `bodyParserLimit(maxBytes=10MiB)`（gateway） | `bodyParserLimit(maxBytes)` | B2 零写死 |
| `corsPreflight(origins)` | `corsPreflight({ origins, methods?, allowHeaders?, maxAgeSeconds? })` | B4 三面漂移参数化 |
...
3. **status 解析链**：`override > HTTP_CODE_STATUS > CATEGORY_STATUS_DEFAULTS[category]`——
   http 自有码中唯一 status 修正是 `payload_too_large: 413`（payload 性质是 invalid_input，
   语义分级要求 413）；其余码严格按 category 派生（C6 结构保证 v1 分级纪律）。
