...
describe('errorHandler：TokenlensError → 对应状态码 + 统一信封', () => {
  ...
  it('face 目录 + override：出站按 face 投影', async () => {
    ...
    const a = new Hono();
    a.onError(errorHandler({ catalog: Face, overrides: { 'handler_test.session_invalid': { status: 401 } } }));
    a.get('/x', () => {
      throw Face.business('session_invalid');
    });
    const res = await a.request('/x');
    expect(res.status).toBe(401);
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe('handler_test.session_invalid');
  });
  ...
  it('Accept-Language: zh → 目录中文文案；无头 → 英文', async () => {
    ...
    expect(((await en.json()) as { error: { message: string } }).error.message).toBe('Path not found');
  });
...
describe('errorHandler：坏 JSON → 400 http.invalid_json（W2 契约）', () => {
  it('非法 JSON 体（手写 c.req.json() 路径的 SyntaxError）→ 400', async () => {
    ...
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe('http.invalid_json');
  });
});
...
describe('errorHandler：Hono HTTPException 4xx 保留状态码（不兜 500）', () => {
  it('通用 4xx → 保留状态 + http.invalid_request 信封', async () => {
    ...
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe('http.invalid_request');
  });
