 ...
 *
 * 命名空间隔离（T1）：fund_operations.operationId 是全局主键，同时承载
 * 客户键与系统自然键（signup-gift:{id} / redeem:{hash}:{uid} / ch-review:{rid} …）。
 * 系统键一律含 ':'，客户端键因此被限制为不含 ':' 的安全字符集——结构性上
 * 不可能抢占/污染系统键（若可投毒 signup-gift:{victimId} 会导致受害者登录永久 500）。
 */
...
export function operationId(c: Context): string {
  ...
  if (!CLIENT_KEY_RE.test(key)) {
    throw new HttpError(
      ...
      'idempotency-key must be 1-64 characters of letters, digits, underscores or hyphens (must not contain colons or other system namespace characters)',
    );
  }
  return key;
}
