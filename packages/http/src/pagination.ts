...
/**
 * 通用分页组件（api-contract §4：统一 ?page=&page_size=，page_size 上限 100）。
 *
 * 约定：
...
/** drizzle limit/offset 计算（offset 从 0 开始） */
...
/**
 * 一次性执行「列表 + 计数」两个查询并组装标准分页响应。
 * countQuery 形如 db.select({ count: sql<number>`count(*)::int` }).from(...)，
