...
import { ERROR_REGISTRY, localizedSpecMessage, type KnownErrorCode } from './error-codes.js';
 ...
 *
 * 双语：出口按请求 Accept-Language 协商（默认英文）。未覆盖文案的错误从
 * 注册表取对应语言；调用点覆盖的文案不参与翻译（保留调用方原文）。
 */
...
export class HttpError extends Error {
  ...
  /** 调用点显式覆盖的文案（undefined = 用注册表默认，可按语言翻译） */
...
export function localeFromContext(c: Context): string {
  return parseAcceptLanguage(c.req.header('accept-language'));
}

/** 组装统一错误响应体；locale 为协商结果时按语言取注册表文案 */
export function errorResponseBody(err: HttpError, locale?: string): {
  error: { message: string; code: string; details?: unknown };
} {
  const message = err.messageOverride
    ? err.messageOverride
    : localizedSpecMessage(err.code, locale ?? 'en', err.message);
 ...
 */
const PG_CODE_MAP: Record<string, { code: KnownErrorCode; message: string; zh: string }> = {
  '23505': { code: 'CONFLICT', message: 'Record already exists (unique constraint conflict)', zh: '记录已存在（唯一约束冲突）' },
  '23503': { code: 'INVALID_REFERENCE', message: 'Referenced resource not found', zh: '引用的资源不存在' },
  '23514': { code: 'CONSTRAINT_VIOLATION', message: 'Operation violates data constraint', zh: '操作违反数据约束' },
  '22001': { code: 'VALUE_TOO_LONG', message: 'Field value exceeds length limit', zh: '字段值超出长度限制' },
  '22P02': { code: 'INVALID_VALUE', message: 'Invalid field value format', zh: '字段值格式无效' },
  '22003': { code: 'VALUE_OUT_OF_RANGE', message: 'Field value out of range', zh: '字段值超出范围' },
};
 ...
 * 出口文案按请求 Accept-Language 协商（默认英文）。
...
export function errorHandler(logger?: ErrorLogger) {
  return (err: Error, c: Context) => {
    ...
    if (err instanceof HttpError) {
      ...
      return c.json(errorResponseBody(err, locale), err.status as 200);
    }
    ...
    if (
      ...
      return c.json(
        { error: { message: localizedSpecMessage('INVALID_JSON', locale, 'Request body is not valid JSON'), code: 'INVALID_JSON' } },
        400,
      );
    }
    ...
    if (err instanceof HTTPException && err.status >= 400 && err.status < 500) {
      ...
      return c.json(
        { error: { message: localizedSpecMessage(code, locale, ERROR_REGISTRY[code].message), code } },
        err.status as 200,
      );
    }
    ...
    if (mapped) {
      return c.json(
        { error: { message: locale === 'zh' ? mapped.zh : mapped.message, code: mapped.code } },
        ERROR_REGISTRY[mapped.code].status as 200,
      );
    }
    logger?.error({ err: err.message, stack: err.stack }, 'unhandled error');
    return c.json(
      { error: { message: localizedSpecMessage('INTERNAL_ERROR', locale, 'Internal server error'), code: 'INTERNAL_ERROR' } },
      500,
    );
  };
}
