...
const PG_CODE_MAP: Record<string, { code: KnownErrorCode; message: string }> = {
  '23505': { code: 'CONFLICT', message: '记录已存在（唯一约束冲突）' },
  '23503': { code: 'INVALID_REFERENCE', message: '引用的资源不存在' },
  '23514': { code: 'CONSTRAINT_VIOLATION', message: '操作违反数据约束' },
  '22001': { code: 'VALUE_TOO_LONG', message: '字段值超出长度限制' },
  '22P02': { code: 'INVALID_VALUE', message: '字段值格式非法' },
  '22003': { code: 'VALUE_OUT_OF_RANGE', message: '字段值超出数值范围' },
};
...
export function errorHandler(logger?: ErrorLogger) {
  return (err: Error, c: Context) => {
    ...
    if (
      ...
      return c.json({ error: { message: '请求体不是合法 JSON', code: 'INVALID_JSON' } }, 400);
    }
