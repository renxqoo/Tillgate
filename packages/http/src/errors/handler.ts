 ...
 * 优先级（v1 语义保持）：坏 JSON → Hono 4xx HTTPException → PG SQLSTATE（探测注入）→ 渲染分派兜底。
...
export function errorHandler(deps: ErrorHandlerDeps = {}) {
  return (err: Error, c: Context): Response => {
    ...
    const render = (error: unknown, statusOverride?: number): Response =>
      respond(c, renderError(error, { locale, catalog: deps.catalog, overrides: deps.overrides }), statusOverride);

    // 坏 JSON 请求体：hono validator('json') 解析失败抛 SyntaxError（手写 c.req.json() 路径同）
    if (err instanceof SyntaxError || (err instanceof HTTPException && err.status === 400 && /JSON/i.test(err.message))) {
    ...
    if (err instanceof HTTPException && err.status >= 400 && err.status < 500) {
      return render(HttpErrors.business(err.status === 413 ? 'payload_too_large' : 'invalid_request'), err.status);
    }
    // PG 约束/值错误全局面兜底（探测注入；已知冲突优先在路由层给业务语义——v1 注释语义）
    if (deps.sqlState !== undefined) {
    ...
    if (rendered.status >= 500) {
      deps.logger?.error({ code: rendered.code, err: err.message, stack: err.stack }, 'unhandled error');
    }
