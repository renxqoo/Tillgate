...
export const usageLogs = pgTable(
  'usage_logs',
  {
    ...
    /** 上游首字延迟 ms（流式观测列，不参与计费；非流式 NULL） */
    upstreamTtftMs: bigint('upstream_ttft_ms', { mode: 'number' }),
    /** 客户端首字延迟 ms（推理管道起点 → 首字节写给客户端；非流式 NULL） */
    clientTtftMs: bigint('client_ttft_ms', { mode: 'number' }),
