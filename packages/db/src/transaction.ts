 ...
 *
 * 重试参数显式化为必填注入策略,参考值 = { maxAttempts: 5, baseDelayMs: 15, maxJitterMs: 20 }。
 *
...
/** 重试策略(必填注入;装配缺省值归 app config schema) */
export interface TxRetryPolicy {
  /** 总尝试次数上限 */
  readonly maxAttempts: number;
  /** 退避基数毫秒(第 n 次重试前延迟 = base · 2^(n-1)) */
  readonly baseDelayMs: number;
  /** 抖动上限毫秒,半开区间 [0, maxJitterMs) */
  readonly maxJitterMs: number;
}

/** 观测钩子(可选):重试发生时通知;钩子异常吞掉——观测系统不可参与资金决策 */
export interface TxRetryHooks {
  onRetry?(info: { attempt: number; code: '40P01' | '40001' }): void;
}

// eslint-disable-next-line max-params -- 跨包导出 API，policy/hooks 为必填策略与可选观测，改 options 对象会波及全仓调用点
 ...
 * 锁键命名是业务语义(如 identity.user:{id}),由调用方构造。
 */
