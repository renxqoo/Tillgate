...
describe('migration journal ↔ SQL files', () => {
  ...
  it('条目总数 = 86(0000-0085,历史缺口 0036 在案;0076 = identity 七表;0081 = admins.role;0082 = 动态 RBAC 权限树/角色;0083 = drop admins.role 旧列;0084 = 接口绑定;0085 = 接口绑定独立成页;0086 = 第三方集成动态配置)', () => {
    expect(journalTags.length).toBe(86);
    expect(journalTags[0]).toBe('0000_rapid_living_mummy');
    expect(journalTags.at(-1)).toBe('0086_integration_settings');
  });
