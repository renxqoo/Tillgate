/** 账户行锁：所有资金变动的串行化点（先建户再 FOR UPDATE；(user, currency) 一币一账） */
import { and, eq } from 'drizzle-orm';
import { walletAccounts } from './schema';
import type { Tx } from './internal';

export interface LockedAccount {
  balance: string;
  inFlight: string;
  creditLimit: string;
}

/** 建户（无则插入）+ 行锁；返回锁定的账户行 */
export async function lockAccount(tx: Tx, userId: number, currency: string): Promise<LockedAccount> {
  await tx.insert(walletAccounts).values({ userId, currency }).onConflictDoNothing();
  const [row] = await tx
    .select({
      balance: walletAccounts.balance,
      inFlight: walletAccounts.inFlight,
      creditLimit: walletAccounts.creditLimit,
    })
    .from(walletAccounts)
    .where(and(eq(walletAccounts.userId, userId), eq(walletAccounts.currency, currency)))
    .for('update');
  if (!row) throw new Error('wallet account lock failed');
  return row;
}
