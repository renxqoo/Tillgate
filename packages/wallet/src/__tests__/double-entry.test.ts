...
import { wallet, nextUser, ref, sameAmount, internalAccount, assertLedgerCoherent } from './helpers';
...
describe('复式：对手科目与守恒', () => {
  it('credit 对 outside / settle 对 platform_revenue / refund 原路退回：科目余额差值正确', async () => {
    ...
    const user = nextUser();
    await wallet.credit({ userId: user, currency: 'DIX', amount: '100', refType: 'topup', refId: ref(user, 't') });
    await wallet.authorize({ userId: user, currency: 'DIX', amount: '30', refType: 'order', refId: ref(user, 'o') });
    await wallet.settle({ refType: 'order', refId: ref(user, 'o'), amount: '25' });
    await wallet.refund({ userId: user, currency: 'DIX', amount: '5', refType: 'topup_refund', refId: ref(user, 'rr') });
