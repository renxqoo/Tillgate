...
describe('原子转账 transfer', () => {
  ...
  it('电商分账：订单结算后 一笔转账拆平台佣金与商家入账', async () => {
    ...
    await wallet.credit({ userId: buyer, currency: 'TSF', amount: '100', refType: 'topup', refId: ref(buyer, 't') });
    await wallet.authorize({ userId: buyer, currency: 'TSF', amount: '90', refType: 'order', refId: ref(buyer, 'od') });
    // 买家结算 90（进入 platform_revenue），再分账：佣金留收入，80 转商家
    await wallet.settle({ refType: 'order', refId: ref(buyer, 'od'), amount: '90' });
    await wallet.transfer({
      from: { code: 'platform_revenue', currency: 'TSF' }, to: { userId: merchant, currency: 'TSF' }, amount: '80',
