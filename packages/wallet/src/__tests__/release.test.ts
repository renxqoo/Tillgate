...
describe('release / releaseExpired：释放、状态机互斥与超时扫描', () => {
  it('释放：余额不动、在途归还；重复释放为幂等 no-op；不落交易（审计在单据）', async () => {
    ...
    const scoped = await db.select().from(walletTransactions).where(inArray(walletTransactions.refId, [ref(user, 't'), ref(user, 'rel')]));
    ...
    const afterScoped = await db.select().from(walletTransactions).where(inArray(walletTransactions.refId, [ref(user, 't'), ref(user, 'rel')]));
