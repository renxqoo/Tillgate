...
describe('科目白名单（accounts）', () => {
  it('拼错的 counterparty 拒绝并列出可用科目；不再静默建抽屉', async () => {
    ...
    const error = await gw
      .credit({ userId: user, amount: '1', refType: 'topup', refId: ref(user, 'a'), counterparty: 'platfrom_revenue' })
      .catch((e: unknown) => e);
  ...
  it('声明的科目可用；内置 outside/platform_revenue 免声明恒可用', async () => {
    const user = nextUser();
    await gw.credit({ userId: user, amount: '8', refType: 'topup', refId: ref(user, 't'), counterparty: 'marketing_budget' });
  ...
  it('transfer 目标与 freeze 目标同样受白名单管', async () => {
    ...
    await expect(
      gw.transfer({ from: { userId: user }, to: { code: 'typo_cost' }, amount: '1', refType: 'payout', refId: ref(user, 'x') }),
    ).rejects.toBeInstanceOf(UnknownAccountCodeError);
    await expect(
      gw.freeze({ target: { code: 'typo_cost' }, frozen: true, refType: 'payout', refId: ref(user, 'f') }),
    ).rejects.toBeInstanceOf(UnknownAccountCodeError);
  });
});
...
describe('业务域白名单（refTypes）——防幂等域分裂双入账', () => {
  ...
  it('settle/release 的 refType 同样受管', async () => {
    await expect(
      gw.settle({ refType: 'ordr', refId: 'x', amount: '1' }),
    ).rejects.toBeInstanceOf(UnknownRefTypeError);
  });
});
...
describe('币种白名单（currencies）——防拼错币种余额隐身', () => {
  it('未声明币种拒绝（含缺省账户语义）', async () => {
    const user = nextUser();
    await expect(
      gw.credit({ userId: user, currency: 'CNH', amount: '1', refType: 'topup', refId: ref(user, 'c') }),
    ).rejects.toBeInstanceOf(UnknownCurrencyError);
    await expect(
      gw.balance(user, 'EUR'),
    ).rejects.toBeInstanceOf(UnknownCurrencyError);
    // 声明过的币种正常
    await gw.credit({ userId: user, currency: 'USD', amount: '1', refType: 'topup', refId: ref(user, 'u') });
