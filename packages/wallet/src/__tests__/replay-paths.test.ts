...
import { FrozenAccountError, RefKeyConflictError } from '../index';
...
describe('重放归属冲突（幂等键顶撞的每条重放路径都要炸）', () => {
  ...
  it('freeze 同键重放为幂等 no-op；同键换目标 → RefKeyConflictError', async () => {
    ...
    const first = await wallet.freeze({
      target: { userId: user },
      frozen: true,
...
describe('冻结与对手科目的交互（安全场景）', () => {
  it('收入科目被风控冻结：settle 拒绝且冻结单回滚保持 active，解冻后可结算', async () => {
    ...
    await wallet.freeze({
      target: { code: 'platform_revenue', currency: 'ZBC' },
      frozen: true,
      refType: 'risk_control',
      refId: ref(user, 'fr'),
    });
    ...
    const [auth] = await db
      ...
      .where(eq(walletTransactions.refId, ref(user, 'o')));
  ...
  it('转入方被冻结：transfer 拒绝；对手科目被冻结：credit 拒绝', async () => {
    ...
    await wallet.freeze({
      target: { userId: frozenUser, currency: 'ZBD' },
      frozen: true,
      refType: 'risk_control',
      refId: ref(payer, 'f1'),
    });
    await expect(
      wallet.transfer({
        from: { userId: payer, currency: 'ZBD' },
        to: { userId: frozenUser, currency: 'ZBD' },
        amount: '1',
        refType: 'p2p',
        refId: ref(payer, 'x'),
      }),
    ...
    await wallet.freeze({
      target: { code: 'outside', currency: 'ZBD' },
      frozen: true,
      refType: 'risk_control',
      refId: ref(payer, 'f2'),
    });
