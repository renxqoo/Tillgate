...
export function assertRefType(guards: WalletGuards, refType: string): void {
  if (!guards.refTypes.includes(refType)) {
    throw BillingErrors.business('invalid_ref', { reason: 'invalid_ref_type', refType });
  }
}

export function assertCurrency(guards: WalletGuards, currency: string): void {
  if (!guards.currencies.includes(currency)) {
    throw BillingErrors.business('invalid_ref', { reason: 'invalid_currency', currency });
  }
}

export function assertInternalCode(guards: WalletGuards, code: string): void {
  if (!guards.internalAccounts.includes(code)) {
    throw BillingErrors.business('invalid_ref', { reason: 'invalid_internal_code', code });
  }
}
