/**
 * rating 域契约：计量收据（UsageReceipt）与授权报价（BillingQuote）——
...
export interface UsageReceipt {
  ...
  coefficient: string;
  /**
   * 首 token 延迟观测（流式专属，非流式缺省；不参与计费）：
   * upstreamTtftMs = 上游尝试开始 → 上游首字节；clientTtftMs = runChat 进入 → 首字节
   * 写给客户端（含限流/报价/授权/路由与换渠等待）。client − upstream ≈ 管道前置开销。
   */
