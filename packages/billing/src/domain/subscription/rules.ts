 ...
 * 全部纯函数——SQL 与资金动词在 application 层；规则拒绝经目录（context.reason 携旧码）。
 */
...
export function changeDiff(newTotalPrice: string, remaining: Decimal | string): Decimal {
  const diff = new Decimal(newTotalPrice).minus(
    remaining instanceof Decimal ? remaining : new Decimal(remaining),
  );
...
export function assertChangeEligibility(input: {
  ...
  if (targetSort < curSort || input.targetQuantity < input.currentQuantity) {
    throw BillingErrors.business('subscription_rule', { reason: 'downgrade_not_allowed' });
  }
  if (targetSort === curSort && input.targetQuantity === input.currentQuantity) {
...
export function assertSeatsAllowed(input: {
  ...
  if (input.quantity > 1 && !input.allowSeats) {
    throw BillingErrors.business('subscription_rule', { reason: 'seats_not_allowed' });
  }
  if (input.allowSeats && input.isEnterprise !== true) {
    throw BillingErrors.business('subscription_rule', { reason: 'enterprise_required' });
  }
}
...
export function assertValidQuantity(quantity: number): void {
  if (!Number.isInteger(quantity) || quantity < 1) {
    throw BillingErrors.business('subscription_rule', { reason: 'invalid_quantity' });
  }
}
