/**
 * 资金来源策略契约：authorize 管线的「PAYG vs 订阅」三次分叉（闸/占/放）收敛为
 * 来源解析 + 多态。
 *
 * 错误语义（开关式，动态判定）：
 *   结构性非法（订阅过期/越权/成员超限）→ probe 抛错，瀑布中断，整个授权拒绝
 *   开关 OFF + 覆盖不足 → 抛 subscription_quota_exhausted（整单拒绝）
 *   开关 ON  + 覆盖不足 → 返回余量（≥0），缺口留给后续来源补差
 *   可选型来源不可用 → 返回 0，跳过
 * probe 返回值契约：恒 ≥ 0（负余量防御性 clamp，不得污染 Decimal.min）。
 * 来源操作全部运行在调用方（authorize/release/settle）事务内——句柄即事务句柄。
 */
...
/** 来源上下文：凭证 + 预解析结果（凭证→订阅绑定与开关由 authorize 统一查一次） */
...
export interface FundingSource {
  ...
  probe(tx: WalletTx, input: ProbeInput): Promise<Decimal>;

  /** 预占 exactAmount（调用方保证 ≤ probe 返回值；advisory 锁内同 user 无竞态，跨 user 由守卫 WHERE 兜底） */
  reserve(tx: WalletTx, input: ReserveInput): Promise<SourceReservation>;

  /** 归还预扣：缺省整笔；结算差额按 amount 部分释放（结算路径 consume < 预留额时）。 */
  release(tx: WalletTx, reservation: SourceReservation, amount?: string): Promise<void>;
