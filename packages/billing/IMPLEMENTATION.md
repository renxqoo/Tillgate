...
| B10 | 计费授权重放对终态单（released/settlement_pending）抛 409——有意防重放还是缺陷需结合 gateway 重试策略                                                       | 语义已锁死：B10 具名回归（billing-authorize.test.ts，released/settlement_pending 两态重放 → state_conflict）；apps/gateway 迁移单元（P5）对照 pipeline 重试层后回写终裁 |
...
| `apps/client-api/services/payments.service.ts` + `repository/payment-order.repo.ts`                                          | 重写（从 app 下沉到能力包；stripe/epay 经 ports）                  | U5 → `billing/application/payments/*` + `ports/payment-ports` + `domain/payment/{stripe,epay,topup}` + `adapters/{payments/providers,postgres/payment-stores}`（=MIGRATION-U5 目标位置：协议规则住 domain，适配器聚合在 adapters） |
...
| 单元 | 内容                                                                                                                                                        | 前置     | 状态                                              |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------------------- |
| U0   | 包骨架 + `domain/money`（D2 收敛）+ `domain/fingerprint`（D1/B4 收敛）+ 错误目录初始条目                                                                    | ADR-0003 | 已核销（MIGRATION-U0 §7：B4 回归×5）              |
| U1   | 钱包垂直：domain/wallet 定律 + application/wallet 动词 + adapters/postgres + 真实 PG 契约/并发/幂等测试（修 B1；锁死 B5 重放规范化）                        | U0       | 已核销（U1a/U1b；MIGRATION-U1 §7：真 PG 13 用例） |
| U2   | 计价与授权链：domain/{rating,billing} + application/billing（authorize/signal/admission/reserve-channel + funding 瀑布 + operations 幂等壳）（修 B2/B3/B6） | U1       | 已核销（U2a/U2b；MIGRATION-U2 §7 + B10 补遗）     |
| U3   | 结算与恢复：application/settlement（claim/settle/process/recover/usage-projection）+ 对账核验迁入（D9 例外）                                                | U2       | 已核销（MIGRATION-U3 §7：真 PG 17 用例）          |
| U4   | 订阅：domain/subscription + application/subscriptions                                                                                                       | U2       | 已核销（MIGRATION-U4 §7）                         |
| U5   | 支付与兑换：application/{payments,redemption} + ports/payment-ports + adapters/{payments,postgres}                                                          | U4       | 已核销（MIGRATION-U5 §7）                         |
| 收口 | facade `createBilling` 全量冻结 + `./wallet`、`./settlement` 子入口 + README                                                                                | U5       | 已核销（1db6e52；§5 状态表 2026-08-23 审查回写）  |
