...
describe('阶段 span（TracePort）：非流式 chat 全路径', () => {
  it('成功路径：prepare → authorize → routing.resolve → reserve_channel → upstream.attempt → settle_signal（顶层成序列）', async () => {
    ...
    const prepare = s.trace.byName('inference.prepare')[0]!;
    ...
    expect(s.trace.byName('billing.authorize')[0]!.attributes).toMatchObject({
      'billing.stream': false,
    });
    expect(s.trace.byName('routing.resolve')[0]!.set).toMatchObject({ 'routing.channels': 1 });
    expect(s.trace.byName('billing.reserve_channel')[0]!.attributes).toMatchObject({
      'channel.key': 'ch-a',
    });
    const attempt = s.trace.byName('upstream.attempt')[0]!;
    ...
    expect(s.trace.byName('billing.settle_signal')[0]!.attributes).toMatchObject({
      'request.id': 'req-1',
    });
  });
  ...
  it('换渠路径：预算尽 channel.skip（skip.reason）→ 耗尽 release_and_fail（error 状态）', async () => {
    ...
    expect(skips[0]!.attributes).toMatchObject({
      'channel.key': 'ch-a',
      'skip.reason': 'budget_exhausted',
      'channel.attempt': 1,
    });
    const release = s.trace.byName('billing.release_and_fail')[0]!;
  ...
  it('上游 4xx：upstream.attempt（error 状态）→ passthrough_4xx（原码属性）', async () => {
    ...
    const passthrough = s.trace.byName('billing.passthrough_4xx')[0]!;
...
describe('阶段 span（TracePort）：流式 stream 路径', () => {
  it('成功路径：settle_signal 嵌套在 upstream.attempt 之下（后台结算继承决定性事件上下文）', async () => {
    ...
    expect(s.trace.byName('billing.settle_signal')[0]!.parent).toBe('upstream.attempt');
  });
  ...
  it('结算重试耗尽：settle_signal 标 error（attempts 属性）后如实返回', async () => {
    ...
    const settle = s.trace.byName('billing.settle_signal')[0]!;
