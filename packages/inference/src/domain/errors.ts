...
/**
 * inference 错误目录（message 英文、zh 必填，动态事实进 context）。
 * HTTP status 映射归 app face 按 category/码渲染——502/503 区分由
 * no_available_channel（渠道面竭尽）与 upstream_failed（上游故障）双码承载。
 */
export const InferenceErrors = defineErrorCatalog('inference', {
  ...
  /** 凭证模型白名单拒绝（App-JWT scope）——403 */
  model_not_allowed: {
    category: 'forbidden',
    message: 'Model is not allowed for this credential',
    zh: '当前凭证无权使用该模型',
  },
  /** 渠道面竭尽：无渠道/预算耗尽/限流——503 */
