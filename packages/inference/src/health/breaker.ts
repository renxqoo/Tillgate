...
export function createCircuitBreaker(env: {
  ...
  async function load(): Promise<BreakerState> {
    return (await env.store.getState<BreakerState>(env.key)) ?? closedState(now());
  }

  /** CAS 辅助：按 mutator 计算 next（version+1）并尝试一次 */
  async function cas(
    ...
    return await env.store.compareAndSet(env.key, current.version, next, ttl());
  }

  /** TTL 略大于冷却时长，保证 open 状态不会因 TTL 提前丢失 */
  function ttl(): number {
    return env.config.cooldownMs + env.config.windowMs;
  }
  ...
  return {
    async canRequest(): Promise<boolean> {
      ...
      return await cas(state, (s) => ({ ...s, state: 'half-open' }));
    },
    ...
    async recordFailure(opts: { circuitTrip: boolean }): Promise<void> {
      ...
      for (let retry = 0; retry < CAS_MAX_RETRIES; retry++) {
        ...
        if (state.state === 'half-open') {
          if (
            await cas(state, (s) => ({
              ...
              state: 'open',
              openedAt: at,
              cooldownUntil: at + env.config.cooldownMs,
              failures: [],
            }))
          )
            return;
        ...
        // closed：滚动窗口计数
        const windowStart = at - env.config.windowMs;
        const failures = state.failures.filter((t) => t >= windowStart);
        failures.push(at);
        if (failures.length >= env.config.failureThreshold) {
          if (
            await cas(state, (s) => ({
              ...s,
              state: 'open',
