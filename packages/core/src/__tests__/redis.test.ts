...
describe('sentinel 拓扑解析（纯函数，无 Redis 依赖）', () => {
  it('host:port 逗号串 → 节点数组；缺省端口补 26379', () => {
    expect(parseSentinels('h1:26379,h2:26379,h3:26379')).toEqual([
      { host: 'h1', port: 26379 },
      { host: 'h2', port: 26379 },
      { host: 'h3', port: 26379 },
    ]);
    expect(parseSentinels('h1,h2:6380')).toEqual([
      { host: 'h1', port: 26379 },
      { host: 'h2', port: 6380 },
    ]);
    // IPv6 场景：lastIndexOf(':') 切分（[::1]:26379 形态取末段端口）
    expect(parseSentinels('[::1]:26379')).toEqual([{ host: '[::1]', port: 26379 }]);
  });

  it('空串/非法节点 fail-fast 抛错（装配期暴露）', () => {
    expect(() => parseSentinels('')).toThrow();
    expect(() => parseSentinels(',,')).toThrow();
    expect(() => parseSentinels('h1:notaport')).toThrow();
    expect(() => parseSentinels('h1:0')).toThrow();
  });
});
