/**
 * 滑动窗口限流器（v1 rate-limit-service 的下沉移植，requirements 4.6）：
 ...
 * RateLimitUnavailableError，调用方拒绝请求（503）；显式 failMode:'open'
 * 才放行（仅失去限流，资金正确性仍由 billing_requests DB 硬闸门兜底）。
 * releaseTpm/renewTpm 恒 best-effort（释放/续租失败不该反杀在途请求，TTL 兜底）。
 */
import type { Redis } from 'ioredis';
import { createRedisScriptRunner } from './script-runner.js';

/** Redis 不可达时的限流器错误（fail-closed 模式；调用方映射 503） */
export class RateLimitUnavailableError extends Error {
  constructor(cause: unknown) {
    super(`rate limit storage unavailable: ${(cause as Error)?.message ?? String(cause)}`);
...
export interface SlidingWindowLimiter {
  ...
  checkAll(dims: Array<{ dimension: string; max: number }>, requestId: string): Promise<RateLimitResult>;
...
export function createSlidingWindowLimiter(
  ...
  async function guard(op: () => Promise<RateLimitResult>, fallback: RateLimitResult): Promise<RateLimitResult> {
    try {
      return await op();
    } catch (error) {
      if (failClosed) {
      ...
      logger?.warn({ err: (error as Error).message }, 'rate limit storage unavailable, failing open (DB hard gate remains)');
      return fallback;
    }
  }

  async function checkInner(dimension: string, maxCount: number, requestId: string): Promise<RateLimitResult> {
    // 固定 hash tag，保证多维 Lua 在 Redis Cluster 中落在同一 slot
    const key = `rl:{rpm}:${dimension}`;
    const res = (await scripts.run(CHECK_SCRIPT, 1, key, Date.now(), WINDOW_MS, maxCount, requestId)) as number[];
  ...
  return {
    async check(dimension, maxCount, requestId) {
      if (maxCount <= 0) return { allowed: true };
    ...
    async renewTpm(requestId) {
      try {
        ...
        logger?.warn({ requestId, err: (error as Error).message }, 'renewTpm storage unavailable (best-effort; TTL reclaims)');
      }
    },
  };
}
