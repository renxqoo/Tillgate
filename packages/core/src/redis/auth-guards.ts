 ...
 *   - closed：抛 AuthGuardUnavailableError，调用方拒绝鉴权（503）——防护
 *     绝对优先（内部高安全部署）。
  ...
  return {
    async isLocked(keyHash) {
      try {
        ...
        return degraded(keyHash, (l) => l.isLocked(keyHash), error);
      }
    ...
    async recordFailure(keyHash) {
      try {
        ...
        return degraded(keyHash, (l) => l.recordFailure(keyHash), error);
      }
      return { locked: false, retryAfterSec: 0 };
    },
  ...
  return {
    async isLocked(ip) {
      try {
        ...
        return degraded(ip, (l) => l.isLocked!(ip), error);
      }
      return { locked: false, retryAfterSec: 0 };
    },
    ...
    async recordFailure(ip) {
      try {
        ...
        return degraded(ip, (l) => l.recordFailure(ip), error);
      }
      return { locked: false, retryAfterSec: 0 };
    },
