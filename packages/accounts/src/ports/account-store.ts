...
export type RateCardProbe = { readonly status: 'ok' } | { readonly status: 'missing' } | { readonly status: 'disabled' };
...
export interface AccountStorePort {
  // ---- 用户 ----
  insertLocalUser(db: DbLike, input: LocalUserInsert): Promise<{ status: 'created'; user: UserRecord } | { status: 'email_taken' }>;
  insertOAuthUser(db: DbLike, input: OAuthUserInsert): Promise<{ status: 'created'; user: UserRecord } | { status: 'exists' }>;
  ...
  updateUser(db: DbLike, input: { userId: number; patch: UserPatch; advanceSessionAnchor: boolean }): Promise<UserRecord | null>;
  ...
  listKeysByUser(db: DbLike, input: { userId: number } & ListQuery): Promise<PageResult<ApiKeyRecord>>;
  findOwnedKey(db: DbLike, input: { userId: number; keyId: number }): Promise<ApiKeyRecord | null>;
  patchKey(db: DbLike, input: { userId: number; keyId: number; patch: ApiKeyPatch }): Promise<ApiKeyRecord | null>;
  ...
  adminPatchKey(db: DbLike, input: { keyId: number; patch: AdminApiKeyPatch }): Promise<ApiKeyRecord | null>;
  findActiveKeyByKeyHash(db: DbLike, keyHash: string): Promise<ActiveKeyRecord | null>;
  rebindSubscription(db: DbLike, input: { fromSubscriptionId: number; toSubscriptionId: number }): Promise<{ keys: number; apps: number }>;
  ...
  rotateAppSecret(db: DbLike, input: { userId: number; appId: number; clientSecretHash: string }): Promise<AppRecord | null>;
  findActiveAppByAppId(db: DbLike, appId: string): Promise<ActiveAppRecord | null>;
  findActiveAppByClient(db: DbLike, input: { clientId: string; clientSecretHash: string }): Promise<ActiveAppRecord | null>;
  ...
  findActiveMembership(db: DbLike, input: { orgId: number; userId: number }): Promise<MembershipRecord | null>;
  ...
  insertOrReviveMember(db: DbLike, input: { orgId: number; userId: number; role: string }): Promise<void>;
  acceptInvitation(db: DbLike, input: { invitationId: number; acceptedByUserId: number }): Promise<boolean>;
  findActiveOrgSubscription(db: DbLike, orgId: number): Promise<{ id: number; quantity: number } | null>;
  /** FOR UPDATE 行锁——只可在事务内调用(席位串行化,G8) */
  lockActiveOrgSubscription(db: DbLike, orgId: number): Promise<{ id: number; quantity: number } | null>;
  patchMember(db: DbLike, input: { orgId: number; userId: number; patch: MemberLimitsPatch }): Promise<MembershipRecord | null>;
  ...
  findUsableSubscription(db: DbLike, input: { userId: number; subscriptionId: number }): Promise<{ userId: number; orgId: number | null } | null>;
  memberLimits(db: DbLike, input: { orgId: number; userId: number }): Promise<{ dailySpendLimit: string | null; monthlyQuota: string | null } | null>;

  // ---- 推荐/拉新参数 ----
  inviterActive(db: DbLike, inviterUserId: number): Promise<boolean>;
  insertReferral(db: DbLike, input: { inviterUserId: number; inviteeUserId: number }): Promise<'created' | 'already_referred'>;
  listInvitees(db: DbLike, input: { inviterUserId: number; limit: number }): Promise<readonly InviteeView[]>;
  getMarketingSettings(db: DbLike): Promise<MarketingSettingsRecord>;
  upsertMarketingSettings(db: DbLike, input: { patch: MarketingSettingsPatch; updatedBy: number | null }): Promise<MarketingSettingsRecord>;
  listReferralRelations(db: DbLike, input: { q?: string } & ListQuery): Promise<PageResult<RelationView>>;
  setReferralRelationStatus(db: DbLike, input: { relationId: number; status: number }): Promise<RelationView | null>;
}
