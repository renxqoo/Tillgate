/**
 * billing FundingSourceResolver 的 postgres 实现（resolver 桥接在 app assembly
 * 完成，本件是其 SQL 面——凭证事实归 accounts，装配方 apps/gateway 从
 * ./composition 取件，SQL 不进 app）。
 *
 * 结构等价而非类型导入：accounts 不依赖 billing——resolve 的 conn
