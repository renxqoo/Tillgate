...
| apps.repo.ts                                                            | 152  | 重构      | B1;生成器迁 domain                                               | application/_-app_.ts + domain/credentials                                           |
...
4. **合规复审补齐(2026-08-23 第二轮,AGENT.md 逐条自查)**:
   - §0.11 边界可执行:新增 `__test__/architecture.test.ts`(9 用例:domain/application/
     ...
     (application 引 decimal.js → 金额比较下沉 domain `isNonNegativeAmountWithin`;
     出口检查误报注释 → 剥离后检查)。
   - §0.5 文件粒度:postgres 适配器 950 行拆为 shared/users/keys/apps/orgs/referrals
     六个分节 + account-store.ts 组装(一聚合一文件,与老仓 8 repo 文件同构)。
   - §0.3 单一真相:营销零值缺省三处归一为 domain `ZERO_MARKETING_SETTINGS`
     (postgres/内存替身均展开该常量)。
   - §0.4:删除仅测试引用的 `isNonNegativeAmount`(并入 isNonNegativeAmountWithin)。
   - 格式门:`oxfmt --check` 全量过(首轮 70 文件欠账已全部重排);MIGRATION 补 §9.3
     要求的测试迁移矩阵;测试脚手架残留(void 语句等)清零。
   - 终态门禁:lint 0/0、typecheck 干净、单测 217(含架构 9)、覆盖率与 real 门数字
     见最终提交信息。
