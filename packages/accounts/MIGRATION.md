...
> 状态:已完成(2026-08-23;单元内行为对照清单全核销,四门 + real 门全绿)
...
4. updateDisplayName:trim 后 1..64。(v1 client-api **不写审计**——audit_logs 仅 admin 侧写入,grep 证实;本单元同口径。)
5. 管理列表:q 模糊命中 subject/email/displayName(ilike 转义);status/enterprise 过滤;排序白名单 + desc(id) 稳定序;分页钳制。
6. 管理补丁:freezeReason 只能随封禁;封禁缺省原因注入;解封清原因;email 变更同语句推进会话失效线;换卡守卫(不存在/停用两分);0 行→`user_not_found`;审计 `user.update`(全量 patch,同事务)。
...
4. invite:owner-only(`org_forbidden`);组织须有 active 订阅(`org_no_subscription`);席位满→`seats_full`;待接受上限 `min(max(剩余,1)×factor, cap)`→`invitations_full`;token 32hex 一次下发;TTL 注入(v1 等价 7 天)。
5. revoke:owner-only;CAS 0→2;0 行→`invitation_not_found`(并入 `invitation_invalid` 族,见 §2 对照)。
6. accept:错误矩阵 `invitation_invalid`(404)/`invitation_revoked`/`invitation_already_accepted`/`invitation_expired`(410 归 face);email 不一致→`invitation_email_mismatch`;事务:FOR UPDATE 锁订阅→复检席位→insertOrRevive(被移除成员复活)→CAS 翻转(0 行回滚)。
7. setMemberLimits:owner-only;金额域校验;仅 active 成员(B5 回归);0 行→`member_not_found`。
8. removeMember:owner-only;owner 自身→`org_cannot_remove_owner`;CAS 0→1。
...
4. 轮换:同事务新行(继承 name/remark/rpm/tpm/dailySpendLimit/expiresAt 原样)+旧行吊销;订阅失格→新行 subscriptionId=null。
5. 创建:限额域(十进制串、正、< 上界注入;rpm/tpm 正整数 ≤ 上界;expiresAt 未来);订阅守卫同 1.2.9。
6. 管理面:q 命中 name/preview/userEmail/userDisplayName;status 翻转仅 {0,1};非法枚举拒绝;审计 `api_key.update`(v1 码 `api_key.update_limit` 归一,同事务)。

### 1.4 Application

1. appId 32hex、clientId `app_`+16hex、secret 48hex;secret 明文仅一次;哈希 SHA-256。
2. 订阅守卫同口径;scope.models 条数/长度上界、rpm/tpm 上界。
3. disable CAS 0→1 不可逆;重复→`app_already_disabled`;越权=not_found。
4. rotateSecret:FOR UPDATE 行锁;新明文一次。
...
5. settings:单行表;金额非负、比例∈[0,1]、精度(整数 ≤10 位、小数 ≤18 位);「下一动作生效、历史不重算」;审计 `marketing.settings.update`(同事务);upsert 单往返(B7)。
6. 关系管理:列表(双方账号投影+状态,无佣金列,B3/G3);封禁/恢复 0|1;0 行→`relation_not_found`;审计 `referral.relation.update`(同事务)。
...
2. 管理动作同库即时生效(无缓存承诺)。
3. 凭证哈希零出现于任何响应形状。
4. 审计仅管理面动作(与 v1 一致):`user.update` / `api_key.update` / `marketing.settings.update` / `referral.relation.update`,同事务落 audit_logs(actor=admin);C 端动作不审计。
...
## 3. 行为对照核销清单(2026-08-23 全部核销)

- [x] 1.1.1–1.1.7 用户与资料(application-provision/profile-admin;真实 PG 门:锚推进/审计行/DB 时钟)
- [x] 1.2.1–1.2.9 组织/成员/邀请(application-orgs + race-branches;真实 PG 门:席位串行化并发单赢家、复活语义、TOCTOU 回滚)
- [x] 1.3.1–1.3.6 API Key(application-keys;真实 PG 门:同事务轮换两行、吊销即刻 miss;明文/哈希零泄漏正则排查)
- [x] 1.4.1–1.4.5 Application(application-apps;真实 PG 门:FOR UPDATE 轮换路径、client 双等值)
- [x] 1.5.1–1.5.7 推荐/拉新参数(application-referral/marketing;真实 PG 门:upsert 单往返、归因回滚)
- [x] 1.6.1–1.6.4 横切
- [x] B3(口径收敛,资金聚合整体不迁)/B5(active-only 设限回归)/B7(upsert returning)/B12(并行 count)回归用例绿;B4/B6(存储时钟统一)由 real 门 updatedAt 断言锁定
- [x] 四门绿(typecheck/lint/build/test 208 用例,覆盖率 95.45/89.34/97.03/97.63 过 90/85/90/90)+ real 门 11/11(独立测试库,`DB_TEST_URL` 约定)
...
postgres-store.real.test.ts(仓储级真实 PG 语义——补齐老仓 repository 层零账号单测的结构性缺口)。

> 备注:§9.2「每个迁移单元一份 MIGRATION.md」在本单元按单文件落地——全部子单元在新仓
> 一次性原子落成(无老路径切换期),独立切换场景不成立;后续子单元需独立演进时
> (如 identity 桥接 provision)再拆分。
