...
| #   | 事项                                                                                                                                                                                                                                                                                                                                                                                                     | 归属波次                                 |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| W1  | ~~users/admins 凭据列退役 DDL + 存量数据迁移脚本（§6）~~ **已收口（2026-08-26）**：退役 DDL = 迁移 0089（drop users/admins.password_hash，零兼容层）。范围裁决：v2 环境为唯一目标，v1 存量库（ai_gateway 系）的 users 半边存量迁移不再作为保留条件。注：admins 半边兑现记录中的 `migrate-admin-credentials.ts` 从未提交进仓（`git log --all` 无记录）——一次性脚本跑完即弃，按铁律 4 口径记为文档漂移在案 | apps 切换单元（已核销）                  |
