...
export interface MfaStore {
  ...
  /**
   * 挂起注册(advisoryLock `identity.user:{userId}` 临界区):无行 → 建挂起;
   * 挂起重挂 → 换钥重置 lastUsedStep=-1;已确认 → already_confirmed。
   */
  upsertEnrollment(
    db: DbLike,
  ...
  /**
   * 确认注册(锁内 CAS 仅挂起可置 confirmed + lastUsedStep=step)+ 恢复码整组重签
   * (旧组全作废;onConflictDoNothing 防同批哈希碰撞,B19)。
   */
  confirmEnrollment(
    db: DbLike,
  ...
  /** 删除 TOTP 注册与全部恢复码(锁内;disable 用例) */
