...
async function resolveAdminSession(
  ...
  // 会话失效线（R5-2）：管理面改密推进 'admin' 域锚点后旧 token 一律拒绝
  // R5-2：毫秒精确失效线（iatMs 亚秒声明）；旧 token 无 iatMs 时回退秒级（部署期一次性收紧）
  const issuedMs = result.iatMs ?? result.iat * 1000;
  if (!(await sessionValidAt(db, { userId: row[0]!.id, realm: 'admin', iat: issuedMs }))) {
    return null;
  }
...
export function adminAuthMiddleware(
  ...
  return async (c, next) => {
    if (!jwtSecret) {
      return c.json({ error: { message: 'Admin session secret is not configured', code: 'SERVICE_UNAVAILABLE' } }, 503);
    }
    const session = await resolveAdminSession(c, db, jwtSecret);
    if (!session) {
      return c.json({ error: { message: 'Missing admin credentials', code: 'UNAUTHORIZED' } }, 401);
    }
