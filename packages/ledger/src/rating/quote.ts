/**
 * rating/quote：报价与收据校验（纯函数，无 DB）。
 * 授权金额推导（候选链最坏费用）+ durable receipt 验收。
 * 单一真相——authorize（落账前）、signal（收据入库前）、复核补录共用。
 * 自 billing/quote.ts 上移（S2 纯迁移，行为零变更）。
 */
import { Decimal, estimateMaxCost, requiredReservation, toDecimal, ReservationError } from '@ai-gateway/wallet/metering';
import { isAttributedEstimate, type BillingQuote, type UsageReceipt } from './types.js';
import { BillingConfigurationError, PoisonReceiptError, ReceiptUserMismatchError } from '../platform/errors.js';
 ...
 * 验收失败全部是毒收据家族（ReceiptUserMismatch/PoisonReceipt）——类型化分类，
 * 结算侧 instanceof 判定即 dead，不靠 message 文本。
 */
