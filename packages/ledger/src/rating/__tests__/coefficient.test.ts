/** rating/coefficient 特征规格（迁移自 __tests__/coefficient-resolution.test.ts，行为零变更）：
 *  费率卡系数解析优先级 model > group > global > '1'；快照单查询加载 + 纯函数挑选。 */
import { randomUUID } from 'node:crypto';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';
import { createDb, type Db } from '@ai-gateway/db';
import { modelMappings, rateCardCoefficients, rateCards } from '@ai-gateway/db/schema';
import { inArray } from 'drizzle-orm';
import { loadRateCardCoefficients, pickCoefficient } from '../coefficient.js';
...
afterAll(async () => {
  // 测试数据纪律：只清自己前缀创建的行（双重条件：前缀 + 本进程 id 清单）
