...
import { BillingBacklogError } from '../errors.js';

/**
 * 结算积压准入闸（authorize 前置）：settlement_pending/processing/retry_wait
 * 堆积过深或最老账单过老时关闭新请求——结算系统过载时的自我保护。
 * 短缓存（cacheMs）+ 并发探针合并（admissionProbe）防准入查询本身打爆 DB。
 */
