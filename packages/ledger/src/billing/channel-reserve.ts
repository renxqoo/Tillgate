...
import { BillingConfigurationError, BillingInvariantError, BillingStateConflictError } from './errors.js';
 ...
 * 认领账单行。任何早退（拒绝）都发生在零变更状态——db.transaction 的 return
 * 是提交，「先释放后预留」的旧顺序在守卫预留输掉并发时会提交孤儿释放
 * （敞口少记 + 结算二次释放偷走他人敞口），已在竞态红测下修正。
 * 认领 CAS（channel_id + channel_reserved_amount 等于读到的旧值）让并发同请求
 * 双切换在结构上不可能产生孤儿敞口：输家 CAS 落空 → 整体回滚。
 */
export async function reserveChannel(
  db: Db,
  clock: () => Date,
