...
export function subscriptionAvailability(
  ...
  if (!sub) throw new SubscriptionRequiredError(input.userId);
  if (sub.ownerId !== input.userId && snapshot.membership == null) {
    throw new SubscriptionForbiddenError(input.userId, input.subscriptionId);
  }
  ...
  if (snapshot.membership != null && sub.ownerId !== input.userId) {
    if (snapshot.membership.dailySpendLimit != null && snapshot.dailySpent != null && snapshot.exposure != null) {
    ...
    if (snapshot.membership.monthlyQuota != null && snapshot.monthlySpent != null && snapshot.exposure != null) {
  ...
  if (availability.lt(need) && !input.allowPaygFallback) {
    if (dailyRemaining != null && dailyRemaining.lt(need)) {
      throw new MemberDailyLimitExceededError(input.userId);
    }
    if (monthlyRemaining != null && monthlyRemaining.lt(need)) {
      throw new MemberQuotaExceededError(input.userId);
    }
    throw new SubscriptionQuotaExhaustedError(input.userId, availability.toString(), input.amount);
  }
  return availability;
}
