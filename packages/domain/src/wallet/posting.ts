...
export type TransactionKind = 'credit' | 'settle' | 'refund' | 'transfer' | 'credit_line' | 'freeze';
...
export function validatePosting(spec: PostingSpec, lockedIds: ReadonlySet<string>): void {
  ...
  if ((audit && spec.legs.length !== 1) || (!audit && spec.legs.length < 2)) {
    throw new WalletInvariantError(`posting ${spec.kind}: leg count ${spec.legs.length}`);
  }
  ...
  for (const leg of spec.legs) {
    if (!lockedIds.has(leg.accountId)) {
      throw new WalletInvariantError(`posting: account ${leg.accountId} not locked`);
    }
    if (seen.has(leg.accountId)) {
      throw new WalletInvariantError(`posting: duplicate account ${leg.accountId}`);
    }
    seen.add(leg.accountId);
    if (currency !== undefined && currency !== leg.currency) {
      throw new WalletInvariantError(`posting: currency mismatch ${currency}/${leg.currency}`);
    }
    currency = leg.currency;
    total = total.plus(leg.amount);
  }
  if (!total.isZero()) {
    throw new WalletInvariantError(`posting ${spec.refType}/${spec.refId}: unbalanced ${total.toString()}`);
  }
  if (audit && !spec.legs[0]!.amount.isZero()) {
    throw new WalletInvariantError(`posting ${spec.kind}: audit leg must be zero`);
  }
  if (spec.kind === 'freeze' && spec.frozenAfter === undefined) {
    throw new WalletInvariantError('posting freeze: frozenAfter receipt missing');
  }
  if (spec.kind === 'credit_line' && spec.creditLimitAfter === undefined) {
    throw new WalletInvariantError('posting credit_line: creditLimitAfter receipt missing');
  }
}
