...
export interface UsageReceipt {
  ...
  coefficient: string;
  /**
   * 首token延迟观测（流式专属，非流式缺省；不参与计费）：
   * - upstreamTtftMs = 上游尝试开始（最终成功渠道 chatStream 发起）→ 上游首字节
   * - clientTtftMs = runChat 进入 → 首字节写给客户端（含限流/报价/授权/路由与换渠等待，
   *   即用户从提交到首 token 的体感；鉴权中间件在外层，对照 request_logs.duration_ms）
 ...
 * 零交付（first_chunk 前失败）不扣，走换渠/释放。估算实扣口径向精确收敛：
 * input/output 均走 BPE 估算；JSON 字节保守上界只作预扣敞口，不作实扣。
 *
 * 归属细分（同日决策）：故障截断 / 闲置超时 / 网关停机与「正常完成缺 usage」
 * 分标签——供应商质量报表、故障率统计、停机补偿三类消费方各取所需。
 */
...
/**
 * 流式 terminated → 估算归属的单一真相（gateway 收据装配与验收共用）。
