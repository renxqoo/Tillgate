/**
 * 生成任务类型词表（视频/音乐等异步任务族的单一真相）：
 *
 * 每个任务类型一条纯数据描述——
 *   execution     执行模型：task_poll（网关提交上游任务号，worker 轮询终态）/
 *                 task_execute（同步阻塞型上游，网关只登记，worker 代执行）
 *   snapshotParams 提交参数快照白名单（落 generation_tasks.params；
 *                 task_execute 族的快照即 worker 代执行的请求体）
 *
 * 计量（units）不在此重复：单一真相在 rating/measurement.ts（按映射 pricingUnit
...
export interface GenerationKindDescriptor {
  ...
  /** 提交参数快照白名单（显式字段——透传全量落库有体积与注入面风险） */
