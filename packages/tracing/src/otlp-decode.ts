 ...
 * 结构级错误（非对象/缺 resourceSpans）抛 DecodeError（400 语义）；
...
function nanoToDate(value: unknown): Date | null {
  const nano = typeof value === 'string' ? /^\d+$/.test(value) ? Number(value) : NaN : Number(value);
...
export function decodeOtlpJson(body: unknown): DecodeResult {
  if (typeof body !== 'object' || body === null) {
    throw new DecodeError('OTLP payload must be a JSON object');
  }
  const resourceSpans = (body as { resourceSpans?: unknown }).resourceSpans;
  if (!Array.isArray(resourceSpans)) {
    throw new DecodeError('OTLP payload is missing the resourceSpans array');
  }
  ...
  for (const rs of resourceSpans) {
    ...
    for (const ss of scopeSpans) {
      const spans = typeof ss === 'object' && ss !== null ? (ss as { spans?: unknown }).spans : null;
        ...
        rows.push({
          ...
          statusMessage:
            typeof span.status?.message === 'string'
              ? span.status.message.slice(0, 512)
              : null,
          ...promote(attributes),
          attributes,
          events: (span.events ?? [])
