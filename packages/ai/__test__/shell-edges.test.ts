...
const ch = (b: string, protocol = 'openai-compatible') => ({
  baseUrl: b,
  apiKey: 'sk-t',
  protocol,
});
...
describe('create-ai 壳：异常分支', () => {
  ...
  it('probe：200 即通；401 归类死凭据', async () => {
    ...
    const s2 = await startServer((_q, res) => {
      res.writeHead(401);
...
describe('usage/token-estimate 输入侧与 overflow', () => {
  it('输入侧：messages/tools/embeddings token 数组分支', () => {
    expect(
      estimateInputTokens({ model: 'm', messages: [{ role: 'user', content: '你好 abc' }] }),
    ).toBeGreaterThan(0);
    expect(
      estimateInputTokens({
        model: 'm',
...
describe('protocol/responses-chat 与 adapter 细节', () => {
  ...
  it('bedrock SigV4：签名头齐全 + 凭据解析', () => {
    ...
    const headers = signAwsRequest({
      method: 'POST',
      url: new URL('https://bedrock.test/model/x/invoke'),
      body: '{}',
      credentials: { accessKeyId: 'AKID', secretAccessKey: 'SEC' },
      at: new Date('2026-01-01T00:00:00Z'),
    });
    expect(Object.keys(headers)).toEqual(
      expect.arrayContaining(['x-amz-date', 'x-amz-content-sha256', 'authorization']),
    );
  ...
  it('vertex token 交换：fetchImpl 注入，SA 解析与缓存语义', async () => {
    ...
    const headers = await adapter
      .signRequest?.({ url: new URL('https://vertex.test/x'), body: '{}', apiKey: 'sa-json' })
      .catch(() => null);
    void headers;
  ...
  it('defineAdapter 组合：只覆写寻址其余落默认', () => {
    ...
    expect(
      custom.planRequest(ch('https://x.test', 'test-combo'), {
        endpoint: 'chat',
        model: 'm',
        requestId: 'r',
        stream: false,
