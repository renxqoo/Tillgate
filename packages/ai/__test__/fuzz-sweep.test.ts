...
import { claudeRequestToChat, chatRequestToClaude, claudeResponseToChat, claudeUpstreamToCanonicalStream, canonicalStreamToClaudeStream, claudeUsageToUsage } from '../src/protocol/claude-chat.js';
import { geminiRequestToChat, chatRequestToGemini, geminiResponseToChat, geminiUpstreamToCanonicalStream, canonicalStreamToGeminiStream, geminiUsageToUsage } from '../src/protocol/gemini-chat.js';
import { completionsRequestToChat, chatResponseToCompletions, canonicalStreamToCompletionsStream } from '../src/protocol/completions-chat.js';
...
import { extractVendorCode, extractDetail, retryAfterMsOf, statusFallbackError, tableOrFallback } from '../src/errors/fallback.js';
...
const CORPUS: unknown[] = [
  null, undefined, 0, 42, '', 'str', true, [], [1], [[]],
  {}, { a: 1 }, { model: 42 }, { messages: 'x' }, { messages: [null] },
  { messages: [{ role: 42, content: 42 }] }, { system: 42 }, { content: [{ type: 42 }] },
  { choices: 'x' }, { choices: [42] }, { usage: 'x' }, { error: 42 }, { input: 42 },
  { contents: 'x' }, { candidates: [42] }, { parts: 42 }, { base_resp: 42 },
  { tool_calls: 'x' }, { tools: [42] }, { input: [42] }, { data: 42 }, { file: 42 },
];
...
describe('fuzz 扫描：纯函数对垃圾语料全量不崩（防御守卫边界）', () => {
  it('codec 请求/响应方向 × 语料', () => {
    for (const c of CORPUS) {
      ...
      NO_THROW('tableOrFallback', () => tableOrFallback({ table: { x: 'rate_limited' }, status: 429, body: c }));
  ...
  it('adapter 能力件 × 语料', () => {
    const adapters = [
      new OpenAICompatibleAdapter(), new AnthropicAdapter(), new GeminiAdapter(),
      new VertexAiAdapter(), new MiniMaxAdapter(), new DashScopeAdapter(),
    ];
    ...
    for (const a of adapters) {
      ...
      for (const c of CORPUS) {
        ...
        NO_THROW(`${a.protocol} finalize`, () => a.finalizeRequestBody((c ?? {}) as Record<string, unknown>, pi));
        ...
        NO_THROW(`${a.protocol} translateBody`, () => (a as unknown as { translateResponseBody?: (b: unknown) => unknown }).translateResponseBody?.(c));
      }
  ...
  it('流式 codec 对垃圾 JSON 帧/非 JSON 帧不崩', async () => {
    const badFrames = ['', 'data: \n\n', 'data: not-json\n\n', 'data: [1,2]\n\n', 'data: "str"\n\n', 'data: 42\n\n', 'event: x\ndata: null\n\n'];
    for (const f of badFrames) {
      const stream = new ReadableStream<Uint8Array>({ start(c) { c.enqueue(new TextEncoder().encode(f)); c.close(); } });
      await new Response(claudeUpstreamToCanonicalStream(stream)).text();
      const s2 = new ReadableStream<Uint8Array>({ start(c) { c.enqueue(new TextEncoder().encode(f)); c.close(); } });
      await new Response(geminiUpstreamToCanonicalStream(s2)).text();
      const s3 = new ReadableStream<Uint8Array>({ start(c) { c.enqueue(new TextEncoder().encode(f)); c.close(); } });
      await new Response(canonicalStreamToClaudeStream(s3, 'm')).text();
      const s4 = new ReadableStream<Uint8Array>({ start(c) { c.enqueue(new TextEncoder().encode(f)); c.close(); } });
      await new Response(canonicalStreamToGeminiStream(s4, 'm')).text();
      const s5 = new ReadableStream<Uint8Array>({ start(c) { c.enqueue(new TextEncoder().encode(f)); c.close(); } });
      await new Response(canonicalStreamToCompletionsStream(s5)).text();
      const s6 = new ReadableStream<Uint8Array>({ start(c) { c.enqueue(new TextEncoder().encode(f)); c.close(); } });
