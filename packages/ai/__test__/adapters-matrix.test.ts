...
import { chatRequestToClaude, claudeResponseToChat, canonicalStreamToClaudeStream } from '../src/protocol/claude-chat.js';
import { chatRequestToGemini, canonicalStreamToGeminiStream } from '../src/protocol/gemini-chat.js';
import { completionsRequestToChat, canonicalStreamToCompletionsStream } from '../src/protocol/completions-chat.js';
import { normalizeUsage } from '../src/usage/normalize.js';

const ch = (baseUrl = 'https://x.test', protocol = 'openai-compatible') => ({ baseUrl, apiKey: 'k', protocol });
...
describe('adapter 寻址矩阵', () => {
  ...
  it('openai-compatible 全端点路径', () => {
    ...
    expect(oc.planRequest(ch(), { ...pi, endpoint: 'audio_transcription' }).path).toBe('/v1/audio/transcriptions');
    expect(oc.planRequest(ch(), { ...pi, endpoint: 'audio_translation' }).path).toBe('/v1/audio/translations');
  ...
  it('gemini：model 进 path + alt=sse；key 进 query 头', () => {
    ...
    expect(g.planRequest(ch('https://x.test', 'gemini'), { ...pi, stream: false }).path).toContain(':generateContent');
  });
  it('vertex：model 进 path 且 baseUrl 保持区域前缀', () => {
    const v = new VertexAiAdapter();
    const p = v.planRequest(ch('https://us-central1-aiplatform.googleapis.com', 'vertex-ai'), { ...pi, stream: false });
  ...
  it('minimax：video/music 任务族寻址', () => {
    ...
    expect(t!.planFileRetrieve(ch('https://x.test', 'minimax'), 'fid').path).toContain('file_id=fid');
    expect(m.supportedEndpoints).toContain('video');
  });
  it('dashscope：images 原生 / compatible-mode chat 双面', () => {
    const d = new DashScopeAdapter();
    expect(d.planRequest(ch('https://x.test', 'dashscope'), { ...pi, endpoint: 'images' }).path).toContain('multimodal-generation');
    expect(d.planRequest(ch('https://x.test', 'dashscope'), { ...pi, endpoint: 'chat' }).path).toContain('compatible-mode');
    expect(d.planRequest(ch('https://x.test', 'dashscope'), { ...pi, endpoint: 'embeddings' }).path).toContain('compatible-mode/v1/embeddings');
  });
});
...
describe('adapter usage/error 补充分支', () => {
  it('anthropic extractUsage：规范形优先 + claude 形兜底', () => {
    const a = new AnthropicAdapter();
    expect(a.extractUsage({ usage: { prompt_tokens: 5, completion_tokens: 2 } })).toMatchObject({ inputTokens: 5 });
    expect(a.extractUsage({ usage: { input_tokens: 5, output_tokens: 2, cache_read_input_tokens: 1 } })).toMatchObject({ inputTokens: 6, cachedInputTokens: 1 });
    expect(a.extractUsage({})).toBeNull();
    expect(a.extractUsage(null)).toBeNull();
  });
  it('gemini extractUsage 双形 + minimax 任务状态解析', () => {
    const g = new GeminiAdapter();
    expect(g.extractUsage({ usageMetadata: { promptTokenCount: 3, candidatesTokenCount: 1 } })).toMatchObject({ inputTokens: 3 });
    expect(g.extractUsage({ usage: { prompt_tokens: 9, completion_tokens: 1 } })).toMatchObject({ inputTokens: 9 });
    const m = new MiniMaxAdapter();
    const st = m.tasks!.parseTaskStatus({ status: 'Success', file_id: 'f1' });
    expect(st).toMatchObject({ ok: true, status: 'succeeded', fileId: 'f1' });
    expect(m.tasks!.parseTaskStatus({ status: 'Fail' })).toMatchObject({ ok: true, status: 'failed' });
    expect(m.tasks!.parseTaskStatus({ status: 'Unknown' })).toMatchObject({ ok: true, status: 'running' });
    expect(m.tasks!.parseFileRetrieve({ file: { download_url: 'https://cdn/x' } })).toMatchObject({ ok: true, downloadUrl: 'https://cdn/x' });
  });
});
...
describe('codec 反向流（客户端面）', () => {
  it('canonical → claude SSE：合成 message_start/delta/stop + usage 还原', async () => {
    ...
    const out = await new Response(canonicalStreamToClaudeStream(new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode(frames)); c.close(); } }), 'm')).text();
  ...
  it('canonical → gemini SSE：finish/usage 映射', async () => {
    ...
    const out = await new Response(canonicalStreamToGeminiStream(new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode(frames)); c.close(); } }), 'm')).text();
    expect(out).toContain('"finishReason":"STOP"');
    expect(out.toLowerCase()).toContain('usagemetadata');
  });
  it('completions 入站请求 → 规范形', () => {
    const chat = completionsRequestToChat({ model: 'm', prompt: 'q', stream: true, temperature: 0.5 });
    expect((chat.messages as Array<Record<string, unknown>>)[0]).toMatchObject({ role: 'user', content: 'q' });
    expect(chat.stream).toBe(true);
  });
  it('completions 入站流 → 规范形流出站', async () => {
    const frames = 'data: {"choices":[{"delta":{"content":"ok"}}]}\n\ndata: [DONE]\n\n';
    const out = await new Response(canonicalStreamToCompletionsStream(new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode(frames)); c.close(); } }))).text();
    expect(out).toContain('"text":"ok"');
    expect(out).toContain('[DONE]');
  });
  it('claude 非流式响应 + usage（含 cache）', () => {
    const r = claudeResponseToChat({ id: 'm1', model: 'c', content: [{ type: 'text', text: 'hi' }], stop_reason: 'end_turn', usage: { input_tokens: 5, output_tokens: 2, cache_read_input_tokens: 3 } }) as Record<string, unknown>;
    expect((r.choices as Array<Record<string, unknown>>)[0]?.message).toMatchObject({ content: 'hi' });
