...
import { geminiRequestToChat, chatRequestToGemini, geminiResponseToChat, geminiUsageToUsage } from '../src/protocol/gemini-chat.js';
...
describe('gemini 入站请求（geminiRequestToChat 深支）', () => {
  ...
  it('generationConfig 映射 + generationConfig 双向', () => {
    const g = chatRequestToGemini({ model: 'm', messages: [{ role: 'user', content: 'q' }], temperature: 0.7, max_tokens: 100, top_p: 0.9 });
  ...
  it('geminiResponseToChat：多 part 文本拼接 + finishReason 映射 + 垃圾容错', () => {
    const r = geminiResponseToChat({ candidates: [{ content: { parts: [{ text: 'a' }, { text: 'b' }] }, finishReason: 'MAX_TOKENS' }] }, 'm');
    expect(JSON.stringify(r)).toContain('ab');
    expect(JSON.stringify(r)).toContain('length');
    expect(geminiUsageToUsage({ promptTokenCount: 1, candidatesTokenCount: 1, cachedContentTokenCount: 1 })).toMatchObject({ promptTokens: 1 });
  });
});

describe('responses codec 深支', () => {
  it('input 数组多角色 + developer→system + 内容块提取', () => {
    const chat = responsesRequestToChat({ model: 'm', input: [
  ...
  it('chatResponseToResponses：id/model/usage 透传', () => {
    const r = chatResponseToResponses({ id: 'resp1', model: 'm', choices: [{ message: { content: 'x' } }], usage: { prompt_tokens: 1, completion_tokens: 1, total_tokens: 2 } });
  ...
  it('canonicalStreamToResponses：错误帧 → response.failed 事件', async () => {
    const out = await new Response(canonicalStreamToResponsesStream(sse([
...
describe('adapter 剩余分支', () => {
  it('anthropic：finalize 流式注入 stream + probe 路径 + usage 缓存写', () => {
    ...
    const fin = a.finalizeRequestBody({ model: 'm', messages: [] }, { endpoint: 'chat', model: 'm', stream: true });
    expect(fin.stream).toBe(true);
    expect(a.probeRequests(ch('anthropic')).length).toBeGreaterThan(0);
    const u = a.extractUsage({ usage: { input_tokens: 5, output_tokens: 2, cache_creation_input_tokens: 1 } });
  ...
  it('gemini adapter：finalize stream 字段删除 + probe x-goog api-key', () => {
    ...
    const fin = g.finalizeRequestBody({ model: 'm', contents: [], generationConfig: {} }, { endpoint: 'chat', model: 'm', stream: true });
  ...
  it('minimax：video finalize duration/resolution 规则 + music 路径 + embeddings 寻址', () => {
    ...
    const v = m.finalizeRequestBody({ model: 'm', prompt: 'p', duration: 99, size: 'ttt' } as never, { endpoint: 'video', model: 'm', stream: false });
    expect(v.duration).toBe(15); // clamp 4-15
    expect(JSON.stringify(v)).toContain('720P'); // 未知分辨率兜底
    expect(m.planRequest(ch('minimax'), { endpoint: 'embeddings', model: 'm', requestId: 'r', stream: false }).path).toContain('/v1/embeddings');
    expect(m.planRequest(ch('minimax'), { endpoint: 'music', model: 'm', requestId: 'r', stream: false }).path).toContain('music_generation');
  });
  it('overflow 模式矩阵（厂商出处注释的边界）', () => {
    expect(detectSilentOverflow(999_999_999, 'anthropic', 'claude-3-5-sonnet-20241022')).toBe(true);
    expect(detectSilentOverflow(1, 'openai', 'gpt-4o')).toBe(false);
    expect(detectSilentOverflow(100_000, 'openai', 'gpt-4o')).toBe(false); // 窗内（128k）
  });
});

describe('internal/stream：peekFirstChunk 深支', () => {
  it('空流 → done；带首帧 → first+rest 完整；超时 → 抛', async () => {
    const empty = await peekFirstChunk(new ReadableStream({ start(c) { c.close(); } }), { timeoutMs: 100 });
    expect(empty.done).toBe(true);
    const full = await peekFirstChunk(new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode('data: {"x":1}\n\n')); c.close(); } }), { timeoutMs: 100 });
    ...
    await expect(peekFirstChunk(new ReadableStream({ start() { /* 永不发 */ } }), { timeoutMs: 50 })).rejects.toThrow();
  });
  it('firstChunkStreamError：错误帧识别与非错误帧 null', () => {
    expect(firstChunkStreamError(new TextEncoder().encode('data: {"error":{"code":"x"}}\n\n'))).toBeDefined();
