...
describe('dashscope 全分支', () => {
  ...
  it('normalizeRequest no-op 透传 + finalize images parameters 收敛', () => {
    const { body } = d.normalizeRequest(
      { model: 'm', prompt: 'p', size: '1024x1024' },
      {},
      'images',
    );
    expect(body).toMatchObject({ prompt: 'p' });
    const fin = d.finalizeRequestBody(body as Record<string, unknown>, {
      endpoint: 'images',
      model: 'm',
      stream: false,
    });
  ...
  it('extractUsage：images 张数计量（usage.image_count → units）', () => {
    ...
    expect(d.extractUsage({ usage: { prompt_tokens: 1, completion_tokens: 1 } })).toMatchObject({
      inputTokens: 1,
    }); // 父类兜底
  });
});
...
describe('bedrock 全分支', () => {
  ...
  it('finalize：claude 形 + anthropic_version 注入', () => {
    const fin = b.finalizeRequestBody(
      { model: 'm', messages: [{ role: 'user', content: 'q' }] },
      { endpoint: 'chat', model: 'm', stream: false },
    );
    expect(fin.anthropic_version).toBe('bedrock-2023-05-31');
    expect(
      (fin.messages as Array<{ content: Array<{ type: string; text?: string }> }>)[0]?.content[0],
    ).toMatchObject({ type: 'text', text: 'q' });
  });
  it('寻址：invoke/stream 双路径 + 探测 /models', () => {
    ...
    expect(b.planRequest(ch('aws-bedrock'), { ...pi, stream: true }).path).toContain(
      'invoke-with-response-stream',
    );
    expect(b.probeRequests(ch('aws-bedrock'))[0]?.path).toBe('/models');
  });
  it('extractUsage：claude 形', () => {
    expect(b.extractUsage({ usage: { input_tokens: 3, output_tokens: 1 } })).toMatchObject({
      inputTokens: 3,
    });
...
describe('task-kit 通用骨架', () => {
  const ops = createRestTaskOps({
    ...
    readStatus: (b) =>
      b.status === 'Done'
        ? { status: 'succeeded' as const, fileId: 'x' }
  ...
  it('提交/查询/取回寻址 + 状态解析', () => {
    ...
    expect(ops.parseTaskStatus({ status: 'Done' })).toMatchObject({
      ok: true,
      status: 'succeeded',
    });
...
describe('gemini 请求出站 + calibration/config/drain 细节', () => {
  it('chatRequestToGemini：contents 映射 + system 指令 + 多模态 part', () => {
    ...
    const parts =
      (g.contents as Array<{ role: string; parts: Array<Record<string, unknown>> }>)[0]?.parts ??
      [];
    expect(parts.some((p) => typeof p.text === 'string')).toBe(true);
    expect(
      parts.some((p) => (p.inlineData as Record<string, unknown>)?.mimeType === 'image/png'),
    ).toBe(true);
  });
  it('geminiUsageToUsage：cached 扣出 + thoughts 计入 + 垃圾不崩', () => {
    ...
    ).toMatchObject({ promptTokens: 10, cachedTokens: 3, completionTokens: 2 });
  ...
  it('calibration：三层合并（defaults → provider:model 命中）', () => {
    ...
    expect(resolveCalibration('unknown', 'unknown').weights).toEqual(
      DEFAULT_TOKEN_ESTIMATE_CALIBRATION.defaults,
    );
  });
    ...
    const p = v.planRequest(
      {
        ...
        apiKey: JSON.stringify({ client_email: 'a@b', private_key: 'k' }),
        protocol: 'vertex-ai',
      },
