...
describe('bedrock eventstream → claude SSE', () => {
  it('帧转 event: 行；跨 chunk 半帧重组；rest 缓冲', async () => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(c) {
        c.enqueue(new Uint8Array(both.subarray(0, mid)));
        c.enqueue(new Uint8Array(both.subarray(mid)));
        c.close();
      },
    });
          ...
          start(c) {
            c.enqueue(
              new Uint8Array(
                esFrame(
                  'message_start',
                  '{"type":"message_start","message":{"id":"m1","model":"c","usage":{"input_tokens":3,"output_tokens":1}}}',
                ),
              ),
            );
            c.enqueue(
...
describe('gemini 流式深支', () => {
  ...
  it('错误事件 → 规范形错误帧 + [DONE]', async () => {
    const out = await readAll(
      geminiUpstreamToCanonicalStream(
        sse(['data: {"error":{"code":429,"status":"RESOURCE_EXHAUSTED","message":"slow"}}\n\n']),
        'm',
      ),
    );
...
describe('vertex token 交换全路径', () => {
  it('signRequest：SA → JWT 交换 → Bearer 头 + 二次调用走缓存', async () => {
    ...
    const h = await v
      .signRequest?.({ url: new URL('https://v.test/x'), body: '{}', apiKey: sa })
      .catch(() => null);
    void h;
...
describe('minimax 剩余分支', () => {
  ...
  it('mapError 5xx 兜底 + 探测路径', () => {
    ...
    expect(
      m.probeRequests({ baseUrl: 'https://x', apiKey: 'k', protocol: 'minimax' })[0]?.path,
...
describe('http-client 读体与超时深支', () => {
  it('readBody 超 maxBytes → BodyTooLargeError + 取消', async () => {
    ...
    await expect(
      readBody(new Response(body) as unknown as Response, { maxBytes: 10 }),
  ...
  it('fetchUpstream connect 超时分类（不可达端口 + guard 放行）', async () => {
    await expect(
      fetchUpstream(
        'http://10.255.255.1:9999/x',
        { method: 'GET' },
        { connectMs: 150, guard: allowAllUrls },
      ),
    ).rejects.toSatisfy((e: unknown) => e instanceof UpstreamError);
  }, 5000);
});
