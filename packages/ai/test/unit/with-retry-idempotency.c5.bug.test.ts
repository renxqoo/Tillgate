...
/**
 * 上游 POST 重试幂等性（C5）：重试重发的 POST 必须可被供应商去重。
 *
 ...
 *
 * 约束：供应商在首请求已处理并计费、仅响应丢失/超时的情况下，无去重凭证的
 * 重试会触发第二次生成 → 供应商成本翻倍。因此每次 POST 必须带
 * Idempotency-Key: <requestId> 头（供应商侧按 requestId 去重）。
 *
 * 本测试用计数 mock fn 验证：单个 retryable 错误导致 fn（即上游 POST）被调用 ≥2 次。
 */
...
describe('C5 — POST 重试非幂等（TDD 复现，当前应 FAIL）', () => {
  it('单次 retryable 错误 → 上游 POST 被调用 ≥2 次（无 Idempotency-Key 去重 → 重复生成风险）', async () => {
    ...
    // 重试确实发生了：上游 POST 被调用 2 次（withRetry 的设计行为）。
    // 去重不在重试原语内：若重发的 POST 不带 Idempotency-Key，供应商无法去重
    // → 第 1 次（已计费）+ 第 2 次（再计费）= 双倍成本。
    expect(postCalls).toBe(2); // 重试发生（证明重复 POST）

    // 关键契约：调用方应在每次 POST 带 Idempotency-Key。
