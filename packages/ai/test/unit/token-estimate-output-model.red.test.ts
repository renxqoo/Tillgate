/**
 * 契约：estimateOutputTokens 必须把 model 传给估算器——与输入侧同口径。
 *
 * token-estimate.ts 的估算单一真相是 countText = BPE 真分词器主路径 + 启发式兜底。
 * estimateInputTokens 对 messages.content 传了 model（走 BPE）；estimateOutputTokens
 * 对 choices[].message.content 调 estimateContent 时同样必须传 model——若漏传，
 * 输出侧恒走启发式兜底。
 *
 * 资金影响：非流式响应缺 usage 时（estimatedFor='usage_missing_nonstream'，估算结算），
 * 输出 token 与输入 token 会采用两套口径——同一文本计价基础不一致（200 字 CJK
 * 实测：输入侧 BPE=100、输出侧启发式=140，偏差 +40%）；偏差方向随文本构成漂移
 * （代码/生僻文本反向低估），估算扣费金额系统性偏离。
 *
 * 断言的是「同一文本同一估算器」的契约，不是具体数值快照。
 */
