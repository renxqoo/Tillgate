...
/**
 * 回归（new-api #6649 同类）：客户端在收到 [DONE] 之后立刻断开（HTTP/1.1 流式
 * 客户端的标准行为）时，必须归类为「正常完成」而非 client_disconnect。
 *
 * relay-stream 的 cancel() 回调按 scanner 状态判定：已收到 [DONE]/终止帧的流
 * 按正常完成归类，否则发 terminated='client_disconnect'。若误分类，下游影响：
 * usage_logs.stream_aborted=true、span 记「用户取消」；无 usage 的流还会走
 * recordEstimatedCancel（估算结算）。
 */
