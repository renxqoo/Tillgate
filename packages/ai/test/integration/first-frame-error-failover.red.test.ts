...
/**
 * 回归（new-api #6643 同类）：上游以 HTTP 200 + SSE 首帧即错误（限流/配额类
 * 错误放在流体内而非状态码）时，ai 层必须发出可供网关换渠道的 failed 终态事件。
 *
 * 机制：peekFirstChunk 除空流检测外还做首帧错误识别（chat-stream.ts），命中即
 * 按 retryable 语义退避重试，耗尽后 failEarly 发 failed 终态事件。网关
 * attempt-runner 仅凭 failed 事件换渠道——若 failed 不发出，该场景下客户端
 * 只能收到上游错误原文、无渠道切换、按部分/零 usage 结算。
 */
