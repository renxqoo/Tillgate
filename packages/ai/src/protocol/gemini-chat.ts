...
export function geminiUpstreamToCanonicalStream(upstream: ReadableStream<Uint8Array>, model: string): ReadableStream<Uint8Array> {
  let started = false;
    ...
    (ev: SseEvent, emit) => {
      ...
      if (finish !== undefined) {
        emit(openaiFrame({
          ...
          ...(usage ? { usage: { prompt_tokens: usage.promptTokens, completion_tokens: usage.completionTokens, total_tokens: usage.promptTokens + usage.completionTokens } } : {}),
        }));
        emit(openaiDone());
      } else if (usage) {
        // 中间帧也可能带 usage（思考 token 计数更新）——最后帧胜出语义由 scanner 保证
        emit(openaiFrame({ id: 'chatcmpl-gemini', object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model, choices: [{ index: 0, delta: {}, finish_reason: null }], usage: { prompt_tokens: usage.promptTokens, completion_tokens: usage.completionTokens, total_tokens: usage.promptTokens + usage.completionTokens } }));
      }
    },
    (emit) => {
