...
export type ChatAttemptResult =
  | {
      ok: true;
      value: { usage?: Usage; body?: unknown; rawBody?: Uint8Array; rawContentType?: string };
    }
    ...
    try {
      ...
      const res = await fetchUpstream(
        url,
        {
          ...
          body: isFormData
            ? (finalBody as unknown as NonNullable<Parameters<typeof fetchUpstream>[1]>['body'])
            : serializedBody,
        },
        {
      ...
      if (!contentType.includes('json')) {
        const rawBytes = await readRawBody(res, {
          signal: totalSignal,
        ...
        return {
          ok: true,
          value: { usage: undefined, rawBody: rawBytes, rawContentType: contentType },
        };
      }
      const raw = await readBody(res, { signal: totalSignal, maxBytes: 8 * 1024 * 1024 });
