 ...
 * 编排层（pipeline/）只做通用编排，不出现任何协议字面量。
 *
 * 两种扩展形态（registry/define-adapter.ts）：
 *   - 全量实现本接口（新原生协议：codec + 全部能力件）
 *   - defineAdapter({ addressing, body, usage, errors }) 部分覆写
 *     （OpenAI 兼容厂商差异：只改寻址/个别件，其余落默认——组合取代继承）
...
export interface BodyFinalizer {
  /** 透传为基底，按规则抹平（vendor profile 编译产物与 per-model 规则在此汇合） */
  normalizeRequest(
    req: unknown,
    rules: ParamRules,
...
/** 能力件⑤：原生线格式 ⇄ 规范形编解码（仅原生协议需要；openai-compatible 本身即规范形） */
...
/** 聚合契约 = 五能力件并集（类型别名形态，非兼容层——扩展方二选一见文件头） */
