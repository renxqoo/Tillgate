import { classifyHttpError, createUpstreamError } from '../errors/classify';
...
/** base_resp 信封错误（HTTP 200 也可能是错误）：MiniMax 码 → 归一错误 */
function minimaxEnvelopeError(body: unknown): UpstreamError | null {
  ...
  // 1004/2049 认证失败（换 Key 维度）；1008 余额；1002/1026/1027 内容审查/参数；
  // 2013 计划/参数不适用；429 限流；5xx 服务端。归一到既有错误码词表。
  ...
  if (code === 1002 || code === 1026 || code === 1027 || code === 2013) {
    return createUpstreamError({
