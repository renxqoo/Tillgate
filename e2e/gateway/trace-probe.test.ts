...
async function waitTrace(requestId: string, deadlineMs = 10_000): Promise<ViewableTrace> {
  ...
  for (;;) {
    const hit = memory.recent(50).find(
      (t) => t.spans.some((s) => s.attributes['request.id'] === requestId),
    );
...
function dumpTrace(trace: ViewableTrace, label: string): void {
  ...
  for (const s of rows) {
    ...
    for (const e of s.events) console.log(`       event ${e.name} @+${(e.timeMs - root.startTimeMs).toFixed(1)}ms ${e.attributes ? fmtAttrs(e.attributes) : ''}`);
  }
  // 树形视图
  ...
  const render = (span: (typeof trace.spans)[number], depth: number): void => {
    ...
    for (const c of (children.get(span.spanId) ?? []).toSorted((a, b) => a.startTimeMs - b.startTimeMs)) render(c, depth + 1);
  };
  console.log('  span tree:');
  for (const r of (children.get(undefined) ?? []).toSorted((a, b) => a.startTimeMs - b.startTimeMs)) render(r, 0);
}
...
/** 结构核对：返回违规清单（空 = 全部通过） */
function checkTrace(trace: ViewableTrace, requestId: string, httpStatus: number, stream: boolean): string[] {
  const bad: string[] = [];
  ...
  if (root.attributes['request.id'] !== requestId) bad.push(`root request.id=${root.attributes['request.id']} != 响应头 ${requestId}`);
  if (root.attributes['http.status_code'] !== httpStatus) bad.push(`root http.status_code=${root.attributes['http.status_code']} != 实际 ${httpStatus}`);
  ...
  if (traceIds.size !== 1) bad.push(`span 跨 ${traceIds.size} 个 traceId：${[...traceIds].join(',')}`);
  for (const s of trace.spans) {
  ...
  for (const s of trace.spans) {
    ...
    if (s.startTimeMs < root.startTimeMs - 0.5 || s.endTimeMs > root.endTimeMs + 0.5) {
      bad.push(`${s.name} 时窗逃逸根 span：+${(s.startTimeMs - root.startTimeMs).toFixed(1)}→+${(s.endTimeMs - root.startTimeMs).toFixed(1)}ms（根 +0→+${(root.endTimeMs - root.startTimeMs).toFixed(1)}ms）`);
    }
  }
  // 阶段先后序（按开始时间）
  const order = EXPECTED_STAGES.map((n) => [n, trace.spans.filter((s) => s.name === n).at(-1)] as const);
  for (let i = 1; i < order.length; i++) {
    const [prevName, prev] = order[i - 1]!;
    const [curName, cur] = order[i]!;
    if (prev && cur && prev.startTimeMs > cur.startTimeMs) bad.push(`时序倒置：${prevName} 开始晚于 ${curName}`);
  }
  const attempt = trace.spans.find((s) => s.name === 'upstream.attempt');
  if (attempt) {
    if (attempt.attributes['upstream.stream'] !== stream) bad.push(`upstream.attempt upstream.stream=${attempt.attributes['upstream.stream']} 应为 ${stream}`);
    if (attempt.attributes['upstream.ok'] !== true) bad.push(`upstream.attempt upstream.ok != true`);
    if (stream && typeof attempt.attributes['upstream.ttft_ms'] !== 'number') bad.push('流式 upstream.attempt 缺 upstream.ttft_ms');
    if (!stream && typeof attempt.attributes['upstream.duration_ms'] !== 'number') bad.push('非流式 upstream.attempt 缺 upstream.duration_ms');
  }
...
describe('诊断探针：全真装配完整请求的 span 树', () => {
  it('非流式 chat：完整请求 → 倒出 span 树并核对', async () => {
    ...
    console.log(`HTTP ${res.status} x-request-id=${res.headers.get('x-request-id')} choices=${JSON.stringify(body.choices ?? body).slice(0, 80)}`);
    ...
    console.log(bad.length === 0 ? '结构核对：全部通过' : `结构核对：${bad.length} 项违规\n  - ${bad.join('\n  - ')}`);
    expect(bad).toEqual([]);
  });
