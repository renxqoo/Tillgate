...
async function waitTrace(requestId: string, terminal: string[], deadlineMs = 15_000): Promise<ViewableTrace> {
  const deadline = Date.now() + deadlineMs;
  for (;;) {
    const hit = memory.recent(50).find((t) => t.spans.some((s) => s.attributes['request.id'] === requestId));
...
describe('失败路径 span 树（全真装配 + mock 上游）', () => {
  ...
  it('② 非流式 + 上游 400：billing.passthrough_4xx，客户端 4xx 透传', async () => {
    await retargetUpstream(world, { baseUrl: upstreamUrl, apiKeyPlain: 'sk-e2e-minimax-0123456789abcdef', protocol: 'openai-compatible' });
  ...
  it.fails('③ 流式 + 用户取消：mid-stream abort → 仍有终态结算 span，树完整落缓冲', async () => {
    ...
    const groups = memory.recent(50).filter((t) =>
      t.spans.some((s) => s.attributes['request.id'] === requestId),
    );
