...
import { call, defined, jsonHeaders } from '../admin/kit.js';
...
/** 用户面 API 客户端收窄（world 就绪时 api 必已装配——与 w() 同口径的运行时守卫） */
function clientApi(): ReturnType<typeof apiClient> {
  if (api === null) throw new Error('cross-app api client not ready');
  return api;
}
...
async function loginTwoStep(
  ...
  const first = await clientApi()('/v1/auth/login', {
    method: 'POST',
  ...
  const verify = await clientApi()('/v1/auth/login/verify', {
    method: 'POST',
    body: JSON.stringify({
...
describe.skipIf(!hasInfra)('跨 app：管理动作 → 用户面即时生效（v1 e2e-cross-app 语义）', () => {
  it('管理员重置密码 → client 旧 token 401 + 旧密码登 401 + 新密码可登', async () => {
    ...
    const reg = await registerUser(w().client, email, 'cross-app-pass-1');
    w().users.push({ id: reg.userId, email });
    expect((await clientApi()('/v1/me', { token: reg.token })).status).toBe(200);
    ...
    expect((await clientApi()('/v1/me', { token: reg.token })).status).toBe(401);
    expect(
      (
        await clientApi()('/v1/auth/login', {
          method: 'POST',
    ...
    expect(
      (await clientApi()('/v1/me', { token: defined(reLogin.token, 'reLogin.token') })).status,
    ).toBe(200);
  }, 60_000);
