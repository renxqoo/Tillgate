...
function parseDirectives(raw: string): { base: string; d: Record<string, number | true | number[]> } {
...
function usageOf(d: Record<string, number | true | number[]>): Record<string, number> {
  ...
  if ('hugeusage' in d) {
    return { prompt_tokens: 1_000_000, completion_tokens: 1_000_000_000, total_tokens: 1_001_000_000 };
  }
...
async function faultGate(
  ...
  if (mode === 's429' && !('s429' in d)) {
    return Response.json({ error: { message: 'ctl rate limited', type: 'rate_limit_error', code: '429' } }, { status: 429 });
  }
  if ('hangbody' in d) {
    return new Response(
      new ReadableStream({
  ...
  if ('empty' in d) return new Response('', { status: 200, headers: { 'content-type': 'application/json' } });
  if ('s429' in d) return Response.json({ error: { message: 'mock rate limited', type: 'rate_limit_error', code: '429' } }, { status: 429 });
  if ('s500' in d) return Response.json({ error: { message: 'mock internal error', type: 'server_error' } }, { status: 500 });
  if ('s403' in d) return Response.json({ error: { message: 'mock forbidden', type: 'permission_error' } }, { status: 403 });
  if ('s400' in d) return Response.json({ error: { message: 'mock bad request', type: 'invalid_request_error' } }, { status: 400 });
  if ('redir' in d) {
...
async function handler(req: Request): Promise<Response> {
    ...
    if (url.pathname === '/__ctl' && req.method === 'POST') {
      const body = (await req.json().catch(() => ({}))) as { scope?: string; mode?: Mode; reset?: boolean };
      if (body.reset) {
        ctl.global = 'ok';
        ctl.vendors = {};
        metrics.requests = {};
    ...
    const m = url.pathname.match(/^\/([a-z]+)\/v1\/(chat\/completions|completions|embeddings|models)$/);
    if (m == null || VENDORS[m[1]] == null) {
      return Response.json({ error: { message: `unknown mock path ${url.pathname}` } }, { status: 404 });
    }
    const vendor = m[1];
    ...
    if (m[2] === 'models') {
      return Response.json({ object: 'list', data: persona.catalog.map((id) => ({ id, object: 'model', owned_by: vendor })) });
    }
    if (m[2] === 'embeddings') {
      return Response.json({
        object: 'list',
    ...
    if (body == null || typeof body.model !== 'string' || body.model.length === 0) {
      return Response.json({ error: { message: 'mock: missing model', type: 'invalid_request_error' } }, { status: 400 });
    }
      ...
      async start(controller) {
        ...
        try {
          ...
          for (let i = 0; i < chunks; i++) {
            const content = bigKb > 0 ? 'x'.repeat(bigKb * 1024) : `chunk-${i}-of-${modelSlice(body.model)}`;
...
function scaled(usage: Record<string, number>, done: number, total: number) {
  ...
  return { prompt_tokens: usage.prompt_tokens, completion_tokens: f, total_tokens: usage.prompt_tokens + f };
}
