/**
 * 注册/登录/挑战/会话攻击面(client-api):畸形输入、重复注册、验证码
...
const uniq = (tag: string) =>
  `rt-${tag}-${Date.now().toString(36)}${Math.floor(Math.random() * 1e4).toString(36)}`;
...
function resetRegisterGate() {
  ...
  Bun.spawnSync([
    'bash',
    '-c',
    `redis-cli -a ${pass} --scan --pattern 'client-api:*' 2>/dev/null | xargs -r redis-cli -a ${pass} del >/dev/null 2>&1`,
  ]);
}
...
define('C1', '注册登录攻击', '畸形 email(无@/超长/unicode/嵌套点)→ 400 干净拒绝', async (c) => {
  for (const email of [
    ...
    ok(
      ...
      `email "${email.slice(0, 24)}" → 400, got ${r.status}`,
    );
  }
});
...
define('C2', '注册登录攻击', '重复注册:精确重复 + 大小写变体都算占用', async (c) => {
  ...
  ok(
    [409, 400].includes(dup2.status),
    `case variant rejected, got ${dup2.status}(若 201 = 同邮箱双账号,严重发现)`,
  );
});
...
define(
  'C4',
  '注册登录攻击',
  'SQLi/XSS payload 打 email/password/aff → 400,无 5xx 无回显',
  async (c) => {
    const payloads = [
      "'; drop table users; --",
      "' or '1'='1",
...
define('C5', '注册登录攻击', '验证码穷举:错 5 次耗尽后正确码也被拒', async (c) => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const r = await http(`${c.url.clientStrict}/v1/auth/register/verify`, {
...
define('C6', '注册登录攻击', '验证码并发穷举(10 并发错误码)→ CAS 不丢计数,零成功', async (c) => {
  ...
  ok(
    ...
    'all concurrent wrong codes rejected',
  );
  const after = await http(`${c.url.clientStrict}/v1/auth/register/verify`, {
    body: { challengeId, code: '999999' },
  });
...
define('C7', '注册登录攻击', '已消费挑战重放 → 400', async (c) => {
  ...
  const replay = await http(`${c.url.clientStrict}/v1/auth/register/verify`, {
    body: { challengeId, code },
  });
...
define(
  'C9',
  '注册登录攻击',
  '并发 verify 同一正确码 ×10 → 恰好 1 个成功(单次消费竞态)',
  async (c) => {
    resetRegisterGate();
...
define(
  'C10',
  '注册登录攻击',
  '同 IP 注册限流(strict 3/hour):第 4 个 → 429 + Retry-After',
  async (c) => {
    resetRegisterGate();
    let last = 0;
    let retryAfter = '';
...
define(
  'C11',
  '注册登录攻击',
  '伪造 X-Forwarded-For 换 IP 绕注册限流 → 无效(TRUSTED_PROXY_HOPS=0)',
  async (c) => {
    // 先耗尽本 IP 限额
    for (let i = 0; i < 3; i++) {
...
define('C12', '注册登录攻击', '登录爆破:5 次错密码 → 锁定,正确密码在锁内也被拒', async (c) => {
  ...
  for (let i = 0; i < 5; i++) {
    const r = await http(`${c.url.client}/v1/auth/login`, {
      body: { email: u.email, password: 'WrongPass#1' },
    });
    eq(r.status, 401, `wrong #${i + 1} → 401`);
  }
  const locked = await http(`${c.url.client}/v1/auth/login`, {
...
define('C13', '注册登录攻击', '登录用户枚举防护:不存在 vs 密码错同码,时序差 < 300ms', async (c) => {
  ...
  const t1 = Date.now();
  const r1 = await http(`${c.url.client}/v1/auth/login`, {
    body: { email: 'ghost@fire.test', password: 'Whatever#1' },
  });
  const d1 = Date.now() - t1;
  const t2 = Date.now();
  const r2 = await http(`${c.url.client}/v1/auth/login`, {
    body: { email: u.email, password: 'Whatever#1' },
  });
...
define('C15', '注册登录攻击', '会话安全:伪造/篡改 JWT 401;登出后 token 即死', async (c) => {
  ...
  eq(
    await (
      await http(`${c.url.client}/v1/me`, { headers: { authorization: `Bearer ${token}` } })
    ).status,
    200,
    'me ok',
  );
  ...
  eq(
    await (
      await http(`${c.url.client}/v1/me`, { headers: { authorization: `Bearer ${tampered}` } })
    ).status,
    401,
    'tampered jwt 401',
  );
  eq(
    await (
      await http(`${c.url.client}/v1/me`, { headers: { authorization: 'Bearer nope.nope.nope' } })
    ).status,
    401,
    'garbage jwt 401',
  );
  const out = await http(`${c.url.client}/v1/auth/logout`, {
    method: 'POST',
...
define(
  'C16',
  '注册登录攻击',
  '越权:A 的 token 摸 B 的 keys/wallet/usage → 无数据泄露',
  async (c) => {
    const a = await c.seed.mkUser(c.db, 'c16a');
    const b = await c.seed.mkUser(c.db, 'c16b');
    const la = await http(`${c.url.client}/v1/auth/login`, {
      body: { email: a.email, password: PASSWORD },
    });
    const tokenA = la.json().token;
    const lb = await http(`${c.url.client}/v1/auth/login`, {
      body: { email: b.email, password: PASSWORD },
    });
    const tokenB = lb.json().token;
    const kb = await http(`${c.url.client}/v1/keys`, {
      body: { name: 'rt-c16-b' },
    ...
    const walletA = await http(`${c.url.client}/v1/wallet/accounts`, {
      headers: { authorization: `Bearer ${tokenA}` },
    });
...
define(
  'C17',
  '注册登录攻击',
  'key 洪泛:连建 60 个 key(设计=数量自由)→ 服务不降级,全部可用',
  async (c) => {
    ...
    let created = 0;
    for (let i = 0; i < 60; i++) {
      const r = await http(`${c.url.client}/v1/keys`, {
        body: { name: `rt-c17-${i}` },
        headers: { authorization: `Bearer ${token}` },
      });
...
define('C18', '注册登录攻击', '改密后旧 token 即废,新 token 与新密码可用', async (c) => {
  ...
  eq(
    await (
      await http(`${c.url.client}/v1/me`, { headers: { authorization: `Bearer ${oldToken}` } })
    ).status,
    401,
    'old token dead',
  );
  const relogin = await http(`${c.url.client}/v1/auth/login`, {
    body: { email: u.email, password: 'Rt!NewPass#99' },
  });
...
define(
  'C19',
  '注册登录攻击',
  '验证码发送冷却:同 email 60s 内二次 register → 429 cooldown',
  async (c) => {
    resetRegisterGate();
...
define(
  'C20',
  '注册登录攻击',
  'OAuth:伪造 state 回调与未知 provider → 4xx,无跳转泄露',
  async (c) => {
