/**
 * 上游故障注入:429/5xx/4xx 透传、挂起超时、mid-stream 断连、
...
async function zeroCharge(c: any, userId: number) {
  ...
  ok(
    new (await import('@tillgate/billing')).Decimal(w.balance).gte(0),
    `balance ${w.balance} >= 0`,
  );
  const n = await c.db.execute(
    sql`select count(*)::int as n from usage_logs where user_id = ${userId}`,
  );
...
define(
  'F1',
  '上游故障',
  '上游 429(非流式)→ 重试耗尽后终态错误(4xx 透传或 502/503),零扣费',
  async (c) => {
...
define(
  'F7',
  '上游故障',
  '上游挂死不响应 → 连接超时×重试×耗尽 → 502,零扣费,不拖死网关',
  async (c) => {
...
define(
  'F10',
  '上游故障',
  '上游 200+响应体立即死亡 → 空补全按估算计费(保守),行为与空 JSON 体不一致(观察项)',
  async (c) => {
    ...
    const usage = await c.db.execute(
      sql`select count(*)::int as n, coalesce(sum(amount),0)::text as total from usage_logs where user_id = ${u.id}`,
    );
    ...
    ok(
      Number(w.balance) < 50 && Number(w.balance) > 49,
      `conservative charge, balance ${w.balance}`,
    );
  },
);
...
define(
  'F12',
  '上游故障',
  '上游 302 重定向到任意路径 → 手动跟随受 SSRF 复查,不无限跟,零扣费',
  async (c) => {
...
define(
  'F13',
  '上游故障',
  '全局打挂 chaosmock(__ctl hang)→ 超时 502;恢复后不无限 hang(熔断兜底)',
  async (c) => {
    const { u, key } = await setup(c, 'f13');
    await http(`${c.url.mock}/__ctl`, {
      method: 'POST',
      body: { scope: 'chaosmock', mode: 'hang' },
    });
    const start = Date.now();
    const r = await http(`${c.url.gw}/v1/chat/completions`, {
      body: chat('rt-429'),
    ...
    ok(
      Date.now() - t2 < 15000,
      `no indefinite hang after vendor recovery (${Date.now() - t2}ms, status ${r2.status})`,
    );
    clearBreaker();
    await healthStillOk(c);
  },
...
define(
  'F14',
  '上游故障',
  '死凭据快速止血:连续 3 次 401 → 渠道停路由(no_available_channel 快速 503)',
  async (c) => {
    const { u, key } = await setup(c, 'f14');
    for (let i = 0; i < 3; i++) {
      await http(`${c.url.gw}/v1/chat/completions`, {
        body: chat('rt-401up'),
        headers: auth(key),
        timeoutMs: 20000,
      });
