...
async function main() {
  ...
  await db.execute(
    sql`delete from usage_logs where user_id in (select id from users where issuer = 'rt-load')`,
  );
  await db.execute(
    sql`delete from billing_reservations where billing_request_id in (select request_id from billing_requests where user_id in (select id from users where issuer = 'rt-load'))`,
  );
  await db.execute(
    sql`delete from billing_requests where user_id in (select id from users where issuer = 'rt-load')`,
  );
  await db.execute(
    sql`delete from wallet_authorizations where account_id in (select id from wallet_accounts where user_id in (select id from users where issuer = 'rt-load'))`,
  );
  await db.execute(
    sql`delete from wallet_transactions where id in (select distinct l.transaction_id from wallet_legs l join wallet_accounts a on a.id = l.account_id where a.user_id in (select id from users where issuer = 'rt-load'))`,
  );
  await db.execute(
    sql`delete from wallet_legs where account_id in (select id from wallet_accounts where user_id in (select id from users where issuer = 'rt-load'))`,
  );
  await db.execute(
    sql`delete from wallet_accounts where user_id in (select id from users where issuer = 'rt-load')`,
  );
  await db.execute(
    sql`delete from api_keys where user_id in (select id from users where issuer = 'rt-load')`,
  );
  await db.execute(sql`delete from users where issuer = 'rt-load'`);
  await db.execute(
    sql`delete from wallet_legs where transaction_id not in (select id from wallet_transactions)`,
  );
  ...
  for (let i = 0; i < USERS; i += 500) {
    const batch = Array.from({ length: Math.min(500, USERS - i) }, (_, j) => {
      const key = `sk_rt${Array.from(randomBytes(20))
    ...
    await db.execute(sql`
      ...
      select 'rt-load', tag, 'local' from (values ${sql.join(
        batch.map((b) => sql`(${b.tag})`),
        sql`, `,
      )}) as v(tag)
      returning id`);
    // 逐行拿 id 再插 key(批量 returning 顺序不稳,这里以 tag 关联)
    const ids = await db.execute<{ id: number; subject: string }>(sql`
      select id, subject from users where issuer = 'rt-load' and subject in (${sql.join(
        batch.map((b) => sql`${b.tag}`),
        sql`, `,
      )})`);
    const idByTag = new Map(
      rowsOf<{ id: number; subject: string }>(ids).map((r) => [r.subject, r.id]),
    );
    ...
    await db.execute(sql`
      ...
      values ${sql.join(
        batch.map(
          (b) =>
            sql`(${createHash('sha256').update(b.key).digest('hex')}, 'sk-load', ${idByTag.get(b.tag) ?? 0}, 'load')`,
        ),
        sql`, `,
      )}`);
    keys.push(...batch.map((b) => b.key));
  }
  const fundable = rowsOf<{ id: number }>(
    await db.execute(sql`select id from users where issuer = 'rt-load' order by id`),
  );
  await mapLimit(fundable.slice(0, USERS), 2, async (u) => {
    await fund(u.id, '1', randomUUID());
  });
  ...
  for (let i = 0; i < jobs.length; i += WAVE) {
    ...
    process.stdout.write(
      `  波 ${Math.floor(i / WAVE) + 1}/${Math.ceil(jobs.length / WAVE)}: 累计 ${results.length} 次(${((Date.now() - t0) / 1000).toFixed(1)}s)\n`,
    );
  }
  ...
  console.log(
    `[load] 延迟(成功): p50=${p50(lat).toFixed(3)}s p95=${p95(lat).toFixed(3)}s p99=${p99(lat).toFixed(3)}s max=${(lat.at(-1) ?? 0).toFixed(3)}s`,
  );
  ...
  for (;;) {
    ...
    if (Date.now() - tDrain > 300_000) {
      console.log(`[load] ⚠️ 排空超时(300s)`);
      break;
    }
  ...
  const bad1 = await db.execute(
    sql`select count(*)::int as n from (select transaction_id from wallet_legs group by 1 having sum(amount) <> 0) x`,
  );
  const bad2 = await db.execute(
    sql`select count(*)::int as n from wallet_accounts ac where ac.balance <> coalesce((select l.balance_after from wallet_legs l where l.account_id = ac.id order by l.id desc limit 1), 0)`,
  );
  const bad3 = await db.execute(
    sql`select count(*)::int as n from wallet_accounts ac where ac.in_flight <> coalesce((select sum(a.amount) from wallet_authorizations a where a.account_id = ac.id and a.status = 'active'), 0)`,
  );
  const usageN = await db.execute<{ n: number }>(
    sql`select count(*)::int as n from usage_logs where user_id in (select id from users where issuer = 'rt-load')`,
  );
  ...
  console.log(
    `[load] 不变量: 腿平衡✗${b1} 余额✗${b2} 在途✗${b3} | usage 行=${usageCount}/${TOTAL}`,
  );
  ...
  console.log(
    `[load] 抽样 20 用户计费一致: ${uniform ? '✓ 全等(' + sums[0] + ')' : '✗ 不一致: ' + sums.slice(0, 3).join(',')} | 每用户期望 ${PER} 笔`,
  );
  const counts = await db.execute<{ n: number }>(
    sql`select count(*)::int as n from usage_logs where user_id = ${sampleIds[0]}`,
  );
