/**
 * 超级并发与系统级:百级并发精确对账、低余额预扣击穿、取消风暴、
...
function clearBreakerKeys() {
  ...
  Bun.spawnSync([
    'bash',
    '-c',
    `redis-cli -a ${pass} --scan --pattern 'inference:health:*' 2>/dev/null | xargs -r redis-cli -a ${pass} del >/dev/null 2>&1; redis-cli -a ${pass} --scan --pattern 'inference:health:*' 2>/dev/null | xargs -r redis-cli -a ${pass} del >/dev/null 2>&1`,
  ]);
}
...
async function waitUserQuiet(c: any, userId: number, expectUsage: number, timeoutMs = 45000) {
  return poll(
    ...
    async () => {
      ...
      if (
        Number(u.n) === expectUsage &&
        Number(b.terminal) === Number(b.total) &&
        Number(w.in_flight) === 0
      ) {
        return {
          usageCount: Number(u.n),
          usageTotal: String(u.total),
          billCount: Number(b.total),
          balance: w.balance,
        };
      }
      return null;
...
define(
  'X11',
  '超级并发',
  '多用户高并发基准:40 用户 × 5 = 200 同瞬并发 → 全成功、无 5xx、精确对账',
  async (c) => {
    ...
    console.log(
      `    [x11] ${X11_USERS * 5} 并发(${X11_USERS}用户×5)墙钟 ${wall}ms: 成功 ${ok200}/${X11_USERS * 5},非200=${JSON.stringify(errors)}`,
    );
...
define('X1', '超级并发', '50 并发混合(5 用户×10,流+非流)→ 全结算,逐用户精确对账', async (c) => {
      ...
      jobs.push(
        (async () => {
          ...
          for (let attempt = 0; ; attempt++) {
            if (stream) {
              const sh = sse(
                `${c.url.gw}/v1/chat/completions`,
                chat(model, { stream: true }),
                auth(s.key),
              );
            ...
            if (attempt >= 3)
              throw new Error(`x1 request exhausted retries (${model} stream=${stream})`);
            await sleep(400);
          }
        })(),
      );
  ...
  for (const s of users) {
    ...
    eqDec(
      q.balance,
      expected.toFixed(18).replace(/0+$/, '').replace(/\.$/, ''),
      'balance = fund − Σusage',
    );
  }
});
...
define(
  'X2',
  '超级并发',
  '低余额并发击穿:余额 0.01 × 50 并发 → 通过数受预扣钳制,余额永不为负',
  async (c) => {
    // 快路径副作用:上一高并发用例的结算信号从长尾变瞬时脉冲,BullMQ 结算
    ...
    console.log(
      `    [x2-diag] 200:${passes} 402:${rejected} other:${JSON.stringify(
        results
          .filter((x) => x !== 200 && x !== 402)
          .reduce((m, x) => {
            m[x] = (m[x] ?? 0) + 1;
            return m;
          }, {}),
    ...
    ok(
      results.every((x) => x === 200 || x === 402 || x >= 500),
      'terminal statuses only (200/402/bounded 5xx)',
    );
    ...
    eqDec(
      w.balance,
      new Decimal('0.01').minus(q.usageTotal).toFixed(18).replace(/0+$/, '').replace(/\.$/, ''),
      'exact reconcile',
    );
  },
);
...
define(
  'X3',
  '超级并发',
  '取消风暴:10 并发慢流随机时点 abort → 10 条账单,in_flight 归零',
  async (c) => {
...
define('X4', '超级并发', '渠道 RPM=3 × 12 并发 → 上游调用被钳制,超额 503 零扣费', async (c) => {
  ...
  const before = ((await http(`${c.url.mock}/__metrics`)).json().metrics.byModel ?? {}) as Record<
    string,
    number
  >;
  const results = await Promise.all(
  ...
  const after = ((await http(`${c.url.mock}/__metrics`)).json().metrics.byModel ?? {}) as Record<
    string,
    number
  >;
...
define('X5', '超级并发', '故障转移:ctl 打挂主渠道 429 → 备渠道接管成功,计费恰好一次', async (c) => {
  ...
  const before = (await http(`${c.url.mock}/__metrics`)).json().metrics.requests as Record<
    string,
    number
  >;
  ...
  try {
    ...
    const after = (await http(`${c.url.mock}/__metrics`)).json().metrics.requests as Record<
      string,
      number
    >;
...
define(
  'X6',
  '超级并发',
  'worker SIGKILL 崩溃中结算 → 重启后恰好一次全结算,无重复无丢失',
  async (c) => {
    ...
    try {
      ...
      eq(
        statuses.filter((x) => x === 200).length,
        30,
        'all requests succeeded (settle-signal durable)',
      );
    } finally {
      await restartWorker();
    }
...
define(
  'X7',
  '超级并发',
  'worker 停摆 + SQL 造积压 → 准入关闸(fail-closed)→ 恢复后真实账单全额结算',
  async (c) => {
    const wp = workerProc();
    ...
    eqDec(
      String(u.total),
      new Decimal('0.000546').times(Number(u.n)).toString(),
      'exact per settled',
    );
    const w = await c.seed.wallet(c.db, s.u.id);
...
define(
  'X10',
  '超级并发',
  '终极对账:三不变量(腿平衡/余额=末腿/in_flight=授权和)+余额公式全库核验',
  async (c) => {
    // ① 每笔交易腿和 = 0
    ...
    for (const row of bad4 as any[]) {
      ...
      ok(
        new Decimal(row.bal).eq(expect),
        `user ${row.id}: balance ${row.bal} == credited ${row.credited} − used ${row.used}`,
      );
    }
