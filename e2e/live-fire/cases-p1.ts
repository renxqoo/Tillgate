/**
 * G7 毒账单回归（FINDINGS F-1 验收用例——BullMQ 重构 + NaN 防护的闭环证明）：
...
define('P1', '毒账单回归', 'F-1 闭环:usage 插入持续失败(FK)→ worker 存活 + 账单死信 + billing_dead 入箱', async (c) => {
  ...
  await poll('dead + billing_dead enqueued', async () => {
    const bill = await c.seed.billOf(c.db, reqId);
