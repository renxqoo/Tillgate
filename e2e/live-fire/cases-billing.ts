/**
 * 计费正确性与薅羊毛向量:上游 usage 伪造/缺失/畸形、中途断开、
...
define(
  'B1',
  '计费·薅羊毛',
  '上游谎报 usage=0 但内容已交付 → 按估算计费防白嫖,终态干净',
  async (c) => {
...
define(
  'B3',
  '计费·薅羊毛',
  '上游伪造巨型 usage(10 亿 token)→ 金额=公式精确值,#over 透支补扣',
  async (c) => {
...
define(
  'B6',
  '计费·薅羊毛',
  '流式逐帧 usage + 中途断开 → 按最后帧精确计费(不多收不免费)',
  async (c) => {
    const { u, key } = await setup(c, 'b6');
    const s = sse(
      `${c.url.gw}/v1/chat/completions`,
      chat('rt-perframe', { stream: true }),
      auth(key),
    );
...
define('B7', '计费·薅羊毛', '非流式客户端早断 → 至多 1 条账单,无悬挂无重复', async (c) => {
  ...
  const n = await c.db.execute(
    sql`select count(*)::int as n from usage_logs where user_id = ${u.id}`,
  );
...
define('B8', '计费·薅羊毛', '费率卡系数 0.5 → 金额恰为半价(逐分毫)', async (c) => {
  ...
  await c.db.execute(
    sql`insert into rate_cards (name) values ('rt-half') on conflict (name) do nothing`,
  );
  ...
  for (const [tag, keyRaw] of [
    ...
    const usage = await poll(`${tag} usage`, () =>
      c.seed.usageRow(c.db, r.headers.get('x-request-id')!),
    );
...
define('B9', '计费·薅羊毛', '用户日限额小于预扣估算 → 402 整单拒绝零扣费', async (c) => {
  ...
  const n = await c.db.execute(
    sql`select count(*)::int as n from billing_requests where user_id = ${u.id}`,
  );
...
define(
  'B11',
  '计费·薅羊毛',
  '上游重试幂等:同 idempotency-key 重发不双花,最终失败零扣费',
  async (c) => {
    const { u, key } = await setup(c, 'b11');
    const before = ((await http(`${c.url.mock}/__metrics`)).json().metrics.idempotencyKeys ??
    ...
    const after = ((await http(`${c.url.mock}/__metrics`)).json().metrics.idempotencyKeys ??
      {}) as Record<string, number>;
...
define(
  'B13',
  '计费·薅羊毛',
  '上游回显伪造 model:改写默认关(§3.6 设计)→ 关键=计费仍按对外目录价',
  async (c) => {
    const { key } = await setup(c, 'b13');
    const r = await http(`${c.url.gw}/v1/chat/completions`, {
      body: chat('rt-wrongmodel'),
      headers: auth(key),
    });
    eq(r.status, 200, 'status');
    ...
    ok(
      ['rt-wrongmodel', 'wrong-model-echo'].includes(seen),
      `model field passthrough/rewrite, got ${seen}`,
    );
...
define(
  'B15',
  '计费·薅羊毛',
  '上游巨量内容洪流(64KB×2 chunk)→ 网关不崩,按 usage 计费',
  async (c) => {
...
define(
  'B16',
  '计费·结算恢复',
  'job 已完结(completed 残留)×唤醒重投 → 终态残留出口,重投不被去重吞掉',
  async (c) => {
    ...
    // 的 jobId 去重静默吞掉 → 结算恢复链路永久停摆;新出口 remove 后重投
