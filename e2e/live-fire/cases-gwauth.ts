/**
 * G4 网关鉴权与协议滥用:坏/废/封/过期 key、JWT 伪造、爆破锁定(strict 实例)、
...
define('A2', '网关鉴权', '超长 10KB token / 控制字符 / emoji 垃圾 → 401 或传输层拒绝,不炸', async (c) => {
  ...
  for (const [name, value] of attempts) {
    let status = -1;
    try {
      const r = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: { authorization: value } });
      status = r.status;
    } catch {
...
define('A3', '网关鉴权', '格式合法但 hash 不存在的随机 key → 401', async (c) => {
  ...
  const r = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(fake) });
...
define('A4', '网关鉴权', '撤销即刻生效:DELETE /v1/keys/:id 后同一 key 立即 401', async (c) => {
  ...
  // 真实路径登录 + 建 key + 撤销
  const login = await http(`${c.url.client}/v1/auth/login`, { body: { email: s.u.email, password: 'Rt!Passw0rd#42' } });
  ...
  const ok1 = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(plaintext) });
  ...
  const r2 = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(plaintext) });
...
define('A5', '网关鉴权', '封禁用户:key 立即 401(热路径逐请求查状态)', async (c) => {
  ...
  const r = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(s.key) });
  eq(r.status, 401, 'banned user key → 401');
  await c.db.execute(sql`update users set status = 0 where id = ${s.u.id}`);
  const r2 = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(s.key) });
  eq(r2.status, 200, 'unban restores');
});

define('A6', '网关鉴权', '过期 key(expires_at 过去)→ 401', async (c) => {
  const s = await setup(c, 'a6');
  await c.db.execute(sql`update api_keys set expires_at = now() - interval '1 hour' where user_id = ${s.u.id}`);
  const r = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(s.key) });
...
define('A7', '网关鉴权', 'App JWT 分支伪造:错 secret 签名 / alg none / 过期 → 401', async (c) => {
  ...
  const jwt1 = await new SignJWT({ realm: 'app' }).setProtectedHeader({ alg: 'HS256' }).setIssuedAt().setExpirationTime('1h').sign(fakeSecret);
  const r1 = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(jwt1) });
  eq(r1.status, 401, 'wrong-secret jwt → 401');
  const jwt2 = await new SignJWT({ realm: 'app' }).setProtectedHeader({ alg: 'HS256' }).setIssuedAt().setExpirationTime(Math.floor(Date.now() / 1000) - 3600).sign(fakeSecret);
  const r2 = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(jwt2) });
  eq(r2.status, 401, 'expired jwt → 401');
  await zeroBills(c, u.id);
});

define('A8', '网关鉴权', 'IP 维度爆破锁定(strict 8812 默认阈值 30/300s)→ 锁内有效 key 也 401(组末执行防污染)', async (c) => {
  // 31 个随机无效 key 打 strict 网关 → IP 锁(authfail:ip:lock:*)
  let last = 0;
  for (let i = 0; i < 31; i++) {
    const fake = `sk_rt${Array.from({ length: 20 }, () => Math.floor(Math.random() * 16).toString(16)).join('')}`;
    const r = await http(`${c.url.gwStrict}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(fake) });
    last = r.status;
  }
  eq(last, 401, 'brute force all 401');
  const s = await setup(c, 'a8');
  const r = await http(`${c.url.gwStrict}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(s.key) });
  ...
  const clear = () =>
    Bun.spawnSync(['bash', '-c', `redis-cli -a ${pass} --scan --pattern 'authfail:*' 2>/dev/null | xargs -r redis-cli -a ${pass} del >/dev/null 2>&1; redis-cli -a ${pass} --scan --pattern 'auth:*' 2>/dev/null | xargs -r redis-cli -a ${pass} del >/dev/null 2>&1`]);
  clear();
  await sleep(500);
  const probe = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(s.key) });
  if (probe.status !== 200) {
    clear();
    await sleep(1000);
    const probe2 = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-base'), headers: auth(s.key) });
...
define('A9', '网关鉴权', '模型不存在/未上架 → 干净 4xx 零扣费', async (c) => {
  ...
  for (const model of ['no-such-model', '', 'rt-base\u0000injection', 'a'.repeat(200)]) {
    const r = await http(`${c.url.gw}/v1/chat/completions`, { body: chat(model), headers: auth(s.key) });
...
define('A11', '网关鉴权', '畸形请求体:非 JSON / 空体 / 缺 messages / 错 content-type → 400 零扣费', async (c) => {
  const s = await setup(c, 'a11');
  const raw = async (body: string, headers: Record<string, string> = { 'content-type': 'application/json' }) =>
    http(`${c.url.gw}/v1/chat/completions`, { body, raw: true, headers: { ...auth(s.key), ...headers } });
...
define('A12', '网关鉴权', '消息数组畸形:空/缺 role/超长 content/1000 条消息 → 干净分流(4xx 拒或 2xx 精确计费)', async (c) => {
  ...
  const bodies = [
    ...
    { model: 'rt-base', messages: Array.from({ length: 1000 }, (_, i) => ({ role: 'user', content: `m${i}` })) },
  ];
  let ok200 = 0;
  for (const body of bodies) {
    const r = await http(`${c.url.gw}/v1/chat/completions`, { body, headers: auth(s.key), timeoutMs: 30000 });
...
define('A13', '网关鉴权', '413 超限 body(10MB+1)→ 413 零扣费', async (c) => {
  ...
  const huge = JSON.stringify({ model: 'rt-base', messages: [{ role: 'user', content: 'x'.repeat(10 * 1024 * 1024 + 100) }] });
...
define('A14', '网关鉴权', '伪造重复 x-request-id 两个请求 → 独立计费不合并不串账', async (c) => {
  ...
  const r1 = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-exact'), headers: forged });
  const r2 = await http(`${c.url.gw}/v1/chat/completions`, { body: chat('rt-exact'), headers: forged });
  eq(r1.status, 200, 'r1');
  eq(r2.status, 200, 'r2');
...
define('A15', '网关鉴权', 'SSRF:严格实例(8812)本地地址渠道必须被拒(逃生门仅 dev 主实例放行)', async (c) => {
