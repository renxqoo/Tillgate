...
export async function lintSample(code: string): Promise<LintResult> {
  const dir = mkdtempSync(join(tmpdir(), 'multi-component-oxlint-'));
  try {
    const config = {
      jsPlugins: [join(pluginDir, 'index.ts')],
      rules: { 'multi-component/no-multi-component': 'error' },
    };
...
export function ruleCount(stdout: string, rule: string): number {
  return stdout.split(`multi-component(${rule})`).length - 1;
}
