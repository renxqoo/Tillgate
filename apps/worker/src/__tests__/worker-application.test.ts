...
describe('worker runtime configuration boundary', () => {
  it('rejects unsafe retry and claim configuration', () => {
    expect(() => loadWorkerEnv({ WORKER_CLAIM_LEASE_MS: '0' })).toThrow();
    expect(() => loadWorkerEnv({ WORKER_MAX_SETTLEMENT_ATTEMPTS: '0' })).toThrow();
    expect(() => loadWorkerEnv({ WORKER_POLL_INTERVAL_MS: '10' })).toThrow();
    expect(() =>
      loadWorkerEnv({ WORKER_RETRY_BASE_MS: '5000', WORKER_RETRY_MAX_MS: '1000' }),
    ).toThrow();
    expect(() =>
      loadWorkerEnv({ WORKER_CLAIM_LEASE_MS: '1000', WORKER_POLL_INTERVAL_MS: '1000' }),
    ).toThrow();
  });
