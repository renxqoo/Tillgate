 ...
 * 轮询是正确性兜底（真相在 DB 认领），PG LISTEN/NOTIFY 唤醒只是低延迟优化。
 */
import { z } from 'zod';
...
const schema = z.object({
  ...
  /** 驱动节奏：结算扫描为唤醒通道的兜底（默认 30s；唤醒通道故障时退化为该节奏） */
  ...
  /** 邀请返利日结节奏（佣金比例存 marketing_settings——每 tick 读现值） */
  WORKER_REFERRAL_INTERVAL_MS: z.coerce.number().int().positive().default(3_600_000),
  /** 告警投递（notify_outbox 消费者——webhook/邮件） */
