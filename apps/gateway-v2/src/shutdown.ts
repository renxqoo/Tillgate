...
export function createShutdown(deps: ShutdownDeps): (signal: string) => void {
  ...
  return (signal: string) => {
    ...
    triggered = true;
    console.log(`[gateway-v2] ${signal} received, draining`);
    deps.server.close(() => {
      void (async () => {
      ...
      await deps.redis?.quit().catch(() => {});
        ...
        console.log('[gateway-v2] drained');
        exit(0);
      })();
    });
    setTimeout(() => {
      console.error('[gateway-v2] drain grace expired, forcing exit');
      exit(1);
