/**
 * gateway-v2 边界铁律的机器强制（domain/services 已提取为共享包后的形态）：
 *
 ...
 *   3. 全新实现：不 import 旧包（ledger / wallet / ledger-core）
 */
...
const ALLOWED_PACKAGES = [
  ...
  'bullmq', // 结算唤醒队列（Redis 必配形态的事件通道——装配层专用）
];
