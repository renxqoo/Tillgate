 ...
 * 与 gateway-v2 同语义（用户面无长流，宽限通常瞬过）。
 */
...
export function createShutdown(deps: ShutdownDeps): (signal: string) => void {
  ...
  return (signal: string) => {
    ...
    triggered = true;
    console.log(`[client-api-v2] ${signal} received, draining`);
    deps.server.close(() => {
      void (async () => {
        await deps.otel.shutdown().catch(() => {});
        await deps.redis?.quit().catch(() => {});
        await deps.db.$client.end().catch(() => {});
        console.log('[client-api-v2] drained');
        exit(0);
      })();
    });
    setTimeout(() => {
      console.error('[client-api-v2] drain grace expired, forcing exit');
      exit(1);
