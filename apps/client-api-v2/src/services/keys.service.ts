...
export interface KeysService {
   ...
   * 降级为个人余额（subscriptionId=null，v1 L1 语义）。明文仅此一次返回。
   */
  rotate(
    ctx: RunContext,
    userId: number,
...
export function createKeysService(deps: KeysServiceDeps): KeysService {
  ...
  return {
    ...
    async revoke(ctx, userId, keyId) {
      ...
      if (!owned) throw new AppError(404, 'key_not_found', 'Key 不存在');
      if (owned.status !== 0) throw new AppError(409, 'key_already_revoked', 'Key 已吊销');
    ...
    async patch(ctx, userId, keyId, patch) {
      ...
      if (!updated) throw new AppError(409, 'key_already_revoked', 'Key 已吊销');
    ...
    async rotate(ctx, userId, keyId) {
      ...
      // 计费来源重新校验（L1）：过期/失格 → 新 Key 降级为个人余额
      ...
      const created = await db.transaction(async (tx) => {
        ...
        // 新 Key 继承设置（含未迁移的过期时间）；quota 闸按「替换」口径不计增量
