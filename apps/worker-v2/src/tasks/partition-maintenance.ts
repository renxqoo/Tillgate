/**
 * 分区维护（v1 双循环的 v2 移植）：trace_spans（packages/tracing maintainPartitions）
 * + request_logs（月分区，v1 request-log-partitions 同逻辑）。
...
/** v1 request-log-partitions.ts 原样移植（可测纯函数；DDL 边界由服务端生成） */
