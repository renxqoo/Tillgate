 ...
 *   - 全局系数 PATCH 只触碰 scope='global' 行（model/group 覆写行隔离——M1）
...
export function createRateCardsService(deps: RateCardsServiceDeps): RateCardsService {
  ...
  return {
    ...
    async update(ctx, input) {
      ...
      if (!row) throw new AppError(404, 'rate_card_not_found', '费率卡不存在');
      await recordAudit(db, {
        actor: 'admin',
        adminId: input.adminId,
    ...
    async remove(ctx, input) {
      ...
      if (bound > 0) {
        throw new AppError(409, 'rate_card_in_use', '该费率卡仍有用户绑定，无法删除（请先迁移用户）');
      }
      const ok = await db.transaction(async (tx) =>
        repos.rateCard.deleteCard({ db: tx, ...ctx }, { rateCardId: input.rateCardId }),
      );
      if (!ok) throw new AppError(404, 'rate_card_not_found', '费率卡不存在');
      await recordAudit(db, {
        actor: 'admin',
        adminId: input.adminId,
