/**
 * admin-api-v2 启动入口（仅 bootstrap，无业务逻辑）：
...
const startupRedis = createRedisClient(config.REDIS_URL, { serviceName: 'admin-api-v2' });
await assertRedisReachable(startupRedis, 'admin-api-v2', config.REDIS_URL);
...
const server = serve({ fetch: app.fetch, port: config.PORT }, (info) => {
  console.log(`[admin-api-v2] listening on :${info.port}`);
  // 配置快照：关键业务参数生效值一处可查（排查「以为配了其实默认」类问题）
  console.log(`[admin-api-v2] config snapshot: ${JSON.stringify({
