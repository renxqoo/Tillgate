/**
 * admin-api-v2 E2E 共享基建：真服务进程（全真装配：真 DB/真 wallet/真幂等内核）
 * + 种子管理员 + HTTP 助手 + 清理台账。与 client-api-v2 e2e-kit 同哲学：
...
export function e2eAdminConfig(overrides: Partial<AdminApiConfig> = {}): AdminApiConfig {
  return {
    ...
    VOUCHER_DIR: `/tmp/aav2-e2e-vouchers-${randomUUID().slice(0, 8)}`,
...
/** 起真 admin-api-v2（全真装配；extra 覆盖配置） */
