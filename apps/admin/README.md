# @tillgate/admin —— Next.js 管理后台（BFF）
...
- **取数唯一出口**：`src/server/admin-api.ts` 的 `adminApi()` = `createNextAdminApiClient()`（`@tillgate/api-client/next`，每请求新建）；路径白名单 `/v1/*`，禁止页面裸 fetch 直连 admin-api
...
- BFF 侧 env（读自 `@tillgate/api-client/next` 装配层）：
...
- 协议/厂商词表：providers 页经 admin-api `/v1/vendor-catalog` 消费（单一事实源 = `@tillgate/ai` 根出口；后端 zod 仍是最终防线）

## 装配与依赖（BFF 模式）

- 运行时能力依赖仅 `@tillgate/api-client`（wire DTO 手写快照 + `./next` 装配）与 `@tillgate/ui`（设计系统）；不 import 任何 `@tillgate/{ai,inference,db,http,runtime,identity,accounts,billing,control-plane,notifications,observability}`
