/**
 * 凭据表单提交方式契约（安全回归，2026-08-25，与 client 端同款）：features/auth
 * 的所有 <form> 必须声明 method="post"——React 注水前用户按回车会触发原生提交，
 * 缺省 method=GET 会把 email/password 拼进 URL（nginx access log、浏览器历史、
 * 代理层留痕；实测 GET /login?email=...&password=...）。注水后 onSubmit 拦截
 * 不受影响；method 只约束预水合的原生提交走 POST 体。
 */
...
describe('auth 表单 method=post 契约', () => {
  it('features/auth 下每个 <form> 开标签都声明 method="post"（预水合提交不进 URL）', () => {
    ...
    expect(checked, '守护面缩小：auth 表单数少于现状 4').toBeGreaterThanOrEqual(4);
  });
});
