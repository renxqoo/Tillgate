/**
 * 密码/密钥输入契约：所有以 password 形态遮蔽的输入框都必须
...
describe('管理端密码输入 placeholder', () => {
  it('全部 password 形态输入框都通过 i18n 提供非空 placeholder', () => {
    const inputs = passwordInputTags();
    expect(inputs.length, '守护面缩小：password 形态输入框少于现状 8 个').toBeGreaterThanOrEqual(8);
  ...
  it('新增 placeholder 键在中英词表中均存在且非空', () => {
    const keys = [
      ...
      ['channels', 'apiKeyPlaceholder'],
      ['channels', 'apiKeyMaskedPlaceholder'],
    ] as const;
