...
**范围演进**：首波因并行波未提交改动（铁律 15）只落零接缝部分；并行波提交后 P1
（billing 四接缝:plans CRUD/订阅管理列表/兑换批次/死信单笔复审）已在同日落地
（billing MIGRATION-U6）。剩余 P2–P7 见 IMPLEMENTATION.md §3。
...
- app 自有目录 `AdminErrors`（`admin.*`）：`invalid_param`(400)、`invalid_sort_field`(400)、
  `user_not_found`(404)、`catalog_source_not_found`(404——形状非法归 admin.*;形状合法未注册经 control-plane 目录面抛 `control_plane.catalog_source_not_found`,e2e 实测)。
...
v1 测试文件，映射见 MIGRATION §2）、`admin-api.real.test.ts`（真实 PG 冒烟，不可达优雅跳过）；**端到端旅程在仓根 `e2e/admin/`**（P5 归位：进程内全真装配 + 真监听 + identity 签发真令牌，经本包 `bun run test:e2e` 运行；v1 五 e2e 覆盖矩阵与 P2/P4/P7 缺口见 `e2e/admin/journey.test.ts` 文件头；bun 运行时默认不应用 exports 的 development 条件——真进程 `bun src/index.ts` 须 `--conditions=development`，已记档）。
