...
export function catalogRoutes(
  ...
  app.get('/v1/model-catalog/sources', guard('catalog:read'), (c) =>
    c.json({ sources: catalog.listSources() }),
  );

  /** 价格溯源：注册在 :sourceId 之前——否则字面段被参数路由吞掉（v1 同序）。 */
  app.get('/v1/model-catalog/price-history', guard('catalog:read'), async (c) => {
  ...
  app.get('/v1/model-catalog/:sourceId', guard('catalog:read'), async (c) => {
    const sourceId = catalogSourceParam(c.req.param('sourceId'));
    return c.json(await catalog.comparison(sourceId));
  });

  app.post('/v1/model-catalog/import', guard('catalog:import'), async (c) => {
  ...
  app.get('/v1/vendor-catalog', guard('catalog:read'), (c) =>
