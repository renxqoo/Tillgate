 ...
 *
 *   1. app 只做协议适配与编排：源码只准 import 共享包 + ai 适配器包
...
describe('admin-api 分层边界', () => {
  ...
  it('源码不 import 白名单外的包，不 import 冻结包', () => {
    ...
    for (const file of files) {
      ...
      for (const match of src.matchAll(/from '([^']+)'/g)) {
        ...
        if (FROZEN_LEDGER_PACKAGES.some((frozen) => spec === frozen || spec.startsWith(`${frozen}/`))) {
          offenders.push(`${relative(SRC_ROOT, file)} → ${spec}（冻结包）`);
          continue;
        }
