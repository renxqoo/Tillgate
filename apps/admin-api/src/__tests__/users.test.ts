...
describe('流水（wallet statement 单一真相）', () => {
  it('三条入金 newest-first + 余额链；列表信封 {rows,total}', async () => {
    const { request } = buildTestApp();
    const { token } = await newAdmin();
    const userId = await newUserRow();
    for (let i = 0; i < 3; i += 1) {
    ...
    // 列表信封 {rows,total}：前端 fetchAdminList 只读 data.rows/data.total——返回 items = 列表恒空
...
describe('用户维度审计（AuditLogRow 完整形状，含 adminSubject/targetType）', () => {
...
describe('用户详情（profile 钱包富化：余额/可用额度）', () => {
