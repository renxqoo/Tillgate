...
const patchSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.freezeReason != null && value.status !== 1) {
      ctx.addIssue({ code: 'custom', message: 'freezeReason can only be set together with ban (status=1)' });
    }
  });
...
export function usersRoutes(
  ...
  app.get('/v1/users/:id/transactions', session, async (c) => {
    ...
    // 列表信封 {rows,total}：前端 fetchAdminList 只读 data.rows/data.total（items 会让列表恒空）
