...
const orderParam = (raw: string): string => {
  if (!/^[0-9a-f-]{16,64}$/.test(raw)) {
    throw new AppError(400, 'invalid_param', '订单 id 必须为 uuid');
  }
  return raw;
};
...
export function opsRoutes(service: OpsLogsService, session: MiddlewareHandler<SessionEnv>) {
  ...
  app.get('/v1/usage-logs', session, async (c) => {
    ...
    return c.json(
      await service.usageLogs(adminCtxOf(c), { query, ...extra, estimated: parseEstimated(extra.estimated) }),
    );
  });
  ...
  app.post('/v1/payment-orders/:id/close', session, async (c) => {
    ...
    return c.json(await service.closePaymentOrder(adminCtxOf(c), { adminId: c.get('adminId'), orderId }));
  });
