...
const amount = z.string().regex(/^\d{1,10}(\.\d{1,18})?$/, 'Amount must be a non-negative decimal string (max 18 decimal places)');
const rate = z.string().regex(/^(?:0(?:\.\d{1,18})?|1(?:\.0{1,18})?)$/, 'Rate must be between 0 and 1 (max 18 decimal places)');
