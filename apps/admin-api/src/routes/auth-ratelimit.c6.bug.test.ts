...
/**
 * TDD 复现测试 —— 控制台 /api/auth/login 无限流无锁定（C6）。
 *
 * gateway 对静态 Key 有 brute-force-guard（5 次失败锁 10 分钟），
 * 但 admin-api 的登录路由没有任何限流/锁定。scrypt verifyPassword 可被放大 DoS。
 *
 * 需要真实 Postgres（hashPassword 走真实 scrypt）。当前应 FAIL：
 * 连续 N 次失败后，下一次请求仍能继续尝试（未锁定）。
 */
...
function makeApp(): Hono<AdminEnv> {
  ...
  app.route('/', authRoutes(db, { jwtSecret: SECRET, giftAmount: 0, secureCookie: false, redis }));
  return app;
}
...
describe('C6 — 控制台登录限流/锁定（修复后达阈值锁定）', () => {
  it('连续失败达阈值（5 次）后 → 后续尝试被锁定（429）', async () => {
    ...
    const app = makeApp();
    // 清理可能的残留限流键（subject 唯一，理论上无残留，保险起见）
    const throttleKey = `login:fails:${subject}:unknown`;
    const lockK = `login:lock:${subject}:unknown`;
    try {
  ...
  it('登录成功 → 清零计数（不误锁）', async () => {
    ...
    try {
      ...
      await redis.del(`login:fails:${subject}:unknown`).catch(() => {});
      await redis.del(`login:lock:${subject}:unknown`).catch(() => {});
      await cleanup(uid);
    }
  });
});
