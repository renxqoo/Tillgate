...
function assertCommand(input: BillingReviewCommand): void {
  if (!input.requestId) throw new AppError(409, 'billing_state_conflict', 'requestId is required');
  if (!Number.isInteger(input.expectedRevision) || input.expectedRevision < 0) {
    throw new AppError(409, 'billing_state_conflict', 'expectedRevision must be a non-negative integer');
  }
  if (!input.reason.trim() || input.reason.length > 1000) {
    throw new AppError(400, 'validation_error', 'Review reason is required (1-1000 characters)');
  }
}
    ...
    async retry(ctx, input) {
      ...
      const { receipt, replayed } = await operations.run(ctx, {
        ...
        execute: async (tx) => {
          ...
          if (!retried) {
            throw new AppError(409, 'billing_state_conflict', 'Billing record not found or state changed concurrently');
          }
