...
export function createUsersService(deps: UsersServiceDeps): UsersService {
  ...
  return {
    ...
    async profile(ctx, userId) {
      ...
      // （findAdminUser 基础行不含钱包列——不富化则详情页余额/可用额度为空）
    ...
    async patch(ctx, input) {
      ...
      if (patch.rateCardId != null) {
        ...
        if (card.status !== 0) throw new AppError(400, 'rate_card_disabled', 'Rate card is disabled');
      }
    ...
    async setPassword(ctx, input) {
      ...
      if (user.issuer !== 'local') {
        throw new AppError(400, 'not_local_account', 'Password can only be set for local accounts (issuer=local)');
      }
    ...
    async transactions(ctx, input) {
      ...
      // 游标分页无总数——单独计数供前端分页器（信封 total 字段）
