...
export const USAGE_SORTS = ['id', 'amount', 'inputTokens', 'outputTokens', 'durationMs', 'createdAt'] as const;
...
export interface OpsLogsService {
  usageLogs(
    ctx: RunContext,
    input: { query: ListQueryParts; from?: Date; to?: Date; userId?: number; model?: string; estimated?: boolean },
  ): Promise<{ rows: unknown[]; total: number; page: number; pageSize: number }>;
  requestLogs(
    ctx: RunContext,
    input: { query: ListQueryParts; from?: Date; to?: Date; userId?: number; statusCode?: number | '2xx' | '4xx' | '5xx' },
  ): Promise<{ rows: unknown[]; total: number; page: number; pageSize: number }>;
  auditLogs(ctx: RunContext, query: ListQueryParts): Promise<{ rows: unknown[]; total: number; page: number; pageSize: number }>;
  paymentOrders(
    ctx: RunContext,
    input: { query: ListQueryParts },
  ): Promise<{ rows: unknown[]; total: number; page: number; pageSize: number }>;
  closePaymentOrder(ctx: RunContext, input: { adminId: number; orderId: string }): Promise<{ ok: true }>;
  generationTasks(
    ctx: RunContext,
...
export function createOpsLogsService(deps: OpsLogsServiceDeps): OpsLogsService {
  ...
  return {
    async usageLogs(ctx, input) {
      const result = await repos.usageLog.listAdminUsage({ db, ...ctx }, {
        q: input.query.q,
        from: input.from,
        to: input.to,
        userId: input.userId,
      ...
      return { rows: result.rows, total: result.total, page: input.query.page, pageSize: input.query.pageSize };
    },

    async requestLogs(ctx, input) {
      const result = await repos.usageLog.listRequestLogs({ db, ...ctx }, {
        q: input.query.q,
        from: input.from,
        to: input.to,
        userId: input.userId,
    ...
    async auditLogs(ctx, query) {
      const result = await repos.auditLog.list({ db, ...ctx }, {
        q: query.q,
    ...
    async paymentOrders(ctx, input) {
      const result = await repos.paymentOrder.listAdminOrders({ db, ...ctx }, {
        q: input.query.q,
    ...
    async closePaymentOrder(ctx, input) {
      const closed = await repos.paymentOrder.closeOrder({ db, ...ctx }, {
        orderId: input.orderId,
        reason: '管理员手动关闭',
      });
      if (!closed) {
        throw new AppError(409, 'conflict', '订单不存在或状态不允许关闭');
      }
    ...
    async generationTasks(ctx, input) {
      ...
      const status = input.status && statuses.includes(input.status as (typeof statuses)[number])
        ? (input.status as (typeof statuses)[number])
        : undefined;
      const result = await repos.generationTask.listAdminTasks({ db, ...ctx }, {
        kind: input.kind,
        status,
        limit: input.limit,
        offset: input.offset,
    ...
    async statsOverview(ctx) {
      ...
      const successRate = today.requests === 0 ? 0 : Math.round((today.successCount / today.requests) * 1000) / 10;
      return {
        today: {
