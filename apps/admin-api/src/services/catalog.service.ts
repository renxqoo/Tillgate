...
export const OPENROUTER_SOURCE: CatalogSource = {
  ...
  fetchModels: async () => fetchCatalogJson('https://openrouter.ai/api/v1/models'),
  mapModels: (raw) => mapOpenAiCompatibleCatalog(raw, { currency: 'USD' }),
};

/** 字典型源：models.dev 本地快照（零网络——在线源被墙/不可达不影响货架；
 *  更新重跑 bun run --filter @ai-gateway/admin-api models-dev:refresh，价格 $/1M） */
...
export function createCatalogService(deps: CatalogServiceDeps): CatalogService {
  ...
  function getSource(sourceId: string): CatalogSource {
    ...
    if (!source) {
      throw new AppError(404, 'catalog_source_not_found', `Unknown catalog source: ${sourceId}`);
    }
    return source;
  }
  ...
  return {
    ...
    async comparison(ctx, sourceId) {
      ...
      try {
        ...
        throw new AppError(
          ...
          `Failed to fetch catalog source "${source.name}": ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    ...
    async import(ctx, input) {
      if (input.models.length === 0) {
        throw new AppError(400, 'catalog_empty', 'At least one model must be selected');
      }
      const source = getSource(input.sourceId);
      if (source.channel == null && source.kind !== 'reference') {
        throw new AppError(400, 'catalog_source_invalid', 'Source is missing channel and dictionary configuration');
      }
        ...
        if (source.channel) {
          ...
          if (existingChannel) {
            ...
            if (!input.apiKey && source.channel.needsKey) {
              throw new AppError(400, 'api_key_required', `First import from ${source.name} requires the platform API key (used to create channels)`);
            }
        ...
        for (const m of input.models) {
          ...
          if (existingMapping) {
            if (existingMapping.realModel !== m.realModel) {
              ...
              throw new AppError(409, 'external_name_conflict', `External name ${m.externalName} is already bound to ${existingMapping.realModel}, please choose another name`);
            }
