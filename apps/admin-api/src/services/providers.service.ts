...
function assertProtocol(protocol: string | undefined): string | undefined {
  ...
  if (!SUPPORTED_PROTOCOLS.includes(protocol)) {
    throw new AppError(400, 'invalid_protocol', `Unsupported protocol (allowed: ${SUPPORTED_PROTOCOLS.join(', ')})`);
  }
  return protocol;
}
...
function assertVendor(vendor: string | null | undefined): string | null | undefined {
  ...
  if (!vendorProfileNames().includes(vendor)) {
    throw new AppError(400, 'invalid_vendor', `Unknown vendor profile (allowed: ${vendorProfileNames().join(', ') || 'none'})`);
  }
  return vendor;
}
...
export function createProvidersService(deps: ProvidersServiceDeps): ProvidersService {
  ...
  return {
    ...
    async update(ctx, input) {
      ...
      if (!row) throw new AppError(404, 'provider_not_found', 'Provider not found');
      await recordAudit(db, {
        actor: 'admin',
        adminId: input.adminId,
    ...
    async retire(ctx, input) {
      ...
      if (!ok) throw new AppError(404, 'provider_not_found', 'Provider not found');
      await recordAudit(db, {
        actor: 'admin',
        adminId: input.adminId,
