...
export function createRedeemService(deps: RedeemServiceDeps): RedeemService {
  ...
  return {
    ...
    async detail(ctx, batchId) {
      ...
      if (!batch) throw new AppError(404, 'redeem_batch_not_found', 'Redeem batch not found');
      return batch;
    },
    ...
    async revokeCode(ctx, input) {
      ...
      if (!revoked) throw new AppError(404, 'redeem_code_not_found', 'Redeem code not found, already used or revoked');
      await recordAudit(db, {
        actor: 'admin',
        adminId: input.adminId,
