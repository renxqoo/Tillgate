...
export function createChannelsService(deps: ChannelsServiceDeps): ChannelsService {
  ...
  return {
    ...
    async update(ctx, input) {
      ...
      if (!row) throw new AppError(404, 'channel_not_found', 'Channel not found');
      await recordAudit(db, {
        actor: 'admin',
        adminId: input.adminId,
    ...
    async retire(ctx, input) {
      ...
      if (!ok) throw new AppError(404, 'channel_not_found', 'Channel not found');
      await recordAudit(db, {
        actor: 'admin',
        adminId: input.adminId,
    ...
    async import(ctx, input) {
      if (input.channels.length === 0) {
        throw new AppError(400, 'validation_error', 'Import list cannot be empty');
      }
      if (input.channels.length > deps.importMax) {
        throw new AppError(400, 'validation_error', `Import limit is ${deps.importMax} items per batch`);
      }
      ...
      for (const [index, item] of input.channels.entries()) {
        try {
          ...
          if (!provider) {
            throw new AppError(400, 'provider_not_found', `Provider "${item.provider}" not found, please create it first`);
          }
