 ...
 *
 * 资金相关不变量：
 *   - isFree=true 必须全零价（创建直判；更新按「旧值 ∪ 新值」合并判——
...
/** 校验：isFree=true 必须全零价（矛盾态在服务边界拒绝） */
function assertFreeConsistency(isFree: boolean, prices: ModelPricesInput): void {
  if (!freePriceConsistent(isFree, prices)) {
    throw new AppError(400, 'free_model_price_conflict', 'Explicitly free model requires all-zero pricing (three token prices + cache write price + unit price must all be 0)');
  }
}
...
export function createModelsService(deps: ModelsServiceDeps): ModelsService {
  ...
  return {
    ...
    async create(ctx, input) {
      ...
      if (existing) {
        throw new AppError(409, 'model_exists', `Model mapping "${input.externalName}" already exists (id=${existing.id}, status ${existing.status === 0 ? 'enabled' : 'disabled'}) — use edit for duplicates`);
      }
    ...
    async update(ctx, input) {
      ...
      if (!existing) throw new AppError(404, 'model_not_found', 'Model not found');
      ...
      if (!row) throw new AppError(404, 'model_not_found', 'Model not found');
    ...
    async retire(ctx, input) {
      ...
      if (!ok) throw new AppError(404, 'model_not_found', 'Model not found');
      await recordAudit(db, {
        actor: 'admin',
        adminId: input.adminId,
        action: 'model.retire',
          ...
          if (result.status === 'success') {
            ...
            results.push({
              ...
              error: result.error
                ...
                : { code: 'empty_response', message: 'Upstream returned empty completion' },
            });
          }
        } catch (e) {
          results.push({
