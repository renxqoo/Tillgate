 ...
 * 与 client-api 同语义（管理面无长流，宽限通常瞬过）。
 */
...
export function createShutdown(deps: ShutdownDeps): (signal: string) => void {
  ...
  return (signal: string) => {
    ...
    triggered = true;
    console.log(`[admin-api] ${signal} received, draining`);
    deps.server.close(() => {
      void (async () => {
        await deps.otel.shutdown().catch(() => {});
        await deps.redis?.quit().catch(() => {});
        await deps.db.$client.end().catch(() => {});
        console.log('[admin-api] drained');
        exit(0);
      })();
    });
    setTimeout(() => {
      console.error('[admin-api] drain grace expired, forcing exit');
      exit(1);
