...
export function assembleAdminApi(
  ...
  const redis = createRedisClient(config.REDIS_URL, { serviceName: 'admin-api', ...process.env.REDIS_SENTINELS ? { sentinels: process.env.REDIS_SENTINELS, sentinelName: process.env.REDIS_SENTINEL_NAME, sentinelPassword: process.env.REDIS_SENTINEL_PASSWORD } : {} });
