 ...
 *   （运行时零网络：admin-api 目录源直接读模块；重跑：bun run --filter @ai-gateway/admin-api models-dev:refresh）
 *
...
const header = `// 本文件由 scripts/fetch-models-dev.mts 生成——勿手改（重跑：bun run --filter @ai-gateway/admin-api models-dev:refresh）
