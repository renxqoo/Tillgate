...
async function createUser(): Promise<number> {
  const [u] = await db.insert(users).values({
    ...
    identityProvider: 'local', displayName: 'G2', balance: '1000',
  }).returning();
  return u!.id;
}
async function createApp(userId: number): Promise<{ appId: number; clientId: string }> {
  ...
  const [a] = await db.insert(apps).values({
    appId: appIdField, userId, clientId, clientSecretHash: 'dummy', name: 'g2-app', status: 0,
  }).returning();
...
function makeApp(): unknown {
  ...
  app.use('/v1/*', authMiddleware(db, redis));
...
describe('G2 — 禁用 App 不清 app_status 缓存（JWT 60s 内仍可用）', () => {
  it('禁用 App 后，同一 JWT 应立即失效（401），实际因缓存仍通过', async () => {
    ...
    try {
      const jwt = await signJwt({ userId, appId, coefficient: 1.0, expiresInSeconds: 3600 }, process.env.JWT_SECRET!);
