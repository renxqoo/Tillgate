...
let upstreamKeyPlain = '';

beforeAll(async () => {
  gateway = await startE2EGateway(db);
  restoreBudget = await keys.snapshotChannelBudget(2);
  const enc = await db.$client.query<{ api_key_enc: string }>('select api_key_enc from channels where id = 2');
  upstreamKeyPlain = decrypt(enc.rows[0]!.api_key_enc, encryptionKeyOf());
}, 30_000);
...
describe('E2E ⑪ 认证绕过全家族', () => {
  const jsonEndpoints = [
    '/v1/chat/completions', '/v1/embeddings', '/v1/completions', '/v1/responses', '/v1/messages',
    '/v1/images/generations', '/v1/audio/speech', '/v1/rerank', '/v1/moderations',
    '/v1/video/generations', '/v1/music/generations',
  ];

  it('全部推理/任务端点：无凭证 → 401（一个都绕不过）', async () => {
    const results = await Promise.all(jsonEndpoints.map((path) =>
  ...
  it('模型目录与任务查询：无凭证 → 401；带坏凭证形态（Basic/裸串/JWT 样/空 Bearer）→ 401', async () => {
    ...
    // 逐形态直发网关（不注入有效凭证）
    const direct = await Promise.all(badForms.map((headers) =>
  ...
  it('上游真实密钥零泄露：响应体 / request_logs / usage_logs / 账单 JSON 全扫（明文与密文）', async () => {
    ...
    // 造一个 200（响应体取样）与一个 502（脱敏错误体取样）
    const okRes = await e2ePost(gateway.baseUrl, raw, { model: E2E_MODEL, max_tokens: 100, messages: [{ role: 'user', content: '只回复：好' }] });
    expect(okRes.status).toBe(200);
    const okBody = await okRes.text();
    const errRes = await e2ePost(gateway.baseUrl, raw, { model: `nope-${randomUUID().slice(0, 6)}`, messages: [{ role: 'user', content: 'x' }] });
    const errBody = await errRes.text();
    await keys.settleAll(userId);

    const haystacks = [okBody, errBody];
    // 日志与账单全量取样（该用户相关的每一行每一列的 JSON 文本化）
    const requestLogs = await db.$client.query('select * from request_logs where user_id = $1', [userId]);
    const usageLogs = await db.$client.query('select * from usage_logs where user_id = $1', [userId]);
    ...
    for (const hay of haystacks) {
      expect(hay.includes(upstreamKeyPlain)).toBe(false); // 明文密钥
      expect(hay.includes(enc.rows[0]!.api_key_enc)).toBe(false); // 密文也不出业务表
    }
  }, 120_000);
});

describe('E2E ⑫ 全库数据审计（落库正确性）', () => {
  it('2 非流 + 1 流请求结算后：七张表逐字段核对（快照/份额/公式/流水连续/预算 delta）', async () => {
    const FUND = '1';
    const { raw, userId } = await keys.issue(FUND);
    const before = await db.$client.query<{ budget: string }>('select upstream_budget::text as budget from channels where id = 2');

    const [a, b] = await Promise.all([
      e2ePost(gateway.baseUrl, raw, { model: E2E_MODEL, max_tokens: 150, messages: [{ role: 'user', content: '只回复：甲' }] }),
      e2ePost(gateway.baseUrl, raw, { model: E2E_MODEL, max_tokens: 150, messages: [{ role: 'user', content: '只回复：乙' }] }),
    ]);
    expect(a.status).toBe(200);
    expect(b.status).toBe(200);
    await a.text();
    await b.text();
    const s = await e2ePost(gateway.baseUrl, raw, { model: E2E_MODEL, stream: true, max_tokens: 150, messages: [{ role: 'user', content: '只回复：丙' }] });
    ...
    const bills = await db.$client.query<{
      request_id: string; status: string; reserved_amount: string; channel_id: string;
      lease_expires_at: string | null; released_at: string | null; quote: { candidates: Array<Record<string, unknown>> };
      receipt: Record<string, unknown>;
    }>('select request_id, status, reserved_amount, channel_id, lease_expires_at, released_at, quote, receipt from billing_requests where user_id = $1', [userId]);
    expect(bills.rows.length).toBe(3);
    for (const bill of bills.rows) {
      expect(bill.status).toBe('settled');
      expect(bill.channel_id).toBe('2');
      expect(bill.lease_expires_at).toBeNull(); // 结算后租约清理
      expect(bill.released_at).toBeNull();
      const candidate = bill.quote.candidates[0]!;
      expect(candidate.mappingId).toBe(2); // RX-M3 映射
      expect(candidate.externalModel).toBe(E2E_MODEL);
      expect(candidate.realModel).toBe('MiniMax-M3');
      expect(new Decimal(String(candidate.inputPrice)).eq('2.1')).toBe(true);
      expect(new Decimal(String(candidate.outputPrice)).eq('8.4')).toBe(true);
      const receipt = bill.receipt as unknown as {
        externalModel: string; realModel: string; channelId: string | null; coefficient: string;
      ...
      expect(receipt.realModel).toBe('MiniMax-M3');
      expect(Number(receipt.channelId)).toBe(2);
      expect(receipt.coefficient).toBe('1');
      expect(new Decimal(receipt.inputPrice).eq('2.1')).toBe(true);
      expect(receipt.usage.estimated).toBe(false);
    }
    const streamCount = bills.rows.filter((x) => (x.receipt as unknown as { stream: boolean }).stream === true).length;
    expect(streamCount).toBe(1); // 旗标与请求形态一一对应

    // ---- billing_reservations：份额合计 == 预扣、状态 settled ----
    const reservations = await db.$client.query<{ billing_request_id: string; amount: string; status: string; source_type: string }>(
      'select billing_request_id, amount::text, status, source_type from billing_reservations where billing_request_id = any($1::uuid[])',
      [bills.rows.map((row) => row.request_id)],
    );
    ...
    const usage = await db.$client.query<{
      request_id: string; input_tokens: string; cached_input_tokens: string; output_tokens: string;
      amount: string; input_price: string; output_price: string; cache_input_price: string;
      real_model: string; external_model: string; estimated: boolean; billed_by: string; duration_ms: string; units: string;
    }>('select request_id, input_tokens::text, cached_input_tokens::text, output_tokens::text, amount::text, input_price::text, output_price::text, cache_input_price::text, real_model, external_model, estimated, billed_by, duration_ms::text, units::text from usage_logs where user_id = $1', [userId]);
    expect(usage.rows.length).toBe(3);
    for (const bill of bills.rows) {
      const log = usage.rows.find((u) => u.request_id === bill.request_id)!;
      const ru = (bill.receipt as unknown as { usage: { inputTokens: number; cachedInputTokens: number; outputTokens: number } }).usage;
      ...
      const snapshot = bill.receipt as unknown as { inputPrice: string; cacheInputPrice: string; outputPrice: string; coefficient: string };
      const uncached = ru.inputTokens - ru.cachedInputTokens;
      const expected = new Decimal(uncached).times(snapshot.inputPrice)
      ...
      expect(log.real_model).toBe('MiniMax-M3');
    ...
    const account = await db.$client.query<{ id: string }>(
      'select id from wallet_accounts where user_id = $1 and kind = $2', [userId, 'user'],
    );
    const legs = await db.$client.query<{ amount: string; balance_before: string; balance_after: string; ref_type: string }>(
      `select l.amount::text, l.balance_before::text, l.balance_after::text, t.ref_type
       from wallet_legs l join wallet_transactions t on t.id = l.transaction_id
       where l.account_id = $1 order by l.id`, [account.rows[0]!.id],
    );
    expect(legs.rows.length).toBe(4); // 1 充值 + 3 结算
    expect(legs.rows[0]!.ref_type).toBe('topup');
    for (let i = 1; i < legs.rows.length; i++) {
      expect(new Decimal(legs.rows[i]!.balance_before).eq(legs.rows[i - 1]!.balance_after)).toBe(true); // 链式连续
    ...
    const settleSum = legs.rows.slice(1).reduce((acc, l) => acc.plus(l.amount), new Decimal(0)).abs();
    expect(settleSum.eq(charged)).toBe(true); // 流水合计==usage_logs 合计

    const authorizations = await db.$client.query<{ ref_id: string; amount: string; settled_amount: string; status: string }>(
      `select wa.ref_id, wa.amount::text, wa.settled_amount::text, wa.status from wallet_authorizations wa
       join wallet_accounts acc on acc.id = wa.account_id where acc.user_id = $1 and wa.ref_type = 'billing' and wa.ref_id not like '%#over'`,
      [userId],
    );
    ...
    const requestLogs = await db.$client.query<{ request_id: string; status_code: string; path: string; method: string }>(
      'select request_id, status_code::text, path, method from request_logs where user_id = $1', [userId],
    );
    ...
    const after = await db.$client.query<{ budget: string }>('select upstream_budget::text as budget from channels where id = 2');
    ...
    console.log(`⑫ 审计通过：3 笔 Σ实扣 ${charged.toString()}，流水 ${legs.rows.length} 腿连续，预算 delta ${delta.toString()}`);
  }, 240_000);
});
