/**
 * 终审加固测试：
 *   ① 限流并罚语义：凭证维与用户维各自生效（无 DEFAULT_USER_* 兜底——未设置=不限）
 *   ② playground JWT 拒绝锁：签名合法的 playground 形态 JWT 一律 401（防误恢复）
 */
...
describe('① 限流并罚制（凭证维与用户维各自生效）', () => {
  it('高限额凭证 + 低用户帽 → 用户帽维度拒绝（旧择优制会放行）', async () => {
    ...
    expect(r3.allowed).toBe(false); // 用户帽生效（高限额 Key 不得越过用户帽）
...
describe('② playground JWT 退役锁（BYOK 改造 2026-08-21）', () => {
  it('签名完全合法的 playground 形态 JWT → 401（形态已删，防误恢复）', async () => {
    ...
    // 操练场走用户自持 API Key 直连（client-api 不持有网关签名密钥）——
