/**
 * 【契约红测】流式收据资金口径（review 2026-08-20）：
 *
 * R1（资金·高危）：run-chat.ts 流式分支组装收据时必须透传可信 usage 的
 *    cacheWriteTokens（streamReceiptUsage 已产出该字段、端口契约与结算端
 *    computeAmounts 均支持）——否则缓存写 token 全部落入 uncached 段按
 *    inputPrice 计费。当 cacheWritePrice > inputPrice（Anthropic 官方写价
 *    1.25×/2×）时系统性少收；反向配置则多收。非流式分支（buildReceipt
 *    直传）口径正确，作对照用例。
 *
 * R2（政策·资金公平性）：上游流故障中断（terminated='upstream_error'）且
 *    上游未回 usage 时，estimatedFor 不得归入 ESTIMATE_ATTRIBUTIONS 白名单
 *    放行估算扣费——domain/rating/types.ts 的政策注释：
