...
export interface RateLimitGate {
  ...
  /** 全局 RPM 闸（所有请求并罚 global 维——整站吞吐护栏；0/null = 关） */
...
function reject429(_dimension: string, retryAfterSec: number): AppError {
  ...
  return new AppError(429, 'rate_limit_exceeded', `Request rate or token budget exceeded, retry after ${retryAfterSec}s`);
}

/** 准入限额维度（并罚制）：
 *  每维独立生效，任一超限即拒（多维一次 Lua 原子判定，后维拒绝不污染前维窗口）。
 *  不做「凭证 > 用户」择优——择优制允许高限额 Key 越过用户帽。 */
