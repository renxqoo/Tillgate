...
import { getChannels } from './route-cache.js';
...
function makeCaptureRedis() {
  ...
  return {
    ...
    async incr(key: string) { const v = (Number(store.get(key) ?? '0') || 0) + 1; store.set(key, String(v)); return v; },
...
describe('路由缓存明文 Key 泄露（修复后只存密文 apiKeyEnc）', () => {
  it('getChannels 写入 Redis 的缓存值只含密文 apiKeyEnc，不含明文 apiKey', async () => {
    ...
    const result = await getChannels(dbMock, redis as never, 'gpt-4', ENCRYPTION_KEY);
  ...
  it('从 Redis 读缓存 → 内存解密出明文（缓存命中路径也安全）', async () => {
    ...
    redis.store.set(
      ...
      JSON.stringify([{ channelId: 1, baseUrl: 'https://api.openai.com', apiKeyEnc, protocol: 'openai', providerName: 'openai', key: 'openai/test' }]),
    );
