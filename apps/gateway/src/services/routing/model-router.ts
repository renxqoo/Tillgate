...
export class ModelRouter {
  constructor(
    ...
    private readonly encryptionKeyOld?: string,
  ) {}
  ...
  async getChannels(realModel: string): Promise<ChannelCache[]> {
    ...
    try {
      ...
      if (cached !== null) {
        ...
        return stored.map((s) => ({ ...s, apiKey: decrypt(s.apiKeyEnc, this.encryptionKey, this.encryptionKeyOld) }));
      }
    } catch {
      // Redis 不可用 → 查 DB
    }
    ...
    const memResult: ChannelCache[] = rows.map((row) => ({
      ...
      apiKey: decrypt(row.apiKeyEnc, this.encryptionKey, this.encryptionKeyOld),
