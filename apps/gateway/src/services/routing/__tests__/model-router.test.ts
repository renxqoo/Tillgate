...
describe('route-cache 模型映射缓存', () => {
  it('miss → 查 DB → 回填缓存', async () => {
    ...
    const r1 = await router.getMapping('gpt-4');
    ...
    // 第二次：命中缓存，不查 DB
    const r2 = await router.getMapping('gpt-4');
  ...
  it('缓存「不存在」结果（防穿透：已下架模型不反复查 DB）', async () => {
    ...
    const router = makeRouter(redis, db);
    const r1 = await router.getMapping('removed-model');
    expect(r1).toBeNull();
    expect(db.query.modelMappings.findFirst).toHaveBeenCalledTimes(1);

    // 第二次：命中「不存在」缓存，不查 DB
    const r2 = await router.getMapping('removed-model');
    expect(r2).toBeNull();
    expect(db.query.modelMappings.findFirst).toHaveBeenCalledTimes(1);
  });
  ...
  it('Redis 不可用 → 降级查 DB（不抛错）', async () => {
    ...
    const db = makeMockDb(SAMPLE_MAPPING);
    const router = makeRouter(failingRedis, db);
...
describe('route-cache 渠道解析缓存', () => {
  it('miss → 查 DB → 回填；命中不查 DB', async () => {
    ...
    const router = makeRouter(redis, db);
    const r1 = await router.getChannels('gpt-4-real');
  ...
  it('空渠道列表 → 缓存 EMPTY，防穿透', async () => {
    ...
    const router = makeRouter(redis, db);
    const r1 = await router.getChannels('orphan-model');
    expect(r1).toEqual([]);
    const r2 = await router.getChannels('orphan-model');
    expect(r2).toEqual([]);
    expect(db.select).toHaveBeenCalledTimes(1); // 第二次命中空缓存
  });
});

describe('route-cache /v1/models 列表缓存', () => {
  it('miss → 查 DB → 回填；命中不查 DB', async () => {
    const redis = makeMockRedis();
    const db = makeMockDb(null);
    db.query.modelMappings.findMany = vi
      .fn()
      .mockResolvedValue([{ externalName: 'gpt-4' }, { externalName: 'gpt-3.5' }]);
    const router = makeRouter(redis, db);

    const r1 = await router.listEnabledModels();
    expect(r1).toEqual(['gpt-4', 'gpt-3.5']);
    expect(db.query.modelMappings.findMany).toHaveBeenCalledTimes(1);

    const r2 = await router.listEnabledModels();
    expect(r2).toEqual(['gpt-4', 'gpt-3.5']);
    expect(db.query.modelMappings.findMany).toHaveBeenCalledTimes(1); // 命中缓存
  });

  it('版本 bump → 列表缓存失效重查', async () => {
    const redis = makeMockRedis();
    const db = makeMockDb(null);
    db.query.modelMappings.findMany = vi.fn().mockResolvedValue([{ externalName: 'gpt-4' }]);
    const router = makeRouter(redis, db);

    await router.listEnabledModels();
    await router.invalidate();
    await router.listEnabledModels();
    expect(db.query.modelMappings.findMany).toHaveBeenCalledTimes(2);
  });
});
