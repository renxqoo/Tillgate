...
function makeMockRedis() {
  ...
  return {
    ...
    incr: async (k: string) => {
      const v = String((Number(store.get(k) ?? '0') || 0) + 1);
      store.set(k, v);
      return Number(v);
    },
  };
...
async function callModels(db: Db, auth: Partial<AuthContext>) {
  ...
  const body =
    res.status === 200 ? ((await res.json()) as { data: Array<{ id: string }> }) : { data: [] };
...
describe('/v1/models 应按 JWT scope.models 过滤（红灯 = 风险确认）', () => {
  ...
  it('JWT allowedModels 多元素子集 → 应只返回子集（当前返回全集 → 红）', async () => {
    ...
    expect(body.data.map((m) => m.id).toSorted(), '应只返回白名单内的模型').toEqual([
      'model-a',
      'model-b',
    ]);
  });
  ...
  it('静态分析：models.ts 应基于 allowedModels 过滤（非空实现）', () => {
    const src = readFileSync(
      resolve(dirname(fileURLToPath(import.meta.url)), '../models.ts'),
      'utf8',
    );
