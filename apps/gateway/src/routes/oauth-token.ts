...
export function oauthTokenRoutes(db: Db, redis: Redis): Hono {
  return new Hono().post('/', async (c) => {
    ...
    // 爆破防护：检查是否已锁定
    const attemptKey = `oauth_attempts:${clientId}:${ip}`;
    ...
    if (!app || app.clientSecretHash !== secretHash) {
      // 失败计数（INCR + TTL，首次设过期）
    ...
    logger.info({ clientId, appId: app.id, userId: app.userId }, 'oauth token issued');
