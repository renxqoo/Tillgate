...
async function setup(): Promise<{ userId: number; appId: number; clientId: string }> {
  const [u] = await db.insert(users).values({
    ...
    identityProvider: 'local', displayName: 'G3', balance: '1000',
  }).returning();
  ...
  const [a] = await db.insert(apps).values({
    appId: appIdField, userId: u!.id, clientId, clientSecretHash: secretHash, name: 'g3-app', status: 0,
  }).returning();
...
async function cleanup(userId: number, appId: number, clientId: string): Promise<void> {
  ...
  let cursor = '0';
  do {
    const [next, keys] = await redis.scan(cursor, 'MATCH', `oauth_attempts:${clientId}:*`, 'COUNT', 100);
    cursor = next;
