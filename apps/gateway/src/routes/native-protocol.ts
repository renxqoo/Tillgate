/**
 * 原生协议端点：模型名在 URL 的路径参数形态——端点注册表（固定路径）表达不了。
...
export function geminiNativeRoutes(runChat: RunChat): Hono<AuthEnv> {
  return new Hono<AuthEnv>().post('/v1beta/models/:modelAction', async (c) => {
    ...
    if (!m) {
      throw new AppError(404, 'not_found', 'Path not found (supported: :generateContent / :streamGenerateContent)');
    }
    const model = m[1]!;
    const stream = m[2] === 'streamGenerateContent';
    const raw = (await c.req.json().catch(() => null)) as Record<string, unknown> | null;
    if (!raw) throw new AppError(400, 'invalid_body', 'Request body must be a JSON object');
