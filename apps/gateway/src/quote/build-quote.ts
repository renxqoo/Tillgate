...
export function createBuildQuote(deps: { db: Db; repos?: Repositories }) {
  ...
  return async function buildQuote(ctx: RunContext, input: BuildQuoteInput): Promise<BillingQuote> {
    ...
    if (!main) {
      throw new AppError(404, 'model_not_found', `Model ${input.model} not found or deprecated`);
    }
    ...
    if (snapshot != null && snapshot.status !== 0) {
      throw new AppError(403, 'rate_card_disabled', 'The rate card bound to this account is disabled, please contact the administrator');
    }
