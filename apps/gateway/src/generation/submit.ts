...
export function createSubmitGeneration(deps: SubmitGenerationDeps) {
  ...
  return async function submitGeneration(
    ...
    if (descriptor == null) throw new AppError(404, 'not_found', `Unknown generation kind ${kind}`);
    ...
    if (auth.allowedModels != null && !auth.allowedModels.includes(externalModel)) {
      throw new AppError(403, 'model_not_allowed', `Model ${externalModel} is not allowed for this credential`);
    }
    ...
    let lastError = { code: 'upstream_error', message: 'no available channel' };
    ...
    const persist = async (
      ...
      try {
        ...
        throw new AppError(503, 'billing_receipt_unavailable', 'Task registration temporarily unavailable, please retry later');
      }
      return {
        status: 201,
        body: {
    ...
    for (const candidate of quote.candidates) {
      ...
      for (const channel of channels) {
        ...
        if (!reservation.allowed) {
          lastError = { code: 'channel_budget_exhausted', message: 'channel budget exhausted' };
          continue;
        }
        await ensureLease();
