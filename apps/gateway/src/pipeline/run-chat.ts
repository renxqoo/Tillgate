...
/** 端点分类（显式 endpoint 的纯函数——单一真相：路由边界声明的端点；
 *  chat/embeddings 之外的一切都属模态族）。 */
...
export type ChatResponse =
  ...
  /** 二进制成功（音频等）：字节体 + 上游 content-type（JSON 信封会毁掉字节流） */
  | { status: 200; rawBody: Uint8Array; rawContentType: string };

export function createRunChat(deps: PipelineDeps) {
  const repos = deps.repos ?? createRepositories();
  const noteError =
    deps.onError ?? ((error, context) => console.error(`[pipeline] ${context}:`, error));
  return async function runChat(
    ...
    // 请求级 TTFT 锚点：每请求进入时打点（放工厂级会拿到进程装配时刻，client_ttft 虚高为「开机到首字」）
    ...
    const outputCap = maxOutputTokensFor(
      kind === 'modality' ? 'embeddings' : kind,
      body,
      deps.config.output,
    );
    ...
    // （其余罕见异常路径由 600s 预占 TTL 自回收）
    let quote;
    try {
      quote = await withAsyncSpan(
        tracer,
        'quote.build',
        {
          'request.id': requestId,
        ...
        async (span) => {
          ...
          span.setAttributes({
            'quote.candidates': q.candidates.length,
            'quote.free': q.explicitlyFree,
          });
          return q;
        },
      );
    ...
    if (quote.candidates.length === 0) {
      ...
      if (deps.rateLimit) await deps.rateLimit.limiter.releaseTpm(requestId).catch(() => {});
      throw new AppError(404, 'model_not_found', 'Model not found or deprecated');
    }
    // 模型维 TPM 预占（主 + fallback 候选 mappingId 一并占住——fallback 切换时不再二次判定）
    if (deps.rateLimit) {
      await withAsyncSpan(
        tracer,
        'rate_limit.reserve_model',
        {
          'request.id': requestId,
          'ai.model': body.model,
          'tokens.estimate': estimatedTokens,
    ...
    let authorization;
    try {
      authorization = await withAsyncSpan(
        tracer,
        'billing.authorize',
        {
          'request.id': requestId,
      ...
      withAsyncSpan(
        ...
        async (span) => {
          span.setStatus({
            ...
            message: error.code ?? 'upstream_client_error',
          });
          if (deps.rateLimit) await deps.rateLimit.limiter.releaseTpm(requestId).catch(() => {});
    ...
    const releaseAndFail = async (): Promise<never> =>
      withAsyncSpan(
        ...
        async () => {
          if (deps.rateLimit) await deps.rateLimit.limiter.releaseTpm(requestId).catch(() => {});
          // 从未得到上游响应、纯渠道面拒绝（限流/预算耗尽）= 503 no_available_channel。
          ...
          await deps.billing.signal(ctx, {
            type: 'request.failed',
            requestId,
            reason: (channelExhausted
              ? 'no_available_channel'
              : (lastError?.code ?? 'no_available_channel')
            ).slice(0, 64),
          });
          if (channelExhausted) {
    ...
    for (const candidate of quote.candidates) {
      const channels = await withAsyncSpan(
        tracer,
        'routing.resolve',
        {
          'request.id': requestId,
      ...
      for (const channel of channels) {
        ...
        if (
          deps.rateLimit &&
          !(await tryChannel(deps.rateLimit, {
            requestId,
          ...
          lastError = { code: 'rate_limit_exceeded', message: 'channel rate limited' };
          await withAsyncSpan(
            tracer,
            'channel.skip',
            {
        ...
        if (!(await startChannel(channel.channelId, upstreamEstimate, channel.channelName)))
          continue;
