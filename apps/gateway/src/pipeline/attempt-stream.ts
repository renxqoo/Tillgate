...
export async function attemptStream(input: AttemptInput): Promise<AttemptOutcome> {
  ...
  const { streamResult, decisive, ttft } = await withAsyncSpan(
    tracer,
    'upstream.attempt',
    {
    ...
    async (span) => {
      ...
      const d = await new Promise<UpstreamStreamEvent>((resolve) => {
        let settled = false;
        s.onEvent((event) => {
          // TTFT 锚点采集（旁路赋值，零阻塞）：first_chunk 一次性（decisive 前到达也先记下）
          if (event.type === 'first_chunk') firstChunkAt = event.atMs;
          if (settled) return;
      ...
      return { streamResult: s, decisive: d, ttft: { firstChunkAt } };
    },
  );
  ...
  // 长流续租：租约 = authorizationTtlMs，每 1/3
  ...
  streamResult.onEvent(async (event) => {
    ...
    const receipt: UsageReceipt = {
      ...buildReceipt({
        ...
        usage: finality.usage.estimated
          ...
          : {
              ...
              ...((finality.usage.cacheWriteTokens ?? 0) > 0
                ? { cacheWriteTokens: finality.usage.cacheWriteTokens }
                : {}),
            },
      }),
      ...
      // 首字延迟观测（流式专属；非流式 buildReceipt 不装配 → 落库 NULL）
      ...(ttft.firstChunkAt > 0
        ? {
            // 上游锚点 = 本次渠道 chatStream 发起（startedAt，换渠后即成功渠道的）；
            // 客户端锚点 = runChat 进入（requestStartedAt）——含限流/报价/授权/路由与全部
            // 换渠等待（= 用户从提交到首 token 的真实体感，鉴权中间件在外层）；
            // client − upstream ≈ 管道前置开销 + 换渠重试等待。
            upstreamTtftMs: Math.max(0, ttft.firstChunkAt - startedAt),
            clientTtftMs: Math.max(0, ttft.firstChunkAt - input.requestStartedAt),
          }
        : {}),
    };
    ...
    if (!finalized) {
      input.noteError(
        ...
        `stream finalize giveup request=${requestId}`,
      );
    }
    streamAlive = false;
  ...
  return {
    kind: 'respond',
    response: { status: 200, stream: streamResult.stream, contentType: 'text/event-stream' },
  };
}
