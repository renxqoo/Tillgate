  ...
  if (result.ok) {
    ...
    const receipt = buildReceipt({
      ...
      usage: result.usage
        ...
        : { estimated: true, inputTokens: input.bpeInput, outputTokens: estimateOutputTokens(result.body, { model: body.model }) },
    });
    const finalized = await signalSucceededWithRetry(deps.billing, ctx, requestId, receipt, input.noteError);
    if (!finalized) throw new AppError(503, 'finalize_unavailable', 'Request completed but settlement is temporarily unavailable, please retry');
    if (result.rawBody) {
      return { kind: 'respond', response: { status: 200, rawBody: result.rawBody, rawContentType: result.rawContentType ?? 'application/octet-stream' } };
    }
