...
export function createUpstreamAdapter(deps: {
  ...
  const deadline = () => (deps.deadlineMs != null ? { deadlineMs: deps.deadlineMs } : {});
  return {
    async chat(
      candidate: RouteCandidateRow,
        ...
        const mapped: UpstreamStreamEvent | null =
          event.type === 'failed'
            ...
            : event.type === 'first_chunk'
              ? { type: 'first_chunk', atMs: event.atMs }
              : event.type === 'success'
                ? {
                    type: 'success',
                    ...(event.usage
                    ...
                    ...(event.bytesRelayed !== undefined
                      ? { bytesRelayed: event.bytesRelayed }
                      : {}),
  ...
  return new ReadableStream<Uint8Array>({
    async pull(controller) {
      ...
      if (pump.failure !== null) {
        controller.error(
          pump.failure instanceof Error ? pump.failure : new Error(String(pump.failure)),
        );
        return;
      }
