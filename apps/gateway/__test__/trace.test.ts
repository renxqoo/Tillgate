...
const rateLimit: RateLimitGate = {
  limiter: {
    ...
    renewTpm: async () => undefined,
    backfillTpm: async () => undefined,
...
function makeApp() {
  const inference = createInference({
    ai: { subscribe: () => () => undefined } as unknown as Ai,
    catalog,
    billing,
  ...
  return createGatewayApp({
    ...
    requestLogs: { insert: async () => undefined } as unknown as RequestLogStore,
...
describe('adapters/trace-port：OTel 绑定契约', () => {
  it('setAttributes/setStatus 双态映射进真实 span（ok/error 分 span——OTel 语义 OK 后 ERROR 不可覆盖）', async () => {
    ...
    const byName = Object.fromEntries(
      otel.memory!.recent().flatMap((t) => t.spans.map((s) => [s.name, s])),
    );
    expect(byName['tp.ok']!.attributes).toMatchObject({ probe: 1, added: true });
    expect(byName['tp.ok']!.status.code).toBe(1); // SpanStatusCode.OK
    expect(byName['tp.error']!.status).toMatchObject({ code: 2, message: 'boom' }); // ERROR
  });
});
...
describe('全链路 span 树（memory OTel + 真实 inference 装配）', () => {
  it('非流式 chat：完整请求路径 ≥9 span，同一 traceId，全部挂 HTTP 根 span 之下', async () => {
    ...
    const traces = otel.memory!.recent();
    ...
    expect(trace.spans.find((s) => s.name === 'inference.prepare')!.attributes['request.id']).toBe(
      echoed,
    );
  });
