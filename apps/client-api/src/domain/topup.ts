...
export function assertTopupWithinLimit(amount: string, min: string, max: string): void {
  ...
  if (value.decimalPlaces() > 2) {
    throw new AppError(400, 'invalid_amount', 'Top-up amount allows at most 2 decimal places');
  }
  if (value.lessThan(min)) {
    throw new AppError(400, 'invalid_amount', `Top-up amount is below the minimum ${min}`);
  }
  if (value.greaterThan(max)) {
    throw new AppError(400, 'invalid_amount', `Top-up amount is above the maximum ${max}`);
  }
}
