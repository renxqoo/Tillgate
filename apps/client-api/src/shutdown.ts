...
export function createShutdown(deps: ShutdownDeps): (signal: string) => void {
  ...
  return (signal: string) => {
    ...
    triggered = true;
    console.log(`[client-api] ${signal} received, draining`);
    deps.server.close(() => {
      void (async () => {
        await deps.otel.shutdown().catch(() => {});
        await deps.redis?.quit().catch(() => {});
        await deps.db.$client.end().catch(() => {});
        console.log('[client-api] drained');
        exit(0);
      })();
    });
    setTimeout(() => {
      console.error('[client-api] drain grace expired, forcing exit');
      exit(1);
