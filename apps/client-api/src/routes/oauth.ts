...
export function oauthRoutes(service: OAuthService, deps: { secureCookie: boolean }) {
  ...
  app.get('/v1/oauth/:provider/authorize', async (c) => {
    ...
    if (!provider) return c.json({ error: { code: 'oauth_unknown', message: 'Unknown login method' } }, 404);
    try {
      ...
      return c.json({ error: { code, message: 'This login method is unavailable' } }, status as 404);
    }
  });
  ...
  app.get('/v1/oauth/:provider/callback', async (c) => {
    ...
    try {
      ...
      const status = (e as { status?: number }).status ?? 502;
      const code = (e as { code?: string }).code ?? 'oauth_failed';
      return c.json({ error: { code, message: 'Third-party login failed, please retry' } }, status as 404);
    }
  });
