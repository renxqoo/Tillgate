...
import { type Mailer, type CaptchaService } from '@ai-gateway/identity';
...
/** 恒定结果的验证器桩（Turnstile 测试 siteKey 造型） */
...
describe('注册面人机验证门禁', () => {
  it('GET /captcha 能力端点：启用返回 siteKey，未启用返回 null', async (ctx) => {
    if (!connected) return ctx.skip();
    const on = makeApp(stubCaptcha({ ok: true }));
    const off = makeApp(null);

    const resOn = await on.app.request('/api/auth/captcha');
    expect(resOn.status).toBe(200);
    expect(((await resOn.json()) as { siteKey: string | null }).siteKey).toBe('1x00000000000000000000AA');

    const resOff = await off.app.request('/api/auth/captcha');
    expect(resOff.status).toBe(200);
    expect(((await resOff.json()) as { siteKey: string | null }).siteKey).toBeNull();
  });
