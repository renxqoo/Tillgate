 ...
 * 禁用 / 轮换密钥。计费来源订阅归属守卫与 Key 同口径（绑他人订阅 404）。
 */
  ...
  return {
    async create(ctx, userId, input) {
      ...
      const row = await db.transaction(async (tx) => {
        // App 配额闸（无闸可无限建 App）——advisory lock 防双击竞态
    ...
    async disable(ctx, userId, appId) {
      ...
      if (!owned) throw new AppError(404, 'app_not_found', 'App not found');
      if (owned.status !== 0) throw new AppError(409, 'app_already_disabled', 'App already disabled');
      const disabled = await db.transaction(async (tx) =>
        repos.apps.disableApp({ db: tx, ...runCtx }, { userId, appId }),
      );
      if (!disabled) throw new AppError(409, 'app_already_disabled', 'App already disabled');
      await recordAudit(deps.db, {
        actor: 'user',
        action: 'app.disable',
        targetType: 'app',
    ...
    async rotateSecret(ctx, userId, appId) {
      ...
      if (!rotated) throw new AppError(404, 'app_not_found', 'App not found');
      await recordAudit(deps.db, {
        actor: 'user',
