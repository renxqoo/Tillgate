...
export interface PaymentsService {
  ...
  /** 订单详情（GET /api/payments/:id——支付后轮询） */
...
export function createPaymentsService(deps: PaymentsServiceDeps): PaymentsService {
  ...
  const resolveProvider = (requested?: 'epay' | 'stripe'): PaymentProviderPort => {
    if (requested) {
      ...
      if (!found) throw new AppError(503, 'payment_unavailable', 'This payment channel is not enabled');
      return found;
    }
    if (deps.providers.length === 1) return deps.providers[0]!;
    throw new AppError(503, 'payment_unavailable', 'This payment channel is not enabled');
  };
    ...
    async createTopupOrder(ctx, userId, input) {
      if (deps.orderLimiter) {
        ...
        if (n > (deps.perMinuteOrderLimit ?? 10)) {
          throw new AppError(429, 'topup_rate_limited', 'Too many order requests, please retry later');
        }
      }
      ...
      try {
        ...
        await db.$client
          ...
          .catch(() => undefined);
        throw new AppError(502, 'payment_channel_unavailable', 'Payment channel temporarily unavailable, please retry later');
      }
      ...
      try {
        await db.transaction(async (tx) => {
          ...
          if (!paid) {
            ...
            if (fresh == null) throw new AppError(409, 'order_state_conflict', 'Order not found');
            ...
            if (fresh == null || fresh.status !== 1) {
              throw new AppError(409, 'order_state_conflict', 'Order state transition conflict');
            }
          }
          ...
          if (!done) throw new AppError(409, 'order_state_conflict', 'Order state transition conflict');
        });
        return 'success';
      } catch (e) {
