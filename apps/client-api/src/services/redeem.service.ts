...
export function createRedeemService(deps: RedeemServiceDeps): RedeemService {
  ...
  return {
    async redeem(ctx, userId, input) {
      ...
      if (!code) throw new AppError(400, 'invalid_code', 'Redeem code must not be empty');
      ...
      if (n > deps.perMinuteLimit) {
        throw new AppError(429, 'redeem_rate_limited', 'Redeem attempts too frequent, please retry later');
      }
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (!claim) {
          ...
          if (!row) throw new AppError(404, 'invalid_code', 'Invalid redeem code');
          // 区分「已使用」与「已吊销」（运营撤回的码语义不同）
          if (row.status === 2) throw new AppError(409, 'code_revoked', 'Redeem code has been revoked');
          if (row.status !== 0) throw new AppError(409, 'code_already_used', 'Redeem code already used');
          throw new AppError(410, 'code_expired', 'Redeem code has expired');
        }
