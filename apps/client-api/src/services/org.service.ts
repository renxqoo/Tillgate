...
export function createOrgService(deps: { db: Db; repos?: Repositories; clock?: () => Date }): OrgService {
  ...
  async function requireOwner(
    ...
    if (!membership || membership.role !== 'owner') {
      throw new AppError(403, 'org_forbidden', '无权操作该组织（仅 owner 可操作）');
    }
    return membership;
  }
  ...
  return {
    ...
    async orgDetail(ctx, userId, orgId) {
      ...
      if (!membership) throw new AppError(404, 'org_not_found', '组织不存在或你不在其中');
      const org = await repos.org.findOrg(runCtx, orgId);
      if (!org) throw new AppError(404, 'org_not_found', '组织不存在或你不在其中');
    ...
    async invite(ctx, userId, orgId, email) {
      ...
      if (!sub) throw new AppError(409, 'org_no_subscription', '组织尚无有效套餐，无法邀请');

      const activeCount = await repos.org.countActiveMembers(runCtx, orgId);
      if (activeCount >= sub.quantity) {
        throw new AppError(409, 'seats_full', '席位已满，无法再邀请');
      }
      ...
      if (pending >= pendingCap) {
        throw new AppError(409, 'invitations_full', '待接受邀请过多，请先撤销或等待处理');
      }
    ...
    async acceptInvitation(ctx, userId, token) {
      ...
      if (!user) throw new AppError(401, 'unauthorized', '账号不存在');

      const inv = await repos.org.findInvitationByToken(runCtx, token);
      // 快路径分型提示（权威校验在事务内的原子翻转）
      if (!inv) throw new AppError(404, 'invitation_invalid', '邀请链接无效或已过期');
      if (inv.status === 2) throw new AppError(409, 'invitation_revoked', '邀请已被撤销');
      if (inv.status === 1) throw new AppError(409, 'invitation_already_accepted', '邀请已被接受');
      if (inv.expiresAt <= clock()) throw new AppError(410, 'invitation_expired', '邀请已过期');
      // 登录账号须与邀请 email 一致（无 email 时按 subject 兜底）
      const userEmail = user.email?.toLowerCase();
      if (userEmail !== inv.email.toLowerCase() && user.subject !== inv.email) {
        throw new AppError(403, 'invitation_email_mismatch', '登录账号与邀请邮箱不一致');
      }
      ...
      await db.transaction(async (tx) => {
        ...
        if (!sub) throw new AppError(409, 'org_no_subscription', '组织套餐已失效');
        const activeCount = await repos.org.countActiveMembers(c, inv.orgId);
        if (activeCount >= sub.quantity) {
          throw new AppError(409, 'seats_full', '席位已满');
        }
    ...
    async patchMember(ctx, userId, orgId, memberUserId, patch) {
      ...
      if (!patched) throw new AppError(404, 'org_member_not_found', '成员不存在');
    },
    ...
    async removeMember(ctx, userId, orgId, memberUserId) {
      ...
      if (memberUserId === userId) {
        throw new AppError(409, 'org_cannot_remove_owner', 'owner 不能被移除');
      }
