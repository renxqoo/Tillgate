 ...
 *
 * 会话形态：Bearer 无 Cookie 会话——回调把会话 token 放 URL fragment
...
export function createOAuthService(deps: OAuthServiceDeps): OAuthService {
  ...
  const requireCreds = (provider: OAuthProviderName) => {
    ...
    if (!conf) {
      throw new AppError(404, 'oauth_not_configured', 'This login method is not configured, please contact the administrator');
    }
    return conf;
  };
  ...
  async function fetchProfile(provider: OAuthProviderName, code: string): Promise<OAuthProfile> {
    ...
    try {
      ...
      throw new AppError(502, 'oauth_exchange_failed', 'Third-party login exchange failed, please retry');
    }
  }
  ...
  return {
    ...
    async authorize(provider, next) {
      ...
      try {
        ...
        throw new AppError(503, 'oauth_state_unavailable', 'Login state store unavailable, please retry later');
      }
    ...
    async callback(ctx, input) {
      ...
      if (!input.cookieState || input.cookieState !== input.state) {
        throw new AppError(403, 'oauth_state_mismatch', 'Login state validation failed, please start the login again');
      }
      let next = '/dashboard';
      ...
      if (!stored) {
        throw new AppError(410, 'oauth_state_expired', 'Login state already used or expired, please start the login again');
      }
      if (stored.provider !== input.provider) {
        throw new AppError(403, 'oauth_state_mismatch', 'Login method mismatch');
      }
      next = stored.next;
      if (!input.code) throw new AppError(400, 'oauth_invalid', 'Missing authorization code');
