 ...
 * v1 apps.subscription-guard（W1）与 keys.rotate-cache（L1）的语义等价覆盖。
 * v2 网关每请求查库无鉴权缓存——「PATCH 后清缓存」问题结构性不存在（断言即时生效）。
 */
...
describe('Apps 凭证（W1 归属守卫）', () => {
  ...
  it('RED（v1 W1 同类）：绑他人订阅 → 404 不得 201', async () => {
    const owner = await newEnterpriseUser();
...
describe('Key 轮换（L1 降级语义）', () => {
  ...
  it('RED（v1 L1 同类）：绑定的订阅已过期 → 轮换产出的新 Key 降级为个人余额', async () => {
    const account = await newUser();
