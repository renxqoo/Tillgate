...
export function RegisterForm({
  ...
  /** 后端 GET /v1/auth/capabilities 下发；null = 未启用，不渲染 widget */
  ...
  function onSubmit(values: RegisterValues) {
    startTransition(async () => {
      ...
      if (res?.challengeId) setChallenge(res.challengeId);
        ...
        // 人机验证被拒（命名空间码）：token 已消费作废，强制换票后让用户重试
        if (res?.code === 'client.captcha_required' || res?.code === 'client.captcha_invalid') {
          setCaptchaToken(null);
          setCaptchaResetNonce((n) => n + 1);
        }
        actionResult(res ?? {}, t('registerFailed'));
      }
    });
  }
    ...
    <Card className="shadow-sm">
      ...
      <CardContent>
        <form noValidate onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <FieldDescription className="text-center">{t('hasAccountInline')}</FieldDescription>
        </form>
        <div className="mt-4">
