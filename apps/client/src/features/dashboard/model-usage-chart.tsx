...
/**
 * 不同模型使用量（横向条形图 Top10）。一次拉全量数据，前端用 ToggleGroup
 * 切换度量（费用 / Token / 次数 / 缓存率），无需重新请求。
 */
export function ModelUsageChart({ data }: { readonly data: ReadonlyArray<UsageByModelItem> }) {
  ...
  const METRICS: Record<Metric, MetricMeta> = {
    cost: { label: t('costLabel'), format: (v) => formatMoney(v, locale) },
    ...
    <div className="space-y-3">
      <ToggleGroup
        ...
        onValueChange={(values: string[]) => {
          const next = values.find((v) => v !== metric);
