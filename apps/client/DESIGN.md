...
- 设计系统 → `@tokenlens/ui`（本 app 零第二套视觉原语；app 内组件仅业务装配）；
- transport/DTO → `@tokenlens/api-client`（本 app 零手写 fetch 细节、零第二套 DTO）；
...
- 解析：`src/i18n/request.ts` 用 `@tokenlens/api-client/next` 的 `resolveLocale`（cookie → 头 → en），无路由段、SSR 全量渲染。
